...
// PjD 2.0（2026-07-27 再設計・正本: obsidian-vault/30-Projects/videodl/pjd-2026-07-27.md）
const PLAN: string[] = [
  '再設計（PjD 2.0・2026-07-27）：「Google待ち」をやめ、今日流入を作れるチャネルを主戦場にする。',
  'Track A（主戦場）：高DAプラットフォーム配信（Tumblr/Blogger/note/知恵袋）＝他人のドメインで検索結果に出る。即インデックス・被リンク・直接流入の3点セット。',
  'Track B（全自動）：Bing/DDG/Yandex 先行SEO。唯一インデックス済みの土俵で需要データを先に学ぶ。',
  'Track C（後追い）：Google はsitemap＋被リンクの自然クロール任せ。GSC手動申請の依頼は廃止（10巡で実行されず＝設計から除外）。',
  '生産は凍結：LP72本で在庫十分。新規LP・ロングテール夜間量産は休止し、「作る」から「配る」へ全面切替。',
  'KPI：北極星＝実UU/週（現状6→4週後50）。先行＝参照元ドメイン数（0→3+）・検索流入/日（0→3+）。',
  'ゲート：8/24 実UU50/週 → 9/21 実UU200/週。未達なら維持モード（自動運転のみ）へ縮退。収益化はゲート2通過まで凍結。',
];
const DO: string[] = [
  '【2026-07-27】PjD 2.0 再設計：正本作成（pjd-2026-07-27.md）・PDCAループのPlan生成を新方針へ差し替え・ロングテール夜間autogen休止。',
  '継続（自動）：IndexNow 日次送信・第一者計測・日次レポート。',
  '準備中：配信用ドラフト9本（drafts/）を最新LP構成に合わせて更新→承認フローへ。',
  '済（資産）：LP72本・広告5面配信正常（CPM $1.87）・ランキング型LP（MC-347）・多言語SEO/sitemap。',
];
const CHECK_READ: string[] = [
  '10巡の学び：LPを25→72本に3倍化しても実UUは3→6。生産はボトルネックでない。',
  'Google未インデックスが継続（新規ドメインのサンドボックス）。Bing/DDGはインデックス済み＝唯一の土俵。',
  '広告は全5ゾーン配信正常・収益 $0.075/7d。問題は広告でなく純粋にトラフィック量。',
  'Keita手動レバー（GSC申請・投稿）は10巡実行されず→「実行されない前提の計画」を排除するのがPjD 2.0の骨子。',
];
const ACT: string[] = [
  '【Keita・一度きり15分の立ち上げパック】①GSC OAuth再同意（apollomansion.com/api/google/oauth/start）②Bing Webmaster Tools登録 ③配信アカウント方針の一言（新規捨てアカでOKか）。以後の日次お願い通知は廃止。',
  'Son：配信ドラフト（Tumblr/Blogger/note/知恵袋）を最終化し、承認フローに順次載せる。',
  '自動：Bing検索結果の掲載観測をCheckに追加。GSC復活後は表示クエリからクラスタ深掘り対象を特定。',
  '8/24 ゲート1判定：実UU50/週 or 参照ドメイン3+。結果をレポートで明示判定。',
];
