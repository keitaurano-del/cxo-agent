...
| MC-156 | Apollo ターミナルビューを2/3/4分割で同時表示できるレイアウト切替を追加（現状はタブで1つずつ表示） | P1 | DONE（2026-06-07 iframe フォーカス検知実装完了。Terminal.tsx L1158 + L1215-1221 で pointerdown 透過オーバーレイを追加し focusedPane を同期。分割表示時のみ有効、単一表示は無効。tsc/eslint/build green（2026-06-07 16:45）） | dev-logic |
