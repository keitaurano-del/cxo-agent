...
| MC-361 | モックアップに図解を標準化 | 高 | IN_PROGRESS（8/8 Son巻き取り確定→着手時に devMockupRouter.ts へ本日の他者WIP(越境テーマ)を検知=型C衝突につき、そのcommit完了待ちで適用保留。詳細→ tasks/MC-361.md） | Son(旧dev-apollo) | MC-360依存。詳細→ tasks/MC-361.md |
...
| MC-370 | Blueairレンタル事業立上げ | 高 | IN_PROGRESS（8/8 需要分析・LPモック・プランv2（法人なし採算・AirRent・新品定価買取）完成→/work?tab=blueair 本番反映済。11:15 ドメイン空きRDAP実査＝airrent.jp含む.jp系すべて空き・.com/.netは取得済→airrent.jp推奨。残=GO/NO-GO・仕入方針・ドメイン取得(決済)のKeita判断待ち。詳細→ tasks/MC-370.md） | Son | Keita依頼8/8「Classic Pro特化レンタル、月商10万・極力自動化」＋「仕事の独立タブにまとめて」。新規事業 |
