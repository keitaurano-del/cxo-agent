...
| MC-173 | Apollo ターミナル: agent busy 時にメッセージをキューに積み、ターミナルが空いたら自動送信する | P1 | IN_PROGRESS（2026-06-07 22:15 JST 林 cxo ティック開始。dev-logic に委譲。★2026-06-07 18:55 Keita「進めて」＝171/174と並行で前進指示。蓮が実装継続） ★19:01 注記:MC-174/175 push 時に祖先 9347cee(MC-173途中)も origin/main へ同時push。以降は新規コミットで積む(amend/force-push不可)。IN_PROGRESS継続 | dev-apollo（ソラ）/ dev-logic（蓮） | なし |
