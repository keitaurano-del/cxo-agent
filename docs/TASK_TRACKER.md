...
| 優先度 | P2 |
| ステータス | DESIGN（2026-06-16 Son・設計書作成→要 Keita レビュー）。Keita「一旦任せるので設計書作って」。**競合リサーチ(2026-06-15 deep-research)**の示唆を反映＝完全自動リスケの最大不満は「コントロール喪失」→我々の提示→承認→書き戻し路線は正解、それを強化。設計書=`docs/MC-250-planner-p4-design.md`。4本柱: ①優先度4段(P1-P4)＋締切が優先度を上書き・低優先は溢れ許容 ②安定性(sticky再プランで前回配置を維持・apply済/着手は自動ロック・movedCount表示) ③集中時間/習慣枠の先取り ④未配置を deadline-miss(要対応・強調)/no-capacity/no-due-overflow に分離＋完了連携。**無確認の自動リスケはしない**を原則明文化。 |
