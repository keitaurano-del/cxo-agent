...
| MC-165 | MC-164 の作り込み：エージェントをドット絵キャラ＋吹き出しで擬人化（現状チープ・誰が誰か不明） | P1 | IN_PROGRESS（2026-06-07 林着手→両者brief完成。①designBrief（artifacts/MC-165_DESIGN_BRIEF.md）: 64×64pxピクセルアート12種(6人格×2状態)、Phase1レンサンプル→Keita承認→Phase2-3展開、工数14-18h、deadline2026-06-12。②implementBrief: personaMap.ts/AgentGroup拡張/PersonaCard.tsx新規、SSEリアルタイム状態切り替え(working/idle)、工数6-10h並行。並行スケジュール: designPhase1→approval→devPhase開始。DoD: 12asset完成+React統合+実機検証+Keita最終承認。次: designerレンsample作成開始→実装skeletonコード開始。【Keita 要望 2026-06-07：ドット絵の実装前に「見れる形」で出す＝アオイは Phase1 サンプル(最低レン+ソラ)を Apollo チャットに画像投稿 or Figma リンクで提示し、Keita の確認を取ってからソラがアートを統合する。ソラは skeleton/配線まで進めてよいが、最終アートの組み込みは Keita のデザイン承認後】） | designer（アオイ）＋dev-apollo（ソラ） |
| MC-166 | Keita がボードで手動変更した status を自動 🔒 ロックし、リコンサイル/keeper/ガードが戻さないようにする | P1 | REVIEW（2026-06-07 実装完了。dev-logic(蓮)が server /api/tasks/status-lock endpoint 追加・web TaskDetail.tsx で status 単独変更時に新 endpoint 呼び出し・status セルに🔒[Keita]自動付与＋git commit で HEAD 即反映。commit 98a6f63。tsc/eslint green。server/src/lib/taskTrackerWrite.ts に updateTaskStatusWithLock()、server/src/taskEditRouter.ts に /status-lock handler、web/src/components/TaskDetail.tsx で status-lock 分岐。Apollo サーバは即座にコード読み込み。残：Masayoshi が guard 側の lock 認識拡張（別ティック）。Keita 検証後に統合） | dev-logic(蓮)実装＋Masayoshi(guard側) |
| MC-167 | タスク詳細ビューの整理：無関係な3欄を削除し、タスク自身の履歴に集約 | P1 | IN_PROGRESS（2026-06-07 林実装第1段階完了。web/src/components/TaskDetail.tsx から削除対象3コンポーネント（WorkflowSection, LinkedConversation, LinkedDeploys）をコメント化。関連の型・ヘルパー関数・unused import も削除（ProjectName, useMemo, useLiveTick, useLiveResource, AgentFeed, agentStatusMeta）。ブロッカー/依存セクションのスケルトン（コメント化）を追加。tsc/eslint/build green、サーバ起動・healthz 200 確認。commit e6fa533。【残課題】(1)server側でTask型に blockedBy/dependsOn フィールドを追加（dev-apollo ソラに委譲）(2)スケルトン実装をアンコメント→完成(3)実機でTaskDetail視認確認。狙い「このタスク今どうなってる？」一目で分かる。ソラが server 側拡張→林がコメント解除→実機検証→Keita 承認。） | 林（前半）→ dev-apollo（ソラ, server 拡張） |
| MC-168 | server/src/lib/types.ts Task 型拡張: blockedBy / dependsOn フィールド追加 | P1 | TODO | dev-apollo（ソラ） | MC-167 |
| MC-169 | server/src/collectors/tasks.ts 拡張: TASK_TRACKER「依存」列をパース→Task オブジェクトに割り当て | P1 | TODO | dev-apollo（ソラ） | MC-168 |
| MC-170 | web/src/components/TaskDetail.tsx ブロッカー/依存セクション実装: スケルトンをアンコメント＋表示確認 | P1 | TODO | 林 | MC-169 |
