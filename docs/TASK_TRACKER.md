...
| MC-391 | 接続ソース最大化（チェーン全網羅・メーカー系・独立系・Google Maps検討） | 高 | IN_PROGRESS | Son | Keita指示2026-08-22 00:59「もっとコインランドリー接続して。可能な限り全部。グーグルマップとかメーカー独立系サイトとか」。既存merge基盤(MC-390)に追加ソースを可能な限り接続。Google MapsはPlaces API(課金要・キャッシュ規約制約)の扱いを別途報告 |
