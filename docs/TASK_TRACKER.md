...
| MC-165 | MC-164 の作り込み：エージェントをドット絵キャラ＋吹き出しで擬人化（現状チープ・誰が誰か不明） | P1 | REVIEW（2026-06-07 実装完了。蓮+残り8体 Gemini V2 高品質ドット絵生成→PersonaCard 配線→build green→commit b3a594e）。push・restart は Masayoshi 検証ゲート（NO_PUSH mode）。 | dev-logic / 林実装完了 / push承認待ち |
...
| MC-192 | ターミナルタブの C1/C2 アカウントバッジを完全削除（プルダウンは MC-179 で削除済も固定バッジが残存。Keita「C1,C2のプルダウンいらない」） | P1 | IN_PROGRESS（2026-06-07 20:57 Keita 指示。Masayoshi 起票→dev-apollo 委譲。Terminal.tsx 830-836 の C1/C2 固定バッジ＋未使用の client 側 account 状態(accountLabels/setAccountLabel/isC2/status由来 account 代入)を除去。モデルセレクタ(別物)は残す。server /api/terminal/account は無変更。NO_PUSH/NO_RESTART＝build/commitまで、反映 Masayoshi） | dev-apollo（ソラ）/ Masayoshi 反映 | なし |
