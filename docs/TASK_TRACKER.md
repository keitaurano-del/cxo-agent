...
| MC-373 | 受託SEO記事工場の構築 | 高 | IN_PROGRESS（8/11 13:25 納品変換 to_delivery.py(md→docx/HTML)実装・検証済 575e251。パイプライン全工程がSon側完備→残はKeita操作(今夜依頼: CW登録+応募3件)とWP入稿(初受注後)） | Son | Keita指示8/11 00:22「1(SEO記事受託)の自動化を仕組化」。リポジトリ=~/projects/writing-factory・設計=DESIGN.md。P2で案件ウォッチ/WP入稿 |
