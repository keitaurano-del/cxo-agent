...
| 優先度 | P2 |
| ステータス | IN_PROGRESS（2026-06-09 Keita「まず一般的なコンサル雛形を用途別に作って」→Son: コンサル文書テンプレ24雛形/7カテゴリ(営業提案/立上計画/現状分析/要件定義/実行進捗管理/報告コミュニケーション/クロージング)を docs/templates/consulting/ に作成・用途別カタログREADME付き・push。各雛形=用途/想定読者/章立て/記入ガイド/推奨フォーマット+front matter。／2026-06-09 Keita提示の note「コンサルスライド完全マニュアル」(有料)をベースに横断ガイドを追加。取得経路=OpenClawブラウザ(ゲートウェイ再起動→admin昇格承認→snap chromium不可のためplaywright同梱chromium+noSandboxに切替)＋Keita正規セッションCookie移送(captchaは破らず)で本編取得。原則を一般手法として自分の言葉で再整理(丸写し/例示画像なし・出典URL明記)し docs/templates/consulting/08-スライド作成/コンサルスライド作成ガイド.md を作成(前編=原理原則/中編=グラフ計数表使い分け/後編=要素別型+各節チェックリスト)。README索引に08カテゴリ追記(24雛形+1横断ガイド/8カテゴリ)。／2026-06-09 Keita「記事のスライドを参考にカタログUI・用途から探す・テンプレを」→Son: 記事の型を13テンプレ/6カテゴリ(汎用レイアウト/グラフ図解/提案/分析/報告/計画)でシード化(data/slide-templates.json・各型にwhenToUse/layout/structure/tips/previewSVG)。実装はsubagent直ディスパッチ=GET /api/templates(:id)新設(server lib+router・mtimeキャッシュ)＋新ビュー/slide-templates(カテゴリタブ+用途検索+previewSVGカードグリッド+詳細モーダル+構成コピー)。Son実機検証=server tsc/web build green→一時インスタンスでAPI200/404・CDP直でブラウザ描画確認(13カード/6カテゴリ/検索)→commit 1d92b19 push→mission-control.service再起動(sudo -n)で本番反映、本番4317でAPI200/13件・healthz200確認。Phase1(カタログ+用途フィルタ)完了。／2026-06-09 Keita「型を作り込み＋AI推薦/pptx出力/記入支援を全部」→Son: 型を13→24拡充(ピラミッド/Agenda/散布図/ヒストグラム/円グラフ/敢えての表/ロードマップ/RACI/WBS/リスクマトリクス/提案3点 等)＋全型に placeholders 付与。実装はsubagent直=①AI推薦 POST /api/templates/recommend(用途文→最適型3件+理由,runClaude) ②pptx出力 POST /:id/pptx(pptxgenjs で空スライド3枚生成・DL/Deliverables保存) ③記入支援 POST /:id/draft(型+文脈→placeholder毎にAI下書き,runClaude)。UI=AI相談バー＋推薦ハイライト/詳細モーダルにpptx DL・保存・AIで下書き。Son実機検証=server tsc/web build green→一時instで3API(recommend200/pptx200 PK65KB/draft200)→CDPでカタログ24型+AI相談バー+モーダル4ボタン描画確認→commit 0c3ecf1 push→mission-control再起動(sudo -n)→本番4317で 24型/pptx PK/recommend200(as-is-to-be等的確) 確認。Phase2完了。／2026-06-09 19:34 Keita「全然使えないから全部削除して。スライド型のメニューもコードもファイルも」→Son: スライド型機能を全撤去（メニュー/slide-templates・ビュー・API recommend/pptx/draft・lib・data/slide-templates.json・型・コンサルスライド作成ガイド・pptxgenjs依存）。commit 60f5091 push→mission-control再起動→本番で /api/templates 消滅(SPAフォールバック)・dist参照ゼロ・build green 確認。**24雛形(01-07カテゴリ)は維持**。git履歴に残り復元可能。本タスクは CANCELLED 相当（スライド型は不採用）。MC-224本体のドキュメント雛形カタログ機能の要否は別途Keita判断） |
