...
| MC-156 | Apollo ターミナルビューを2/3/4分割で同時表示できるレイアウト切替を追加（現状はタブで1つずつ表示） | P1 | IN_PROGRESS（2026-06-07 Keita要望。dev-logic に委譲。実装は web/src/views/Terminal.tsx の作業ツリーのみ＝未コミット・未ビルド・未デプロイで実機未反映。残：commit→web build→本番デプロイ→実機検証。未コミット変更は git 操作で消失リスクあり要早期commit。スマホ対象外＝md以上幅のみ表示の現仕様でOK） | dev-logic |
