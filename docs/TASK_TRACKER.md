...
| MC-183 | Phase 1: RAG 診断・可視化（notebookRouter ログ追加、/api/notebooks/:id/status 新設、embedding エラー改善） | P0 | DONE（2026-06-07 22:35 林ティック完了。dev-apollo 実装→tsc/web build green→commit 6c3a9f。embedding.ts exponential backoff (1s→60s max)・notebookRouter ask/generate 詳細ログ([timestamp/chunkCount/elapsed])・/api/notebooks/:id/status endpoint 実装） | dev-apollo | なし |
