...
| MC-339 | ClipItNow ExoClick広告購入テスト（少額$10-20でトラフィック購入→採算検証） | 中 | IN_PROGRESS（2026-07-24 Keita「決済もしたよ」＝ExoClickへ直接入金・実質承認。22:00時点でAPI残高$0=反映待ち（Advertiser: Verified・campaigns 0件）。残高反映を確認→金額に合わせキャンペーン設計・入稿・計測はSon） | Son | 決裁フロー投入済→Keitaが直接入金で実質承認 |
