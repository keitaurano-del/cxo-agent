...
| MC-355 | Apolloデイリーニュース改善（読みやすいデザイン＋出典リンク末尾集約＋選択テキスト深掘りポップアップ） | 高 | IN_PROGRESS（2026-08-01 07:41 Keita依頼）。要望: ①見た目を分かりやすく親しみやすく（不自然なハイライト回避・内容量は不変）②出典元ニュースサイトのリンクを最後にまとめて掲載③文章をタップ/ドラッグ選択→ウェブ自動検索→「これはこういうことです」解説＋関連リンクをポップアップ表示。実装方針: ②=daily-news-briefing.shのプロンプトに末尾「出典リンク一覧」セクションを追加（翌日号から反映）／①=News.tsxのコールアウト/ハイライト調整・タイポグラフィ改善／③=News.tsxに選択ハンドラ＋ポップアップ、server新設 POST /api/news/deepdive（Gemini google_search grounding・GEMINI_API_KEY・geminiText.ts方式の堅牢フォールバック）。**【2026-08-01 08:00 Son実装・本番反映済】**①news.css新設（コールアウトを淡色カード化・行間1.85・出典リンクカード化）②generatorへ「📚 出典リンク一覧」末尾セクション追加（bash -n済・翌8/2号から反映）③newsRouter.ts＋深掘りUI（選択→🔍ピル→ポップアップ・モバイル=ボトムシート・Esc/背景で閉）。検証=server typecheck 0・unit 10/10・web build成功・dev一声→backend再起動→本番 /api/news/deepdive 実測200（日銀テキストで平易解説＋grounding関連リンク）/空文字400・本番News bundleに深掘りUI確認。残=8/2号で出典セクション見た目確認 | Son | backend再起動済（dev一声済）。共有ツリー注意 |
