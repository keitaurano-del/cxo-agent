...
| MC-149 | /api/chat/agent-message の JSON 不正構文エラー（エージェントが malformed JSON を送信） | 高 | bug | TODO（2026-06-05 apollo-keeper 起票。06:28 JST 林・21:03 JST ハル の2件で entity.parse.failed を確認。body に `"senderId:haru"` 等の不正 JSON（key:value がコロン区切りで正しく引用符で囲まれていない）が送信されている。エージェント側の agent-message 送信ロジックで JSON 構築に誤りあり。根本修正: agentMessageHandler の呼び出し側でプロパティ名と値を正しくシリアライズする） | dev-logic | MC-141 |
