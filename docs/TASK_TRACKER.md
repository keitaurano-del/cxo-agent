...
| MC-193 | エージェントアバター追加生成：masayoshi / task-manager / test-functional の3体ぶん V2 ドット絵（working/idle）を生成し配信 | P2 | TODO（2026-06-07 21:37 Son 起票。MC-165 で6体ぶんの V2 アニメGIF（working/idle）を web/public/avatars/ 配置・Agents.tsx 実描画・push/restart 反映済。残り masayoshi / task-manager / test-functional の3体は V2 未生成でフォールバック表示中。残作業=①3体ぶん V2 ドット絵 working/idle 生成②web/public/avatars/ へ MC-165 命名規約で配置③Agents.tsx の personaMap に3体を追加④build＋実機で3体もドット絵表示を確認。担当 designer（Gemini V2 生成）→ dev-apollo（配線）。push/restart は Masayoshi 検証ゲート） | designer（生成）→ dev-apollo（配線） | MC-165 |
