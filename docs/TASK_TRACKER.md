...
| MC-102 | ターミナル画像添付にプレビュー表示と複数枚ステージング UI を追加（MC-95 拡張） | 中 | feature / UX | IN_PROGRESS（dev-logic 着手中。MC-95 のサーバ側複数枚対応済を活かし、フロントを「即送信」→「ステージング」方式へ。サムネプレビュー＋個別削除＋5枚上限→まとめて送信） | dev-logic | MC-95、MC-92〜94、MC-100/101 |
