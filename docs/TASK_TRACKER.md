...
| 優先度 | P2 |
| ステータス | TODO（2026-06-20 Keita 依頼・Son 起票）。Keita「アポロの全メニューで、メニューを押した後のメニュー名やタブを最小化できるようにして。じゃないと見れる画面が小さくなる」。設計方針: 共通 `web/src/components/PageHeader.tsx`（タイトル＋説明、18ビュー共有）に最小化トグルを追加→畳むとタイトル＋説明を隠し本文を拡張。タブは各ビュー個別実装のため、最小化時はタブ行を細い1行に圧縮（ナビは維持）か共通化を検討。折りたたみ状態は localStorage で全メニュー・再読込後も保持。再展開ボタン常設。担当=ソラ(dev-apollo)直接委譲。注意: 起票時点でソラが Childcare.tsx/Development.tsx を未コミット編集中＝衝突回避のためソラ自身が共有ツリー状態を見て実装。 |
| 担当 | dev-apollo（ソラ） |
| 受け入れ条件（DoD） | 全メニューでヘッダー最小化トグルが効き、畳むとメニュー名＋説明が隠れて本文表示領域が広がる。タブはナビ可能なまま。状態は localStorage で全画面・再読込後も維持。build green＋実機（OpenClawブラウザ）検証。 |
| 依存 | なし |
| 更新日 | 2026-06-20 |
...
| 優先度 | P1 |
| ステータス | TODO（2026-06-20 Keita 依頼・Son 起票）。**狙い**: Keita 自身がコードを読めるようになること。開発＝モノ作りだけでなく「ルールで動いている仕組みを読めるようにしたい」。**言語=TypeScript で確定**（理由: ①Apollo本体がTSで自分の製品をそのまま読める ②型＝ルールの明文化で「どういうルールで動くか」を読む目的に直結。Python比較の上でKeita承認 2026-06-20）。**方針=A案**: 既存 /dev パイプライン（MC-251 モック→252 デザイン→253 実装仕様書）に乗せ、生成物に対し (a) TypeScript 実装コード (b) 構造化解説ビューを出す。解説は3点立て＝①エントリ（このコードはどこから始まるか）②各部の役割（ここで何をしているか）③ルール（型/規約=どういう約束で動くか）。コードと解説をセクション対応で読める形（どの関数がどこか分かる）。**解説の入り方は段階的**: 最初は流れ（手続き）として読ませ→慣れたら「この注釈＝型=ルール」と重ねる。難しい記法（ジェネリクス等）は後回し・専門語は都度かみ砕く。**HTMLはプレビュー用**、学習素材はTS実装＋日本語解説に分離。担当=ソラ(dev-apollo)直接委譲。 |
| 担当 | dev-apollo（ソラ） |
