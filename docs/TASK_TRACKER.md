...
| MC-461 | 集客の自動化（IndexNow送信・GSCはCowork試行） | 中 | IN_PROGRESS | Son | Keita 2026-08-23「Coworkで自動でやって」。①IndexNow(ログイン不要): public/<key>.txt設置＋api.indexnow.orgへトップ+厚い店舗500件通知(laundry-jp a5a5f5d)。初回403→数分後の再送でHTTP200成功(トップ+500店通知済)。scripts/indexnow.mjs再利用可。②Google Search Console: CoworkブラウザのGoogleアカウント(clipitnownet.official@gmail.com)が『Verify it's you』2FA要求で停止＝Keita端末必須(Cowork/Sonはパスワード/2FA自動化しない方針)。要Keita: Cowork画面でGoogle本人確認を通す→以降(プロパティ追加/HTMLタグ検証/sitemap送信)はSonがCowork/CDPで自動化。③SNS: 外部送信はKeita確認必須のため下書きのみ(自動投稿しない)・アカウント要指定 |
