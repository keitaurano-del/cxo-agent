...
| MC-171 | tasks.ts パーサ修正: 「着手順／ID／内容」等 ID 列が先頭でない/ステータス列を持たないプランニング表を非タスク表として除外し、幽霊カード（id=1〜5＝T-J/T-W/T-I/T-K/T-L、T-U の壊れ行）をボードから消す | P1 | IN_PROGRESS（2026-06-07 22:15 JST 林 cxo ティック。tasks.ts パーサに ID列位置チェック＋ステータス列チェック追加。id 列が先頭でない or ステータス列なしの表は除外。server tsc green。commit ff40136。★2026-06-07 18:55 Keita「進めて」＝restart GO。ソラがクリーンrestart→幽霊カード(id1〜5/T-U壊れ行)消滅を確認→commitまでで承認待ち。174と並行） | dev-apollo（ソラ） | なし |
...
| MC-173 | Apollo ターミナル: agent busy 時にメッセージをキューに積み、ターミナルが空いたら自動送信する | P1 | IN_PROGRESS（2026-06-07 22:15 JST 林 cxo ティック開始。dev-logic に委譲。★2026-06-07 18:55 Keita「進めて」＝171/174と並行で前進指示。蓮が実装継続） | dev-apollo（ソラ）/ dev-logic（蓮） | なし |
| MC-174 | ダッシュボード（/）の初期表示を「一番左のタブ」にする。現状 web/src/App.tsx:503 で常に `/today`（ブリーフィング＝最右タブ）へ Navigate しているのを左端タブ表示に変更 | P1 | REVIEW（2026-06-07 22:20 JST 林 cxo ティック。ソラに実装委譲。useNavOrder('dashboard')で保存済みタブ順先頭へ動的リダイレクト。mc-175と同梱実装・同1build。web tsc/build green、server tsc green、systemd restart OK、healthz→{"ok":true}。commit 379fc56。push 承認リクエスト req-3414bcbe 投げ済み）| dev-apollo（ソラ） | なし |
| MC-175 | ダッシュボード左ナビのラベル変更：「フォルダ」→「ドキュメント」、「ノートブック」→「RAG」 | P2 | REVIEW（2026-06-07 22:20 JST 林 cxo ティック。ソラに実装委譲。MC-174と同梱・同1 build/restart。App.tsx NAV def: 'フォルダ'→'ドキュメント'(shortLabel:'ドキュ')、'ノートブック'→'RAG'。tsc/build green、healthz OK。commit 379fc56。push 承認リクエスト req-3414bcbe） | dev-apollo（ソラ） | MC-174 と同梱 |
