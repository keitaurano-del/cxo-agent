...
| MC-118 | 成果物にファイルアップロード機能を追加（大容量対応＝ディスクへストリーム保存） | 高 | feature | TODO（2026-06-03 Keita 依頼） | dev-logic | MC-116/117・multer diskStorage |
| MC-119 | Apollo ターミナルを3つに分割（同一窓口で別枠の独立セッション。ターミナル1=Logic/2=円茶会/3=仕事 等で使い分け） | 高 | feature | TODO（2026-06-03 Keita 依頼） | dev-logic+林(ttyd/infra) | MC-92/104・ttyd・tmux |
