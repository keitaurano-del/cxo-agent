...
| 優先度 | P2 |
| ステータス | IN_PROGRESS（2026-06-09 Keita「まず一般的なコンサル雛形を用途別に作って」→Son: コンサル文書テンプレ24雛形/7カテゴリ(営業提案/立上計画/現状分析/要件定義/実行進捗管理/報告コミュニケーション/クロージング)を docs/templates/consulting/ に作成・用途別カタログREADME付き・push。各雛形=用途/想定読者/章立て/記入ガイド/推奨フォーマット+front matter。次=既存標準様式の資産化＋カタログUI/推薦(設計書§の本体)はKeita方針次第） |
