...
| MC-58 | Apollo Vault ノート編集機能（obsidian-git 同期競合対策込み） | P2 | TODO（2026-06-07 Keita 承認フローで解禁承認 req-3d6c5483＝ブロック解除。dev-apollo(ソラ)に実装委譲。要件：Apollo の Vault ビューでノート編集→保存。obsidian-git 同期競合対策必須＝保存前に最新を読み直し・競合時は上書きせず検知して退避/マージ導線・破壊的 git 禁止。実装→tsc/eslint/build→実機検証→REVIEW。本番反映は承認フロー） | dev-apollo（ソラ） | MC-57 |
...
| タイトル | tasks collector のステータス誤表示（表行DONEがREVIEWに巻き戻る）＋縦型カード非対応を修正 |
| 優先度 | P1 |
| ステータス | DONE（2026-06-01 cxo 自律ティック（林）で検証確定。実装は commit `e154e00`＝(A)(B) worker プロンプト＋物理ガード（`~/cron-scripts/autonomous-worker.sh`：他タスクの BLOCKED 行を ID 非依存で HEAD から復元する決定的 python ガード L215-293／`SELECTED_TASK_ID` キャプチャ L212／鉄則 L163・L168）＝リポ外ローカルスクリプトで即時有効（本ティック自身がこの強化版プロンプト下で走行＝live 反映を実証）。(C/D) collector status 正本一本化は `server/src/collectors/tasks.ts` の `inNonTaskTable`（非タスク表を行ごと除外）で実装。**当ティックで実ファイル＋git 裏取りし独立に再検証 green**: `git log` に e154e00 在・ガード実コード実在・`tsc --noEmit` EXIT0・`tasks.summaryTable` 3/3・`approvals.decision` 9/9・`tasks.normStatus` 回帰 31/31。DoD（原因特定＝a58c147 の reconcile 上書き／修正＝ガード＋正本一本化／再現確認＝ガード単体検証①〜④＋本ティック green）充足。[[feedback-review-agent-verify-then-done]] によりエージェント検証で DONE 化（Keita 実機確認不要）。**collector のライブ反映には cxo-agent server（mission-control.service）restart が必要＝Keita 承認待ち（restart まで Apollo live は未変化）。push も Keita 承認領域（NO_PUSH）。** MC-89／MC-88-MC-89 共通行と同根・同 commit で対処済み。**2026-06-01 根治強化（dev-logic 蓮）: 物理ガードの保護対象を BLOCKED 単独から BLOCKED/REVIEW/CANCELLED の3種に拡張し、SELECTED_TASK_ID 自身の行は除外（自タスクの正当な前進は許容、他タスクの降格のみ HEAD 復元）。根因の決定打＝a58c147 が AF-06 着手中に summary table を丸ごと再生成し DF-F4/AM-N/T-U(BLOCKED)・REVIEW・CANCELLED を bare TODO に降格していた実 diff を確認。REVIEW/CANCELLED も TODO 降格すると未着手偽装/幽霊再浮上で実害のため保護に追加。`~/cron-scripts/autonomous-worker.sh` の guard_blocked_rows python を更新（PROTECTED=(BLOCKED,REVIEW,CANCELLED)・selected 除外）、bash -n/ast.parse green、a58c147 再現シナリオで DF-F4/AM-N/T-U→BLOCKED・DF-F12→REVIEW・MC-97→CANCELLED 復元かつ AF-06(selected)/AF-07(DONE) は非介入を単体検証。collector 側は adversarial 4ケース（table BLOCKED+section TODO 非降格／REVIEW→DONE 前進可／非タスク表の旧新状態を拾わない／CANCELLED 注記内 TODO 無視）green、normStatus 31/31・summaryTable 3/3・approvals.decision 9/9 非退行。mission-control.service restart 済（17:42）・healthz 200・/api/tasks で DF-F3/DF-F13/T-U=BLOCKED 確認＝Apollo 盤面正。push は Keita 承認領域。) |
| 担当 | task-manager（棚卸し調整）+ test-functional（内部検証）+ reviewer（品質判定） |
...
| MC-156 | Apollo ターミナルビューを2/3/4分割で同時表示できるレイアウト切替を追加（現状はタブで1つずつ表示） | P0 | REVIEW（2026-06-07 回帰バグ修正完了。焦点：overlay の z-index を `-z-10`（iframe 後ろ）に変更し pointer-events-auto でのスクロール遮断を解消＋onWheel handler で wheel event を親に propagate させる。Terminal.tsx:1216-1235 修正コミット f00ba50(予定)。修正前後で分割2/3/4レイアウト全パターン実機検証（スクロール・入力・ペイン間移動）が必須。tsc/eslint/web build green。本番反映は承認フロー） | dev-logic |
| MC-157 | 旧箱（Vultr `dev@139.180.202.62` hostname claude-code＝ターミナル2/OpenClaw Satoshi💡）解約に伴う decommission | P1 | IN_PROGRESS（2026-06-07 Keita解約完了・旧箱SSHタイムアウトで確認。Satoshi workspace は /home/dev/projects/_archive/satoshi-oldbox-20260607/ に退避済。✅後始末実施：systemd 無効化 apollo-reverse-tunnel/apollo-terminal-2（inactive+disabled・failed解消）、crontab 旧箱2行除去（sync-satoshi-knowledge・auto-compact-watch、22→20行・他行温存、backup /home/dev/backups/crontab.before-MC157-20260607.bak）。Apollo本体/ターミナル1・3・4 健全:4317=200。残：web/src/views/Terminal.tsx をT1/T3/T4の3枚へ=dev-logic、MC-156と同波。ターミナル名 Main(T1)/Aux(T3)/Ops(T4) 化＝.mc.env の TERMINAL_1/3/4_LABEL を設定済だが【未反映】。原因：web/src/views/Terminal.tsx の TERMINAL_TABS が label をハードコード（'ターミナル1'…）で、env はサーバ config.ts と /terminal status-all API（label 返却済）にしか効かずフロントが無視。要修正：TERMINAL_TABS の label をサーバ status-all API の terminals[].label から取る（env 駆動化）か、最低限 Terminal.tsx の直書きを Main/Aux/Ops に変更。3枚化と同じビルド＋デプロイで反映。env 値は維持。【担当変更 2026-06-07 Keita：レン→ソラ(dev-apollo)】。【追記 2026-06-07：旧箱SSH残骸あり】server/src/config.ts:302-314 の TERMINALS id=2（apollo-terminal-2.service / remote sshHost 139.180.202.62）が残存し、サーバが旧箱へ probe SSH を試行し続けている（バックグラウンドで Connection timed out）。3枚化の際に config.ts の TERMINALS から id=2 を除去し web の T2 タブも落とす（server+web 同時でないと T2 タブが壊れる）。これで残骸 SSH も消える。） | dev-logic（暫定実行・本来ソラ） |
...
| MC-159 | チャット未読バッジ（App.tsx chat.unreadBadge）が 99+ まで膨らむ。Keita 宛て/人間発言だけ数え、イベントルーター自動投稿・エージェント間チャットは加算しないよう加算条件を絞る | P1 | TODO（2026-06-07 Keita指摘「99とか何で」。現状 App.tsx:361 が全 chat SSE を一律+1し dev チャンネル190件等の自動投稿で膨張、リセットはチャット画面を開いた時のみ。要件：senderId が router/各エージェント等の自動発信を除外、Keita メンションや人間発の通知だけカウント。実装は dev-logic、tsc/eslint/build→restart→実機で 0 復帰と非膨張を検証。【範囲 2026-06-07 Keita追記】①チャット未読バッジ（/chat の chat.unreadBadge）の自動チャッター加算を止める。②タスクボードの通知も不要＝NAV_BADGE_MAP の `tasks:'/tasks'` 行と UPDATE_TOAST_META の `tasks`（'タスクボードが更新されました' トースト）を除去し、/tasks バッジ＆トーストを出さない。残す：承認 /approvals の approvalCount、vault・deliverables の badge.{path}・トースト、ダッシュ。実装 dev-logic、Web ビルド→deploy→実機検証。Terminal.tsx 系の web デプロイ波に同梱可。【担当変更 2026-06-07 Keita：レン→ソラ(dev-apollo)】） | dev-logic（暫定実行・本来ソラ） |
...
| MC-162 | ナビでダッシュボードと他ページ（ターミナル/フォルダ/ノートブック/チャット）が同時にアクティブ点灯する | P2 | TODO（2026-06-07 Keita指摘「ダッシュとターミナルが一緒に選択されて気持ち悪い」。原因：web/src/lib/nav.ts の isDashboardPath が除外リスト方式で NON_DASHBOARD_PREFIXES=['/tasks','/approvals','/vault'] 以外を全部ダッシュboard扱い→true。後から追加の /terminal-view /deliverables /notebooks /chat がリスト未追従で、ダッシュボードナビが forceActive で co-点灯（App.tsx:225 forceActive = item.to==='/' && dashActive）。修正：除外リストに /terminal-view /deliverables /notebooks /chat を追加、または許可リスト方式（実ダッシュ子: /today /feed /agents /activity /news /plan-usage 等）に反転して将来の追加に強くする。web build→実機で各ページ単独点灯を確認。dev-apollo(ソラ)領分。【全メニュー確認済 2026-06-07 Masayoshi】二重点灯する項目＝フォルダ/ノートブック/チャット/ターミナルの4つ。発生面はサイドメニュー(App.tsx:243)とモバイルBottomNav(BottomNav.tsx:37)の両方＝共に isDashboardPath+forceActive 使用。正常はタスク/承認/Vault（除外リスト内）。ダッシュサブタブ(DashboardLayout DASH_TABS)は別ロジックで問題なし。根本は isDashboardPath 1か所なので修正1つでサイド・ボトム両面＋4項目を一括解消。実機検証は PC幅サイド と スマホ幅ボトム の両方で） | dev-logic（暫定実行・本来ソラ） |
...
| MC-164 | 【最優先】エージェントの稼働をタスクボード上でリアルタイム可視化（擬人化・誰が今何をしているか） | P0 | TODO（2026-06-07 Keita最優先「ターミナルが止まっていても他エージェントが動いているか可視化したい。擬人化してタスクボード上で何をしているかリアルタイムに把握したい。アイドルだと本当に実装されてるか心配」。土台は既存＝/agents ビュー(Agents.tsx)＋/api/agents/grouped(人格別 status/活動/現在タスク)＋/api/agents/:id/feed(会話タイムライン)＋useLiveResource(SSE駆動でリアルタイム)。ギャップ＝この情報がタスクボードに統合されておらず常設で見えない。要件：①タスクボードの各タスク行/カードに、今そのタスクで稼働中のエージェント(擬人化アイコン/名前)と最新アクションを表示②ボードに常設の「エージェント稼働ストリップ」（各人格の稼働中/待機＋現在タスク）を出す③SSE でリアルタイム更新（claude プロセス稼働・dev チャット投稿・自律ティックを反映）。MC-163(タスク別履歴)と連携。実装→tsc/eslint/build→実機検証→REVIEW、本番反映は承認フロー。dev-apollo(ソラ)領分だが最優先のため実働エージェント(autonomous-cxo/レン)で着手可） | dev-apollo（ソラ）/実働は最速の手 |
