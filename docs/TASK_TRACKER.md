...
| MC-149 | /api/chat/agent-message の JSON 不正構文エラー（エージェントが malformed JSON を送信） | 高 | bug | IN_PROGRESS（2026-06-05 林着手。根本原因確定: kpi-report.sh L90 で `\"senderId":"haru\"` `\"senderName":"ハル\"` の `\` が欠落、シェル文字列の境界がずれて `senderId:haru` がキーになる malformed JSON が生成される。修正: L90 の senderId/senderName を `\"senderId\":\"haru\"` `\"senderName\":\"ハル\"` に修正＋全 agent md ファイルの例示 curl コマンドを python3 全フィールド JSON 構築方式に更新。dev-logic 委譲中） | dev-logic | MC-141 |
