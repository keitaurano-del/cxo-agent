...
| MC-393 | Google Maps連携＋料金Web採掘＋口コミ評価表示 | 高 | IN_PROGRESS | Son | Keita指示2026-08-22 08:46「グーグルマップでコインランドリーも調べて。料金はネット/GMaps画像/口コミから取得できるはず。口コミ評価も表示して」。正規ルート=Places API(New)。GeminiキーのGCPプロジェクト(874265526203)は生きておりPlaces API有効化のみ必要(Keitaワンタップ待ち)。並行してWeb料金採掘・評価表示UI・Places統合コード(envフラグ)を実装 |
