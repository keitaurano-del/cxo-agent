...
| MC-147 | chat-autonomous.sh のエンドポイントパス修正（/api/autonomous-tick → /api/chat/autonomous-tick） | 高 | bug | TODO（2026-06-04 apollo-keeper 起票。chatRouter は /api/chat に mount されており endpoint は /api/chat/autonomous-tick だが、cron スクリプトは /api/autonomous-tick を呼んでいる。MC-146 の AGENT_TOKEN 修正後も cron は 404 で失敗し続ける。server restart 後に初めて顕在化する。fix: chat-autonomous.sh の curl URL を /api/chat/autonomous-tick に修正） | dev-logic | MC-145/MC-146 |
