...
| MC-333 | Apollo端末でURLリンクを開けるようにする（クリック/タップでURL検出→開くトースト） | 高 | IN_PROGRESS（2026-07-22 Keita「ターミナルのリンクが機能してない」。原因=ttyd iframe＋TAP_FIXがタップをtmuxへ流すためxterm link層に届かない。方針=terminalProxy注入のLINK_FIXでクリック/タップ位置のバッファからURL抽出→「開く」トースト表示→window.open。誤爆防止で即開かずトースト経由） | Son | server変更＝backend再起動要（Keitaへ一声） |
