...
| MC-288 | 💡「アイデアを生成」をバックグラウンドジョブ化（ページ離脱→再訪で結果が消えない） | 中 | REVIEW（2026-07-06 dev-apollo実装＋林がrestart後API実機検証。POST /api/dev/idea即jobId返し→10秒ポーリング停止[離脱模擬]→復帰でgenerating→done、idea取得成功＝裏で生成継続を実証。healthz200・新ルート/api/dev/idea/job/:idはJSON登録確認[stale罠なし]。永続化なし=再起動窓に当たった時のみ消え静かに通知。クライアントUI[離脱→復帰でlocalStorage'dev-idea-job-v1'復元→要望欄反映]はtest-functionalでブラウザE2E確認中） | dev-apollo(ソラ) | restart反映済(Keita承認) |
