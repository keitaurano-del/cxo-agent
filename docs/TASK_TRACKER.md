...
| MC-171 | tasks.ts パーサ修正: 「着手順／ID／内容」等 ID 列が先頭でない/ステータス列を持たないプランニング表を非タスク表として除外し、幽霊カード（id=1〜5＝T-J/T-W/T-I/T-K/T-L、T-U の壊れ行）をボードから消す | P1 | IN_PROGRESS（2026-06-07 22:15 JST 林 cxo ティック。tasks.ts パーサに ID列位置チェック＋ステータス列チェック追加。id 列が先頭でない or ステータス列なしの表は除外。server tsc green。commit ff40136。restart 承認待ち） | dev-apollo（ソラ） | なし |
...
| MC-173 | Apollo ターミナル: agent busy 時にメッセージをキューに積み、ターミナルが空いたら自動送信する | P1 | IN_PROGRESS（2026-06-07 22:15 JST 林 cxo ティック開始。dev-logic に委譲） | dev-apollo（ソラ）/ dev-logic（蓮） | なし |
