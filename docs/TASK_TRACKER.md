...
| MC-58 | Apollo Vault ノート編集機能（obsidian-git 同期競合対策込み） | P2 | REVIEW（2026-06-07 林着手→dev-logic 実装完了。Vault.tsx edit UI + server POST /api/vault/notes/:id/save + vaultWrite.ts git 競合対策（pull --rebase --autostash + conflict 検出→abort + .conflict 退避、reset --hard/clean -f 禁止）。tsc/eslint/build green。healthz 200 確認。実機 web 編集→保存テスト待ち。commit cdc8bbb） | hayashi-rin（林） | MC-57 |
