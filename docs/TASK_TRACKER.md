...
| MC-269 | Google AdSense: アカウント作成→サイト審査通過→ads.txt→実広告タグ差込（現状 Aurora Cloud モック置換） | 高 | **審査待ち（BLOCKED=Google審査 / 2026-07-05）**。完了: 開設(pub=`ca-pub-6825072541659667`)・**確認スニペットを web/site/index.html `<head>` 掲出**・**`/ads.txt`(google.com, pub-6825072541659667, DIRECT, f08c47fec0942fa0)をルート配信**(server/src: 配信ループ＋publicSite.ts許可リストに ads.txt 追加・再起動反映)・**Keita が審査リクエスト済**。実測: `https://pdfdotai.com/ads.txt`=200・トップにスニペット掲出・保護境界回帰なし。残: **①Google審査(数日〜・人手・待つのみ) → ②承認後 Son が Auto ads/実広告ユニットでモック(Aurora Cloud)差し替え＋必要ならEEA向けCMP(Googleプライバシーとメッセージング)結線** | Google(審査)→Son(点火) | Keita作業は一旦なし |
