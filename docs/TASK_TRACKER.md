...
| 優先度 | P1 |
| ステータス | BLOCKED（Keita 判断待ち：カラー案A/B・タイポ・アイコン方針） |
...
| ロールアウト | フェーズ0=CSS変数土台(tsx非依存・並行衝突なし) → フェーズ1=Overview 1画面サンプル→Keita承認 → フェーズ2=全7view横展開 → フェーズ3=アイコンのドット絵化＋任意の9体ドット顔キャラ |
