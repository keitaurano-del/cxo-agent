...
| MC-176 | タスクボード(/tasks)でカードをドラッグ&ドロップしてステータス変更できるようにする（列間ドロップで TODO/IN_PROGRESS/BLOCKED/REVIEW/DONE/CANCELLED を更新） | P1 | REVIEW（2026-06-07 20:20 林無人ティック。web/src/views/Tasks.tsx を @dnd-kit（SortableContext）で全面改造。(1)TaskCard/Column に useSortable で draggable/droppable 化 (2)PointerSensor/TouchSensor/KeyboardSensor で multimode 対応 (3)handleDragEnd で POST /api/tasks/status-lock 呼び出し→台帳正本 .md 更新 (4)楽観更新+失敗時 revert (5)DragOverlay で card image 表示。既存サーバ /api/tasks/status-lock 流用。web tsc green・build success・server tsc green・healthz 200・API /api/tasks 正常応答。commit 3本(6bcffa7/fa35059/e9a1f34)。テスト待機: test-functional で実機 D&D・モバイル検証→DONE 化。push/deploy 承認待ち: req-2de67388-856d-47af-a5d4-2af1e45d0f40） | test-functional（試野）+林実装 | なし |
