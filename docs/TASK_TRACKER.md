...
| 依存 | 新規。NAV/Routes/childcareData の日付ユーティリティ・既存アップロード基盤流用。Phase2/3 は Google Cloud OAuth(client id/secret)・Calendar API有効化・(Picker採用時)Photos Picker API・keita.urano2 同意・data/google-tokens.jsonl。 |
| 備考 | **Keita 判断待ち3点**: (1)写真/動画の取り込み方式 A=Picker手動 / B=アプリ直アップロード / C=両方（Google Photos完全自動は規約上不可）。(2)Googleカレンダー連携の向き=育児ToDoを書き出し / 既存予定を読み込み表示 / 双方向。(3)Google Cloud のOAuthクレデンシャル＋API有効化を用意可否（Sonが手順書提供）。Phase1(Google非依存=タブ/カレンダー/ToDo/日記/直アップロード写真/成長グラフ)は判断不要で先行着手可。 |
| 更新日 | 2026-06-14 |
