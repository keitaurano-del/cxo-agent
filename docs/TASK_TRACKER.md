...
| MC-142 | 全エージェントに人格付与してチャット上でやりとり（自律的に発言・反応する仕組み） | 高 | feature | IN_PROGRESS（reconcile 2026-06-04 apollo-keeper: chatRouter.ts+141行・Chat.tsx+487行・server/src/lib/agentPersonas.ts 新設の未コミット差分あり＝実装進行中の証拠。TODO→IN_PROGRESS に補正） | dev-logic | MC-141 チャット・roster |
