...
| MC-313 | Apollo UI 改善 | 中 | IN_PROGRESS（次=タブ共通化 TabStrip/CountBadge・最後の🟢。詳細→ tasks/MC-313.md） | Son | [[son-owns-board-reconciliation]] | [[clipitnow-exoclick-integration]] [[son-owns-board-reconciliation]] [[cxo-agent-shared-tree-concurrency]] [[cxo-agent-prod-restart-loads-worktree]] |
...
| MC-339 | ClipItNow 広告テスト | 中 | CANCELLED（2026-08-01 決裁で広告テスト中止確定・再決済せず。入金は最後まで未反映=決済未完。経緯→ tasks/MC-339.md） | Son | 決裁フロー投入済→Keitaが直接入金で実質承認 |
...
| MC-357 | Keita操作キュー常設 | 高 | IN_PROGRESS（keita-actions.md運用開始済。残=ApolloボードのKeitaキュー表示カード=MC-358 P4。詳細→ tasks/MC-357.md） | Son | なし |
| MC-358 | タスクボード再構築 | 高 | IN_PROGRESS（P1設計✅・P2個票切り出し✅・個票タップ閲覧✅ 2026-08-02。次=P3 DONEアーカイブ＋board-audit手順更新。詳細→ tasks/MC-358.md） | Son | MC-357 |
...
| MC-360 | アイデア生成の多様性・面白さ向上（2026-08-02 Keita「どれも大体同じようなものばかりでつまらない。これいいね！これあったらいいね！と言われるサービスに」）。根因=シード12個が全部遊休資産マッチング同型＋出力型が固い。対策=多軸ランダム合成（業種×レバレッジ×サービス形態×顧客）＋wow基準＋直近アイデアの反復回避 | 高 | REVIEW（2026-08-02 実装・実機検証済。IDEA_SEEDS廃止→4軸ランダム合成(舞台15×レバレッジ12×形12×喜ぶ人12≒2.6万通り)＋wow基準(飲み会いいね自己チェック/心が動く要素/定番ネタ一捻り)＋直近8件リングバッファで反復回避。検証=tsc green・restart後healthz 200・/api/dev/idea 5連続生成 全done(2/8/11/12/10s、全て30s以内)・業種/形態/構造すべて別(造園記念樹/畳職人SNSカード/訪問介護ペット見守り/建具の家歴史レポート/習い事×移住体験)・遊休マッチング0件・市場規模文5/5） | dev-apollo | devMockupRouter.ts。MC-359の続き |
