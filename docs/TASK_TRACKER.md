...
| MC-288 | 💡「アイデアを生成」をバックグラウンドジョブ化（ページ離脱→再訪で結果が消えない） | 中 | IN_PROGRESS（2026-07-06 dev-apollo実装中。現状=同期await fetchで離脱すると中断→通知なく空。✨mockup生成のjobs機構[fire-and-forget＋jobIdポーリング＋localStorage/再訪復元]に載せ替え。restart反映はKeita承認待ち） | dev-apollo(ソラ) | MC-285系ジョブ機構流用 / restart=本番反映はKeita承認 |
