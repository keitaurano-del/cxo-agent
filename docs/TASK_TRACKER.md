...
| MC-165 | MC-164 の作り込み：エージェントをドット絵キャラ＋吹き出しで擬人化（現状チープ・誰が誰か不明） | P1 | IN_PROGRESS（2026-06-07 林 workflow 実行完了。①Phase1 design sample 完成: artifact/avatars/avatar-ren-{working,idle}.{png,svg}（64×64px、透明背景、色パレット定義済）→Keita design approval 待ち。②skeleton code 完成: personaMap.ts（avatar?プロパティ+9体stub）＋PersonaCard.tsx 新規（64×64 avatar + name label）→tsc/eslint/build green。次: design image URL 確定→hardcoded stub 置き換え→final test→approval。deadline 2026-06-12。） | 林 workflow / Keita design approval 待ち |
