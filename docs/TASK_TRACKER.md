...
| MC-203 | Keita への確認/判断を承認フローに統合（選択肢付き決裁＋結果をターミナルへ＋専用タブ＋専用オートモードトグル） | P1 | IN_PROGRESS（★2026-06-08 07:14 Masayoshi: dev-apollo（ソラ）へ実装委譲・着手。4機能は同一ファイル群(approvalRouter/approvalRequestStore/autoModeStore/Approvals.tsx)を触るため単一実装エージェントで実施。NO_PUSH・型チェック/build green を自己検証後 Masayoshi が実機検証→push＋restart（ゲート）。Son 駆動・追跡。 ／★2026-06-08 07:09 Keita 要望→Son 起票。【目的】エージェントから Keita への確認/判断依頼はターミナルだと埋もれる→承認フローに集約。【要件】①エージェントが『Keita 決裁依頼』を承認フローに投入できる（単純承認だけでなく**選択肢を選ぶ形**にも対応＝複数選択肢を提示し1つ選ばせる） ②承認UIに**専用タブ**（既存の exec承認/confirm/blocked とは別枠）でこれら決裁を表示 ③Keita が選択肢から選んで決裁→**結果を要求元ターミナル/エージェントに流す**（承認と同様、notify-agent.sh 連携で当該エージェントへ） ④**専用のオートモードトグル**（既存の承認オートモードとは別タブ・別トグル）。【拡張対象】server: approvalRouter/approvalRequestStore/autoModeStore に decision種別＋options＋別autoMode、approval-watch/realtime で結果配信。web: Approvals.tsx に専用タブ＋選択肢UI＋別トグル。MC-201(承認履歴/オート)・MC-200(通知経路)の延長。dev-apollo の承認コード領域につき Masayoshi/ソラへ委譲、Son spec/駆動。NO_PUSH。 | dev-apollo（ソラ）/ Masayoshi 検証・Son 駆動 | なし |
