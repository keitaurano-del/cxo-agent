...
| MC-202 | RAG機能の改善: 回答品質＋議事録を外す＋生成物の用途明確化/不要削除（フロント＋バック） | P1 | IN_PROGRESS（★2026-06-08 07:00 Son 調査完了→叩き台を Masayoshi・林に共有・討議へ。【現状調査】(構造) notebook=資料(sources)＋Q&A(ask: RAG検索→Claude stream/SSE)＋生成物(generate: 要約/FAQ/時系列/テンプレ/テンプレ抽出/カスタムの6種)。議事録は notebooks 内機能('議事録'フォルダ＋onOpenMinutes＋minutes系import)として統合。(発見) **全ノートブック3つ(Documentation/プロジェクト管理/PMBOK)が sources:0＝資料ゼロ**→RAGに根拠が無く回答不達/無根拠の主因候補。【叩き台(3者討議用)】①回答品質: 空NBでの ask 挙動確認＋『資料なし』明示UI／実資料での回答到達検証／RAG0件時のフォールバック・エラー表示改善。②議事録を外す: notebooks から minutes機能(onOpenMinutes/'議事録'フォルダ/minutes import/UI)を分離・削除。backend minutesRouter を別機能として残すか要判断。③生成物: 6種に各1行の用途説明を付与 or 使われない種別(テンプレ抽出等)を削除して絞る。→ Masayoshi・林へ notify-agent.sh で共有済、方針確定後フロント/バックに分割実装。 ／★2026-06-08 06:50 Son 整理・起票: 2026-06-07 21:31 Apollo投入。inbox自動採番が MC-200(連絡体制)と衝突→MC-202 へ振替（詳細セクション同番に修正）。Keita 要望=①RAGの回答がちゃんと返ってこない→回答品質改善 ②RAGに議事録はいらない→議事録を外す ③生成物が各々何用途か不明→用途明確化 or 不要なら削除。フロント・バック両方。MC-182系(RAG実装)後の品質改善フェーズ。『Masayoshi・Son・林で議論しながら進めて』＝方針は3者討議で確定→分割実装。次アクション=Son が現状(回答不達の再現・議事録/生成物の扱い)を調査し叩き台を出す。 | 未定（Masayoshi/Son/林 討議）/ Son 駆動 | なし |
