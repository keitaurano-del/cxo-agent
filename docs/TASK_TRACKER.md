...
| MC-176 | タスクボード(/tasks)でカードをドラッグ&ドロップしてステータス変更できるようにする（列間ドロップで TODO/IN_PROGRESS/BLOCKED/REVIEW/DONE/CANCELLED を更新） | P1 | REVIEW（2026-06-07 19:20 ソラ実装→林push&deploy承認 req-2de67388→Masayoshi検証: @dnd-kit実装実在(Tasks.tsx onDragEnd→POST /api/tasks/status-lock→台帳書き戻し+commit、server updateTaskStatusWithLock でロック対応)。コードは本物。origin/main へ push 済み。ただし承認本文どおり実機D&Dテストは未実施＝REVIEW据え置き。deploy-production.yml は cxo-agent に存在せずApolloはrestartベースのため deploy 手順はスキップ。次=test-functional/Keita 実機D&D確認でDONE） | test-functional（試野）+林実装 | なし |
...
| MC-178 | ダッシュボードに文字サイズ変更機能を追加 | P1 | TODO（2026-06-07 19:18 Keita 依頼・Masayoshi 起票。小/中/大の3段＋永続化が無難。★2026-06-07 20:15 Keita 回答=「ターミナル以外」＝ダッシュボード全体に適用（ターミナル限定でない）。スコープ確定→dev-apollo 着手可） | dev-apollo（ソラ） | なし（スコープ確定） |
...
| MC-181 | Masayoshi と同等の OpenClaw エージェントをもう1体追加（秘書レイヤの冗長化＝Masayoshi が詰まっても依頼できる体制） | P1 | TODO（2026-06-07 19:18 Keita 依頼・Masayoshi 起票。OpenClaw 設定タスクのため Masayoshi 主導(project_openclaw_secretary_masayoshi 参照)。★2026-06-07 20:15 Keita 回答=名前『Son』/役割=Masayoshi の補佐（同権限・Masayoshi が忙しい時にタスク起票や諸々サポートを代行）/Apollo ターミナルを『Sub』という名で1タブ増設。仕様確定→Masayoshi 着手可。実装メモ=config.ts TERMINALS に id5『Sub』追加・ttyd 別ポート/別 systemd・proxy ルート・openclaw を Son 人格で常駐） | Masayoshi | なし（仕様確定） |
...
| MC-186 | Apollo 投入（inbox）の即時ボード登録機能（遅延削減） | P1 | TODO（2026-06-07 inbox より新規起票） | task-manager / dev-logic | なし |
...
| MC-189 | Apollo 投入画面：優先度設定フィールドを追加 | P1 | TODO（2026-06-07 inbox より新規起票） | designer / dev-logic | なし |
| MC-190 | 承認フロー『オートモード』トグル（ON時はエージェント承認リクエストを自動承認。安全ゲート＝deploy カテゴリは除外し従来通り手動承認） | P0 | REVIEW（2026-06-07 20:15 Keita『最優先で実装』→Masayoshi 起票・dev-apollo 実装。実装完了＋Masayoshi コードレビュー検証。当初 MC-186 で作業したが inbox 自動起票が MC-186〜189 を並行取得し番号衝突＝正番 MC-190 に採番（コード commit 8e304ad のメッセージは作業中番号 MC-186 表記のまま）。変更=autoModeStore.ts新規/config.ts APPROVAL_AUTOMODE_FILE/approvalRouter.ts GET・POST /api/approvals/automode/approvalRequestHandler.ts 自動承認フック/types.ts/Approvals.tsx トグルUI。検証=server tsc exit0・web build green・deployカーブアウト実在(category!=='deploy'のみapproved)・ルート順序OK。commit 8e304ad ローカルのみ。★残=push＋本番restart は Keita 検証ゲート（Apolloはrestartで反映）→GoでMasayoshi反映。設計確認1点=deploy除外でよいか） | dev-apollo（ソラ）実装 / Masayoshi 検証・反映 | なし |
