...
| 優先度 | P1 |
| ステータス | IN_PROGRESS（2026-06-09 Keita 方針確定=「Q&A専用にして良くする」。生成物(要約/FAQ/時系列/雛形)はRAG不適のため撤去。Phase1実装着手=QA専用化＋検索品質UP(スコア閾値・ハイブリッドBM25+ベクトル・該当なしは正直に返す・使用ソース提示)。外部API無し(Gemini+自前リランク)・ベクトルストアはPhase3。Son subagent実装中） |
