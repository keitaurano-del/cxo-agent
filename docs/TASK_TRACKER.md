...
| MC-335 | ClipItNow 中国語版（zh-TW/zh-CN）LP＋主要ガイド展開 | 高 | IN_PROGRESS（2026-07-24 Keita「できることは全部やろう」。人間アクセスの言語最多=zhなのに受け皿なし。MC-332のEN化の型（/en/→/zh/）を流用しzhメタ/LP/主要ガイド＋hreflang） | Son | V2EX投稿(MC-331)と連動 |
...
| MC-337 | ClipItNow アカウント不要ディレクトリへの登録実施 | 中 | IN_PROGRESS（2026-07-24 Keita承認。フォーム送信のみで済むツールディレクトリを洗い出し→登録実施。要アカウント分はMC-331のKeita待ちリストへ追記） | Son | 送信内容=公開情報のみ（サイト名/URL/説明文） |
