...
| MC-156 | Apollo ターミナルビューを2/3/4分割で同時表示できるレイアウト切替を追加（現状はタブで1つずつ表示） | P0 | IN_PROGRESS（DONE誤判定→差戻し。2026-06-07 Keita実機で「分割中スクロール/テキスト入力不能」リグレッション。原因：フォーカス検知の透過オーバーレイ Terminal.tsx:1216-1228 が absolute inset-0 + pointer-events-auto で全ペインの iframe を覆い、全 pointer イベントを食う＝ttyd に届かない（iframe 境界はイベント透過不可でこの方式は破綻）。13:35 dist で配信され実害。ホットフィックス指示済(レン)：オーバーレイをフォーカス中ペインに出さない `&& !isFocusedPane`、または overlay 廃止し window blur+activeElement 検知に。デプロイ前に分割2/3/4でスクロール&入力&枠移動を必ず実機検証（今回それが抜けた）。回避策：レイアウトを1分割に戻せばoverlay非表示で操作可） | dev-logic |
