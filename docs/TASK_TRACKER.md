...
| MC-146 | chat-autonomous.sh の AGENT_TOKEN 取得パス修正（~/.mc.env→projects/cxo-agent/.mc.env） | 高 | bug | TODO（2026-06-04 apollo-keeper 起票。cron が 14:20/14:40 に連続 "AGENT_TOKEN is empty" で失敗中。chat-autonomous.sh が ${HOME}/.mc.env を参照しているが AGENT_TOKEN は projects/cxo-agent/.mc.env にある。MC-145 の cron は毎20分失敗しており MC-145 実機稼働の前提バグ） | dev-logic | MC-145 |
