...
| MC-191 | ニュース(/news)の可読性改善：トピックごとに行間/区切りを設け、図解（mermaid等）を入れて分かりやすく | P1 | REVIEW（★2026-06-08 02:46 Masayoshi リコンサイル：実装・反映済・Keita 見た目確認待ち。dev-apollo commit 1f67b3e→Masayoshi push ff9e4fc＋restart(active/API200)で本番反映確認。生成側は 2026-06-08 朝 7:03 cron で自動反映予定。残=Keita /news 見た目確認） | dev-apollo（ソラ）/ Masayoshi 反映 | なし |
