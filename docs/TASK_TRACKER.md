...
| MC-174 | ダッシュボード（/）の初期表示を「一番左のタブ」にする。現状 web/src/App.tsx:503 で常に `/today`（ブリーフィング＝最右タブ）へ Navigate しているのを左端タブ表示に変更 | P1 | IN_PROGRESS（2026-06-07 22:20 JST 林 cxo ティック。ソラに実装委譲。要件：useNavOrder('dashboard') で保存済みタブ順の先頭へ動的リダイレクト(fallback /plan-usage)。App.tsx:503 navigate('/today')→動的化。MC-175(NAV ラベル変更)と同梱・同1 build で対応。build/restart 後 commit まで） | dev-apollo（ソラ） | なし |
| MC-175 | ダッシュボード左ナビのラベル変更：「フォルダ」→「ドキュメント」、「ノートブック」→「RAG」 | P2 | IN_PROGRESS（2026-06-07 22:20 JST 林 cxo ティック。ソラに実装委譲。MC-174 と同梱・App.tsx 同1 編集で対応。/deliverables label/shortLabel 'フォルダ'→'ドキュメント'、/notebooks label/shortLabel 'ノートブック'/'ノート'→'RAG'。build/restart 後 commit） | dev-apollo（ソラ） | MC-174 と同梱 |
