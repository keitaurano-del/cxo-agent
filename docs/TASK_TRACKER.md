...
| MC-162 | ナビでダッシュボードと他ページ（ターミナル/フォルダ/ノートブック/チャット）が同時にアクティブ点灯する | P2 | REVIEW（2026-06-07 林実装完了。web/src/lib/nav.ts の NON_DASHBOARD_PREFIXES に /terminal-view /deliverables /notebooks /chat 追加。tsc -b/npm run build green。commit 68d018c。push/deploy 承認リクエスト req-0173b568-505b-41b5-bc27-700c8cbfed34 → Keita承認待ち） | dev-logic | |
