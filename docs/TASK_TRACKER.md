...
| MC-388 | Laundry.tokyo 実サイト化（実店舗データ投入・本番品質） | 高 | IN_PROGRESS | Son | Keita指示2026-08-21 21:33「実際のサイトも作ってくれる？」。プロトタイプ(デモ12店)を実サイト化: OSM Overpassで東京の実在コインランドリー店舗データを取得・投入、大量ピン対応(クラスタリング)、本番品質の仕上げ。公開は laundry.apollomansion.com 継続(laundry.tokyoドメイン取得は決済要=Keita/Cowork) |
