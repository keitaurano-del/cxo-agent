...
| MC-177 | ドキュメント/RAG ページの本文文言とアイコンを新名称に統一（MC-175 のラベル変更に追従） | P2 | BLOCKED（★2026-06-08 00:03 ①完了・②③は Keita 承認待ち。①= Deliverables.tsx 見出し title 'フォルダ'→'ドキュメント' 実装、フォルダ操作語(新規フォルダ/フォルダ名/ツリー/D&D)は温存確認済、web build green、commit 300ee25(main, NO_PUSH)。②/notebooks の 'ノートブック'→'RAG' 表記変更と ③nav アイコン差替は board 仕様で『候補提示→Keita 承認→確定』＝独断で名称/アイコンを当てず保留（人間ゲート）。Keita へ確認: (a)/notebooks を『RAG』表記に統一して良いか (b)アイコン案[RAG=DB系/検索系/AI(スパークル)系のいずれか]。承認後に②③を最小差分で反映。 ／2026-06-07 19:04 Keita 依頼・Masayoshi 起票。①/deliverables: Deliverables.tsx のページ見出し(L859 付近 title='フォルダ')→'ドキュメント'。ただしファイル操作系の『フォルダ』(D&D文言/フォルダツリー/新規フォルダ作成 L755等)は実フォルダ意味なので据え置き＝セクション名だけ変える。②/notebooks: 'ノートブック'表記(タイトル L3621/L3291・作成ボタン L3434・placeholder L3424)→'RAG'。ただし notebook 内の 'フォルダ' タブ(artifacts)は別概念=ソラ判断。③nav アイコン: /deliverables=DocumentsIcon, /notebooks=NotebookIcon を新名称に合うものへ変更。指定なしのため候補2-3案(例: RAG=DB/検索/AI系)を出して Keita 承認→確定。NO_PUSH継続・build/restart→commitまで→承認待ち） | dev-apollo（ソラ） | MC-175 |
...
| MC-194 | Apollo の動作改善（パフォーマンス・操作性・体感速度）：施策の洗い出し→優先度付け→実装 | P2 | BLOCKED（★2026-06-08 00:05 Son 実測完了→Keita 入力待ち。【API実測(2回計測)】tasks 36-55ms / agents 2.6ms / roster 17ms / narrative 1.7ms / vault 1.5-2.7ms / workflows 117-125ms / ticks 1-11ms / timeline 1.7ms / deliverables 20ms / secretaries 1ms / moods 1.4ms / alerts 142ms。＝**バックエンドは健全、目立つボトルネック無し**。最遅は workflows・alerts(~120-140ms, キャッシュ余地あるが軽微)。初回バンドルもエントリ671K/gzip200Kで中程度。【結論】推測での重い実装は不要かつ非推奨。残る選択肢は (a) 体感が遅い具体箇所を Keita が指す→そこを狙い撃ち (b) workflows/alerts に TTLキャッシュ(agents.ts同型, ~100ms削減・軽微) (c) route単位コード分割(初回描画・但し Masayoshi 編集域)。いずれも Keita の方針/承認待ち＝人間ゲート。 ／★2026-06-07 23:50 Son 進捗・診断修正: 【重要】当初診断は外れ＝重い可視化ライブラリ(mermaid/cytoscape/katex/wardley)は Mermaid.tsx の `await import('mermaid')` で既に動的ロード済み＝初回バンドルに非同梱だった。実際の初回ロードはエントリ index-*.js=671K(gzip200K)。Phase①(News の Mermaid を React.lazy 化)は実施したが削減~1.6KBと僅少＝branch perf/mc-194-mermaid-lazy(c83fada)に退避（低価値・任意マージ、main には入れず）。【本当に効く施策】=エントリ671Kの route 単位コード分割（App.tsx で各ビューを React.lazy）。ただし App.tsx/各ビューは Masayoshi 編集域と隣接＝衝突回避のため実測（どの操作が遅いか）＋Masayoshi 調整の上で実施。②collectorキャッシュ・③ポーリング集約も実測してから。＝推測実装を避け、次は計測ベースで。 ／原因候補(当初・①は誤判定と判明): ①初期ロード重＝可視化ライブラリ非分割（web/dist: mermaid 605K＋wardley 601K＋cytoscape 434K＋katex 256K＝計~1.9MB が初回バンドル同梱の疑い）→ React.lazy/dynamic import で遅延ロードし初回大幅軽量化【最優先】。②collector キャッシュ不足: agents.ts は TTLキャッシュ有だが vault/workflows/narrative は毎回ファイル走査の疑い→同様の TTL キャッシュ化。③ポーリング多重: Ticks/Agents/PlanUsage/Usage/Activity/Notebooks が個別 setInterval→集約/スロットル余地。実装は①→②→③の順で dev-apollo へ。NO_PUSH。 ／2026-06-07 22:45 Keita 指示「アポロの動作を改善する施策」を Son 起票。push/restart は Masayoshi 検証ゲート） | dev-apollo / dev-logic（Son ドライブ） | なし |
| MC-195 | 議事録機能の強化：特に PDF 出力時のレイアウト崩れ防止＋生成品質改善 | P1 | REVIEW（★2026-06-07 23:50 Son 実装・検証完了→Masayoshi push＋Keita目視待ち。【根本原因】議事録PDFは minutesExport.ts buildPdf() が playwright(chromium)+marked で HTML→PDF レンダリング（当初の officeToPdf/LibreOffice 推測は誤り）。CSS font-family が Hiragino/Yu Gothic/Meiryo/Noto Sans JP を指定するも、Linux サーバに全て未インストール（fc-list:lang=ja=IPAGothic/IPAPGothic と WenQuanYi(中)/Unifont のみ）→ generic sans-serif が中国語フォント/ビットマップにフォールバックして字形・行間が崩れていた。【修正】font-family に実在の IPAexGothic/IPAPGothic/IPAGothic を sans-serif の前へ明示（CSSのみ・apt不要）。commit ea85b49(main, NO_PUSH)。【検証】server tsc green / テスト議事録を実際にPDF化(74KB生成OK) / 出力PDFの埋め込みフォントが IPAPGothic に変わったことを確認（崩れ要因のフォントフォールバックが解消）。残=Masayoshi push＋Keita が実PDFで目視。※将来 fonts-noto-cjk を入れればさらに高品質（apt=要承認の別タスク）。 ／2026-06-07 23:36 当初 dev-apollo へ #dev 委譲も agent-dispatch バグで空振り→Son 直接実装に切替。 ／2026-06-07 22:45 Keita 指示「議事録機能の強化、特に PDF にしたときレイアウトが崩れない改善」を Son 起票。MC-151（ノートブック議事録生成）の後続改善。着手時=PDF 化経路（生成元 markdown→PDF レンダラ）の崩れ要因を診断→レイアウト安定化→dev へ委譲。push/restart は Masayoshi 検証ゲート） | dev-apollo / designer（Son ドライブ） | なし |
