...
| MC-269 | Google AdSense: アカウント作成→サイト審査通過→ads.txt→実広告タグ差込（現状 Aurora Cloud モック置換） | 高 | **一部解除**（2026-07-05: サイト側の審査前提が揃った＝**公開URL `https://pdfdotai.com/` 稼働・法務/使い方/FAQ/運営者(雛形)・robots/sitemapルート配信**）。残: ①Keita が AdSense 開設(Googleログイン/規約/住所/税務=人手・自動化不可)→publisher ID を Son へ ②Son が確認スニペット+ads.txt+広告タグ実装 ③Keita 審査申請 ④Google審査(人手) ⑤承認後 Son が Auto ads/実広告点火。※Googleログイン自動化は凍結リスクで非推奨 | Keita(開設/申請)/Son(実装) | クリティカルパス解けた。次はKeitaの開設 |
