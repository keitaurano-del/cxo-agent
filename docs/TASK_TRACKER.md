...
| MC-165 | MC-164 の作り込み：エージェントをドット絵キャラ＋吹き出しで擬人化（現状チープ・誰が誰か不明） | P1 | IN_PROGRESS（2026-06-07 林着手→両者brief完成。①designBrief（artifacts/MC-165_DESIGN_BRIEF.md）: 64×64pxピクセルアート12種(6人格×2状態)、Phase1レンサンプル→Keita承認→Phase2-3展開、工数14-18h、deadline2026-06-12。②implementBrief: personaMap.ts/AgentGroup拡張/PersonaCard.tsx新規、SSEリアルタイム状態切り替え(working/idle)、工数6-10h並行。並行スケジュール: designPhase1→approval→devPhase開始。DoD: 12asset完成+React統合+実機検証+Keita最終承認。次: designerレンsample作成開始→実装skeletonコード開始。） | designer（アオイ）＋dev-apollo（ソラ） |
| MC-166 | Keita がボードで手動変更した status を自動 🔒 ロックし、リコンサイル/keeper/ガードが戻さないようにする | P1 | TODO（2026-06-07 Keita「完了にしたタスクが戻される」根本対策。現状：Keita が Apollo ボードで status を変えても 🔒 が付かず、autonomous-worker のガード(MC-88)や apollo-keeper の DoD 差し戻しが HEAD ベースで元に戻す＝Keita の手動完了が消える（実例 AM-O が BLOCKED へ反復差し戻し）。要件：①Apollo ボード UI で Keita が手動 status 変更したら、その行に 🔒 を自動付与し即コミット（HEAD 反映）。②🔒/[Keita] 付き行は guard・apollo-keeper・autonomous-worker が status を一切変更・差し戻ししない（既存ルール project_board_keita_lock の徹底）。③autonomous-worker のガードを lock 認識に拡張＝🔒 行は status に関わらず HEAD から保護。Masayoshi 側で既に guard の PROTECTED に DONE 追加済（暫定緩和）。dev-apollo(ソラ)＝UI/コミット、Masayoshi＝guard 側の lock 認識。実機検証→承認フローで反映） | dev-apollo（ソラ）＋Masayoshi |
