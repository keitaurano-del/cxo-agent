...
| MC-344 | 開発アイデア生成を「メルカリ級プラットフォーム＋潜在市場規模分析付き」へ再刷新 | 中 | IN_PROGRESS（2026-07-25 Keita「メルカリのようなサービスを作れるアイデアに。潜在市場規模も分析しつつ」）。Son直接。IDEA_SEEDS=遊休資産流動化/非公式取引の公式化/セミプロ供給解放など潜在需給接続の12切り口、buildIdeaPrompt=「潜在供給×潜在需要を初接続・なぜ今まで市場にならなかったか・AIが障壁を壊す」基準＋出力最終文に潜在市場規模のフェルミ推定（対象人数×頻度×単価）を必須化、cleanIdea=2部構成対応で400字へ拡大。server tsc 0 | Son | server変更＝backend再起動要 |
