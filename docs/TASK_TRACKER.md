...
| 優先度 | P2 |
| ステータス | DONE（2026-07-21 Son棚卸しでクローズ: 設計フェーズDoD〔設計書レビュー通過・D1〜D5確定・Phase子タスク採番〕は 2026-06-15 Keita「おすすめでいいよ全部」で全て充足済み。実装も子タスク P1=MC-247／P2=MC-248／P3=MC-249／P4=MC-250 が全て DONE＝本設計票に残作業なし。元DESIGN: 2026-06-15 Son・設計書作成→要 Keita レビュー）。Keita 要望(音声 06:25)「カレンダーを週/日でも見たい・やることが多いのでスケジューリング・やることを予定に落とし込み自動でいつ何をやるか提示・**設計からちゃんと**」。設計書=`docs/MC-245-auto-scheduler-design.md`。方針=「AIで所要時間/優先度/最適時間帯を見積り、決定的アルゴリズムで重複ゼロ・締切順守の配置（説明可能）」。現状土台=月グリッド/予定・タスク・育児締切は読込済／予定作成APIは**終日のみ＝時間指定イベント作成はサーバ追加要**／Googleタスクに所要時間・優先度・固定可変が無く補完が肝。**要 Keita 確認 5点(D1置き場所/D2対象範囲/D3配置の賢さ/D4反映方法/D5前提制約)**。 |
...
| 優先度 | P2 |
| ステータス | DONE（2026-07-18 Son棚卸し: 全Phase実装・本番反映済＝PDFエディターは pdfdotai.com で PDF.ai として公開稼働。REVIEW=Keita実利用FB待ち）旧=IN_PROGRESS（Phase1 完了・本番反映 / 2026-07-03 **Son 直接実装**＝Keita「ソラに渡さなくていいから直接やって」）。**Phase1 実装済**: 新規 `web/src/views/PdfEditor.tsx`、依存 pdfjs-dist@4.10.38+pdf-lib@1.17.1 追加、App.tsx に3行結線（lazy import＋NAV"PDF"＋/pdf route、アイコンは暫定 DocumentsIcon 流用・icons.tsx 不変）。方式B(`npx vite build`)で web/dist 反映＝restart不要。**検証**: build EXIT0・全アセット/worker/`/pdf` 配信200・playwright ヘッドレスで実PDF(2p)読込→pdf.js 実描画(canvasピクセル確認)・「2ページ」表示・**console error 0**。App.tsx/package.json はソラWIPと同一→保全共有済(ソラは次コミットで3行＋依存2件を保持)。バックアップ /tmp/App.tsx.son-bak-pdf。 **Phase2 完了・反映**(注釈: テキスト/ハイライト/手書き/署名/選択削除。新規 pdfAnnot.ts/AnnotLayer.tsx/SignatureModal.tsx。検証: 注釈をPDFへ焼込 pdftotext確認・error0)。 **Phase3a 完了・反映**(本文インプレース編集＝pdf.js textContentでクリック編集→whiteout+再描画、＋**日本語フォント対応**＝@pdf-lib/fontkit で IPAゴシック(web/public/fonts/IPAGothic.ttf)をサブセット埋込。新規 EditLayer.tsx/pdfFont.ts。検証: 出力PDFに日本語「日本語テスト編集」を pdftotext抽出＆実画像で描画確認・Phase1/2非破壊・error0)。現行チャンク PdfEditor-DPv58H4U.js 配信200。依存追加 @pdf-lib/fontkit（保全共有済）。**既知の制限**: whiteoutは視覚上書きで原テキストはストリームに残る(真の削除/redactionではない)・複雑レイアウトは非再現・スキャンPDF不可。 **Phase3b 完了・本番反映**(変換方式＝`/api/pdf/extract`[pdftotext -layout で本文抽出]→モーダルで編集→`/api/pdf/build`[UTF-8 HTML→soffice でPDF生成]。新規 server/src/pdfConvertRouter.ts＋index.ts に import/mount 2行[バックアップ /tmp/index.ts.son-bak-pdf]。soffice は per-request UserInstallation で衝突回避・finally 削除。**反映手順**: 別ポート4319でブート検証[ソラWIPごと起動OK]→/api/pdf 往復をエフェメラルで検証→ソラへ一声→`sudo systemctl restart mission-control.service`[2秒復帰]→本番 extract/build 200＋playwright E2E[変換モーダルで日本語本文抽出・error0]確認。現行フロントチャンク PdfEditor-DQ9QCNyE.js)。 **⇒ MC-264 は Phase1(閲覧/ページ操作)・2(注釈/署名)・3a(インプレース本文編集+日本語フォント)・3b(変換方式本文編集) 全実装・本番反映済。REVIEW=Keita 実利用フィードバック待ち。 **スマホ最適化 完了・反映**(md未満で「ページ/編集」タブ切替・プレビュー可変幅(ResizeObserver)＋ズーム±・タップ領域44px・touchAction:none＋setPointerCaptureでタッチ描画・モーダル全画面。PdfEditor.tsx/AnnotLayer/EditLayer/SignatureModal 更新、現行チャンク PdfEditor-WNGRyv5H.js。playwright 390pxで検証: canvas366px/タップ44px/ズーム動作/error0/デスクトップ非破壊)。 **【第1弾 作り込み・Fable 5で実装／2026-07-04】Keita「acrobat有料エディタ並みに作り込んで」＋モデル Claude Fable 5(claude-fable-5, CLI経由=既定Opus据置・gateway再起動なし)で実装**: Undo/Redo(履歴+Ctrl+Z/Y)・**真の墨消し**(墨消しページをpdf.jsラスタライズ→黒塗り→画像差替=下のテキストを物理除去)・図形(四角/楕円/直線/矢印)・画像挿入・UX仕上げ(ツールバーグループ化/ズームプリセット幅/全体/100%/ショートカット/ツールチップ/選択プロパティパネル/DLボタン見切れ修正)。変更=PdfEditor.tsx/AnnotLayer.tsx/pdfAnnot.ts のみ(許可範囲・他者WIP非巻込・依存追加なし)。現行チャンク PdfEditor-DBEiO1fs.js 配信200。**検証**: Fable自己検証16/16 PASS＋**Son独立検証で真の墨消し=機密テキストが出力PDFのpdftotextで空(物理消滅)確認・console error0・スコープ遵守(mtimeで確認)**。次弾候補: PDF内検索/図形回転。 **【第2弾・Fable 5／2026-07-04】本文編集を Acrobat風に強化**: (1)本文編集ONで編集可能行を明確ハイライト(点線枠+塗り0.28,hover強調), (2)**編集後テキストを元フォントに合わせる**=getDocument に fontExtraProperties:true で埋込フォントのbytes保持→pdf.js commonObjs から取得→@pdf-lib/fontkit で同一TrueType再埋込(subset,全字cmap検証), 不可時は fallbackName/flagsで StandardFonts(Times/Helvetica/Courier×Bold/Italic)分類フォールバック, CJKは IPAGothic。新規 pdfOrigFont.ts＋EditLayer/pdfAnnot/PdfEditor変更(許可範囲・依存追加なし)。現行チャンク PdfEditor-Z7auflPL.js 配信200。**検証**: Fable自己検証＋**Son独立検証(モバイル実.tap()で編集→確定→反映・回帰なし・error0)＋出力PDF画像でセリフ体が周囲と一致を目視**。モバイル行タップ編集(前回修正)は維持。 【第3弾/2026-07-04 Son直接】(1)本文編集ハイライトの挙動修正=常時表示の点線枠が密集して重なる問題→点線を廃し薄い塗り(opacity0.16)のみ+hover/focusで枠強調, (2)指でピンチイン/アウトのズーム=previewWrapにネイティブtouchリスナ(passive:false)をhasDoc/mobileTab依存で装着, 2本指の間隔比でzoom更新(0.2〜4.0), 1本指は素通しでスクロール, touchAction:pan-x pan-y。EditLayer/PdfEditor変更。現行チャンク PdfEditor-C1RHEZ6U.js 配信200。検証(実タッチ): 合成2本指ピンチで100%→拡大400%/縮小67%・ハイライトすっきり目視・検証(実タッチ): 合成2本指ピンチで100%→拡大400%/縮小67%・ハイライトすっきり目視・モバイル実.tap()編集の回帰なし・error0。 【第4弾/2026-07-04 Fable5】本文編集を項目(run)単位化＋位置固定＋プレビュー切替に作り込み: (1)行まとめ廃止しpdf.jsのrun毎に編集(「名前:田中太郎」→"田中太郎"だけ編集), (2)EditAnnotにrun実測x/baseline/size保持→焼込で baseline=pageH-y-baseY で元ベースラインちょうどにdrawText, whiteoutはmax(元幅,新幅)で最小限=他項目は不動でレイアウト非崩れ(pdftotext-bbox一致・pdftoppmで差分は編集run矩形のみをFableが実証), (3)編集⇄プレビュー切替(mode-edit/mode-preview, プレビューはハイライト無しの最終見た目=DL同等)。EditLayer/pdfAnnot/PdfEditor変更・依存追加なし。現行チャンク PdfEditor-DNmlX6Pv.js 配信200。検証: Fable自己検証(bbox/pixel)＋Son独立検証(モバイル実.tap()で佐藤一郎→鈴木次郎の1項目編集・プレビュー切替でedit-box消滅・error0)。進捗ビューアに「コード変更(差分)」リアルタイム表示も追加。 【第5弾/2026-07-04 Fable5(run5)】**PDF上で直接インライン編集**: 別入力ボックス/確定バーを廃し、run の正確な位置に元と同じ書体/サイズ/色の contentEditable(白背景)を重ねてその場で書換。確定=Enter/blur・取消=Esc・空確定は変更なし・非制御でキャレット保持。項目単位/位置固定/元フォント一致/プレビュー/ピンチ/モバイルタップは維持。EditLayer/PdfEditor変更。チャンク PdfEditor-D4VeIUt6.js。検証: Son独立(モバイル実.tap()でインラインeditor位置 dx/dy=0一致・SPAN contentEditable・textarea0・確定ボタン0・確定反映・error0)。 【第6弾/2026-07-04 Fable5(run6)】**ダウンロード破損の真因特定＆根絶**: 真因=xref-stream/圧縮オブジェクト構成のAES保護PDF(Acrobat系)を `ignoreEncryption:true` で load→save まで通し「どのビューアでも開けない壊れPDF」を黙って出力していた(pikepdf/pdftoppmで再現)。修正=(a)読込時 pdf.js `getPermissions()!==null` で保護PDF検出し拒否, (b)buildPdf第2ゲート `pdf.isEncrypted` throw, (c)保存後に出力を厳格再パース(妥当性検証)して壊れDL根絶, (d)**ゴースト文字除去**=content streamトークナイザ+テキスト状態解釈で編集run帯の原文グリフを物理除去(Phase3a既知制限「原文がストリーム残存」を解消・fail-safe設計)。PdfEditor/pdfAnnot変更・依存なし。チャンク PdfEditor-BAc8pHmu.js。検証: pikepdf生成の各暗号方式でround-trip・鴻池組3p・ゴースト消滅を Fable自己＋Son独立(実.tap())で確認・error0。 【保護PDFのアプリ内自動解除(run7)は見送り/2026-07-04】保護解除(暗号復号)の自動実装は Anthropic利用ポリシーで自動ブロック(violative cyber content判定/FailoverError)→**安全フィルタ迂回はしない**方針(Keita「それはそうだね。まかせる」了承)。代替=Son直接で保護PDF検出時の**案内文を実用化**(「PDFを開く→印刷→PDFに保存 で保護を外したコピーを作り読込」)＋ユーザーへ print-to-PDF 回避策/個別pikepdf解除の案内。チャンク PdfEditor-C1QHmKNc.js 配信200。検証: 実.tap()で保護PDF読込→案内表示/編集ブロック/error0。 【第7弾/2026-07-04 Fable5(run8)】**編集を"元のフォント"で**: 画面側が総称CSSフォントだった主因を特定し、pdf.jsの埋込フォントbytes(font.data)を `new FontFace('mcpdf-<loadedName>')`＋`document.fonts.add()` でブラウザ登録→インラインeditor/確定プレビューの font-family を実フォント名に。書き出しは和文含め元フォント再埋込(subset)を最優先化(従来はCJKで無条件IPAGothic直行の欠陥を修正)、欠字時のみrun単位fallback。pdfOrigFont/EditLayer/pdfAnnot/PdfEditor変更・依存なし。検証: Son独立(実.tap()でeditorのfont-familyが登録実フォント・base14ラテンは総称fallback＝正しい・error0)。 【第8弾/2026-07-04 Fable5(run9)】**編集時に"元の文字色"を反映**: 従来 color:'#000000' ハードコードで元色を取っていなかった→EditLayerに `getOperatorList()` 解釈の色抽出を新設(pdf.js worker が setFillGray/CMYK/ColorN を全て setFillRGBColor へ正規化する性質を利用・q/Q・Form XObjectスタック尊重・showText開始位置でrun対応付け)、RGB/Gray/CMYKをhex化して RunBox.color→editorBox/EditAnnot へ。取れない時は黒fallback(fail-safe)。EditLayerのみ変更・依存なし。検証: Fable(RED/BLUE/GRAY/CMYK 画面色=出力色 完全一致)＋Son独立(実.tap()でeditor色=rgb(217,20,20)＝元赤・黒化せず・error0)。 【第9弾/2026-07-04 Fable5(run10)】**ハイライト精度＋UI**: (1)青ハイライト=タップ判定と同一で膨らみ精度低→**可視fillを文字ぴったり(幅=実測テキスト幅・高=fontSize)に、タップ判定は透明で44px系を維持**して分離, (2)編集⇄プレビュー切替を横スクロールtoolbarから独立させ**スマホ最上部の明確な2分割セグメント**(min44px・選択中アクセント塗り), (3)ツールバーを機能グループでまとめ視覚ノイズ削減(機能は不削)。EditLayer/PdfEditor変更・icons/App/依存 不変。現行チャンク PdfEditor-CXeQQc5q.js 配信200。検証: Son独立(可視fillH12.3px<タップbtnH22px・fill幅=テキスト幅・切替UI高54px最上部・実.tap()編集成立・色/フォント回帰なし・error0)。スクショ pdf-ui-mobile-v1.png / pdf-hl-tight-v1.png 配信。** |
...
|---|---|---|---|---|---|
| MC-266 | `pdfdotai.com`（PDF.aiサイト）だけ認証なしで一般公開（ダッシュボード/API/apollomansion は MC_TOKEN 保護維持） | 高 | **DONE**（2026-07-05 Keita GO。Fable5実装＋**Son敵対的検証**。デフォルト拒否の許可リスト＝公開はサイト静的＋`/api/ocr`＋`/api/pdf/unlock`のみ・他は404で保護資源を晒さない。新規 server/src/lib/publicSite.ts・auth.tsにバイパスフック・ホスト厳格判定。**検証実測**: 実ドメイン`https://pdfdotai.com/`無認証200／`/api/tasks`等404／正規トークンでもpdfdotai保護404／apollomansion無トークン401·トークン200無傷／ホスト詐称401。bak /tmp/index.ts.son-bak-siteroot） | Son | 公開達成 |
| MC-267 | OCR/compute API のレート制限（従量コスト・DoS対策） | 高 | **DONE**（2026-07-05 Fable5・新規 server/src/lib/rateLimit.ts。CF-Connecting-IP単位・自前スライディングウィンドウ・依存なし。`/api/ocr`=10/分·60/時·1000/日、`/api/pdf/unlock`=5/分·20/時·200/日、超過429+Retry-After。認証済(apollomansion)は対象外。**Son実測**: 空body連打で11回目から429確認） | Son | メモリ内(再起動でリセット) |
| MC-268 | 独立ドメイン確定・DNS/TLS・OGP(og:url)/sitemap の URL 更新 | 高 | DONE（2026-07-18 Son棚卸し・実機確認: pdfdotai.com 稼働(HTTP200)・専用トンネル/DNS 常駐）旧=IN_PROGRESS（**`pdfdotai.com` 取得・配信疎通まで完了** / 2026-07-05。**真因=ドメインがurano2アカウント・既存トンネルは主アカウントで cfargotunnel が跨げず1033**→urano2アカウントに新トンネル `pdfdotai-son`(TID 74498993…)をAPIで作成・remotely-managed・ingress pdfdotai/www→:4317・systemd `cloudflared-pdfdotai.service`(enabled/EnvironmentFile=/etc/cloudflared/pdfdotai.env)で常駐。DNS(apex/www CNAME→新TID.cfargotunnel)張替。**`https://pdfdotai.com/site/` =200 PDF.ai表示**確認。残: OGP/canonical/sitemap を新ドメインへ切替(定数1箇所)＝実公開時に。**要掃除**: 主アカウント側の誤登録 `pdfdotai.com.apollomansion.com`/`www.…` CNAME(害なし)） | Son | 実公開(認証OFF)は MC-266 GO 後。**追記2026-07-05: pdfdotai.com はルート配信=公開サイト(/site/をURLに出さない)。server/src/index.ts にホスト判定(SITE_HOSTS)追加→ルート/SPAフォールバックで site/index.html を返す・資産は/site/assets絶対参照で流用・apollomansionはダッシュボード維持・/site/後方互換維持。エフェメラル4977で検証後 mission-control 再起動で反映。バックアップ /tmp/index.ts.son-bak-siteroot。ダッシュボードの「公開版」リンクも pdfdotai.com/ ルートへ更新** |
| MC-269 | Google AdSense: アカウント作成→サイト審査通過→ads.txt→実広告タグ差込（現状 Aurora Cloud モック置換） | 高 | CANCELLED 🔒[Keita] | Google(審査)→Son(点火) | Keita作業は一旦なし |
| MC-270 | プライバシーポリシー本文の確定（第三者Cookie/パーソナライズ広告の開示・OCR送信の開示・問い合わせ先） | 高 | DONE（2026-07-18 Son棚卸し・実機確認: /privacy 200 でライブ配信）旧=IN_PROGRESS（Fable） | Fable/Son | OCR送信の開示は実装済。広告Cookie開示を追記 |
| MC-271 | 運営者情報・お問い合わせ窓口ページ新設 | 中 | DONE（2026-07-18 Son棚卸し・実機確認: /about 200 でライブ（運営者の実値差替は別途Keita））旧=IN_PROGRESS（Fable=雛形） | Fable/Son | 運営者情報の実値（主体/連絡先）は **要Keita**。雛形＋プレースホルダで用意 |
| MC-272 | Cookie同意(CMP)が広告/解析Cookieを実際にゲートする実装・EEA同意管理 | 中 | DONE（2026-07-18 Son棚卸し・実機確認: CookieConsent.tsx 実装・組込済）旧=IN_PROGRESS（Fable） | Fable/Son | CookieConsent.tsx 既存を実効化 |
| MC-273 | 利用規約の確定（免責・アップロード物の扱い・準拠法） | 中 | DONE（2026-07-18 Son棚卸し・実機確認: /terms 200 でライブ）旧=IN_PROGRESS（Fable） | Fable/Son | |
| MC-274 | robots.txt / sitemap.xml / canonical 整備 | 中 | DONE（2026-07-18 Son棚卸し・実機確認: robots.txt・sitemap.xml とも 200）旧=IN_PROGRESS（Fable） | Fable/Son | |
| MC-275 | コンテンツページ（使い方 / FAQ）新設＝SEO＋AdSense審査「価値あるコンテンツ」要件 | 中 | DONE（2026-07-18 Son棚卸し・実機確認: /faq・/how-to とも 200）旧=IN_PROGRESS（Fable） | Fable/Son | |
| MC-276 | 軽量化: AI-OCR化で不要になった tessdata(11-16MB)/tesseractアセット削除・和文フォントのサブセット最適化 | 中 | DONE（2026-07-18 Son棚卸し・実機確認: tessdata/traineddata はリポジトリに不在（削除済））旧=IN_PROGRESS（Fable） | Fable/Son | 参照グリップ確認のうえ未使用のみ削除 |
| MC-277 | OGP最終化（og-image 確定・twitter card・favicon） | 低 | DONE（2026-07-18 Son棚卸し・実機確認: og-image.png・favicon とも 200）旧=IN_PROGRESS（Fable=微修正） | Fable/Son | og-image.png/favicon.svg は存在 |
...
| MC-279 | AI-OCR（Gemini vision）完了後の独立検証 | 高 | DONE（2026-07-18 Son棚卸し・実機確認: AI-OCR 本番稼働・API検証済（tesseract→Gemini置換完了））旧=IN_PROGRESS（Son: API検証済/実機E2E残） | Son | tesseract→AI置換。本番 `/api/ocr` 200確認済 |
| MC-280 | クロスブラウザ実機QA（iOS Safari / Android Chrome / PC）＝公開前ゲート | 中 | CANCELLED（2026-07-18 Keita取り下げ・注力はClipItNow。必要時に再起票） | Son | |
| MC-281 | 運用監視・Gemini OCR 従量コスト監視・バックアップ | 中 | CANCELLED（2026-07-18 Keita取り下げ・注力はClipItNow。必要時に再起票）（旧TODO後段） | Son | |
...
| MC-284 | 解析から運営者本人アクセスを除外（notrack） | 中 | **DONE**（2026-07-05 Keita「本人のアクセス除外」。siteAnalytics.ts: `?notrack=1`で長期Cookie `pdfai_notrack` 付与→以後の来訪/OCR/DLを非計上・`mc_token`保有(認証済)も自動除外。当日テスト分(views13/DL3)リセット。エフェメラル＋本番で検証。**要: Keitaは各端末で `https://pdfdotai.com/?notrack=1` を一度開く**〈22:31に開いた分は実装前で無効〉） | Son | Cookieレス集計は維持 |
| MC-283 | 海外流入向けSEO/国際化（実パス化・**11言語**・LP・構造化データ） | 高 | DONE（2026-07-18 棚卸しでパーサ整合化・旧表記は**DONEで未完誤表示。11言語 本番稼働・Son検証 / 2026-07-06）**: en/ja/es/pt/de/fr/id/**ko/zh(簡Hans)/zh-tw(繁Hant)/hi**。パス方式(/<lang>/)・翻訳はGeminiをスクリプトでファイル出力(文脈超過回避)・hreflang全言語+xdefault・prerender言語×ページ静的HTML(99p)・sitemap全URL・言語別フォント(Noto KR/SC/TC/Devanagari)。簡繁を字体/語彙で分離確認。境界=保護404/未知ロケール404/apollomansion無傷/既存機能回帰なし 全て実測。※Fableのpkillで2度prod自動再起動→以後no-pkill厳守。**要注意: 全PDF.ai変更は共有ツリー上で未コミット(他agentWIP混在のため一括commit不可)→/tmp/pdfai-backup-20260706 に退避済。正式commitはチーム調整要**。旧DONE（本番稼働・Son敵対的検証: 実パス9本200＋英語SEOタイトル・保護資源404・apollomansion無傷・英語ファースト(Accept-Language)・OCR/解析回帰なし。LP=/edit-pdf・/edit-scanned-pdf・/add-text-to-pdf。sitemap 18URL+hreflang。prerenderで主要ページ静的HTML。※Fableのエフェメラル停止pkillが本番も巻き込み22:27自動再起動で先行反映→検証済）／旧**IN_PROGRESS**（2026-07-05 Keita「海外流入増やして」。Fable5実装中: ①HashRouter→BrowserRouter＋旧#リダイレクト＋ページ別メタ/canonical/OG/JSON-LD(SoftwareApplication/FAQPage)/hreflang＋クローラ用静的HTML ②英語ファースト(Accept-Language/国で自動判定) ③キーワードLP(/edit-pdf・/edit-scanned-pdf・/add-text-to-pdf)＋sitemap全URL。境界(公開GETのみ許可・保護404・apollomansion無傷)厳守。完了後Son敵対的検証＋再起動） | Son/Fable | 効果は解析の国別で計測。投稿系(ProductHunt/HN/Reddit)はKeita・文面Sonドラフト |

### 追加: 2026-07-06 解析ファネル・国別DL・同意ベース収集

| ID | タイトル | 優先 | ステータス | 担当 | 備考 |
|---|---|---|---|---|---|
| MC-285 | 解析にファネル(離脱ポイント)追加 | 中 | **DONE**（Fable5＋Son検証。来訪→開いた(open)→編集(edit)→DL押下(download_click)→完了。PdfEditorから各段sendBeacon(1文書1回)・既存eventルート拡張・notrack/bot/mc_token除外・Apolloに各段人数＋離脱率。本番各段+1確認・境界404・apollomansion無傷） | Son | 集計ボリューム型(個人経路でない・注記済) |
| MC-286 | 同意ベースのデータ収集(分析&OCR改善) Phase1 | 高 | **DONE（EN/JA・本番稼働・Son敵対的検証 / 2026-07-06）**。Keita当初「同意なし収集」→Sonが法的/信用リスクで保留・オプトイン同意形を助言→Keita承認。**オプトイン既定OFF**(localStorage pdfai_collect_consent・二重ゲート・無いと収集ゼロ)・**AES-256-GCM暗号化**(COLLECT_ENC_KEYは.mc.env・保存物先頭MCC1/平文%PDFなしをSon実測・鍵無しは保存せず204)・**公開から保存物404**(閲覧は認証のみ・復号round-trip)・匿名メタ集計(スキャン/種別/ページ数等)・**privacyPage全面改訂(private by default＋opt-in)**・/api/collectレート5分/サイズ30MB。新規 collectRouter.ts/collectStore.ts/CollectConsent.tsx/pdfCollect.ts。**残Phase2=9言語の表記/ポリシー翻訳**。注: at-rest暗号はサーバ侵害には非防御 | Son | data/collected(gitignore・暗号化) |
| MC-287 | ダウンロードの国別内訳 | 中 | **DONE**（2026-07-06 Son直接実装。siteAnalytics DayStatsに downloadsByCountry(CF-IPCountry・任意/後方互換)・recordFunnelEventのdownload時にbump・summary集計・Apollo「ダウンロード(国別)」カード。実測 FR2/JP1 反映。テスト分はクリーンアップ済） | Son | notrack/bot/mc_token除外は共通 |
| MC-288 | 💡「アイデアを生成」をバックグラウンドジョブ化（ページ離脱→再訪で結果が消えない） | 中 | **DONE**（2026-07-06 dev-apollo実装＋林API実機検証＋test-functionalブラウザE2E PASS。核心=離脱→復帰で要望欄にアイデア自動反映[実ブラウザで39秒後流入・空残らず]。localStorage'dev-idea-job-v1'生成中保存→完了クリア・二重押し防止・notfound静か通知・error数秒収束すべてPASS。API側もPOST即jobId→ポーリング停止跨ぎでdone取得を実証。restart反映済[Keita承認]。永続化なし=再起動窓のみ消え静か通知[要れば追加可]。E2E証跡 tmp/mc288-shots/・tmp/mc288-*.mjs[untracked]） | dev-apollo(ソラ) | git commit/push未・変更はワーキングツリー残置 |
| MC-289 | Apolloタブ整理: PDF計画/仕事の一部タブをVault格納して削除・壁打ちチャットに名前＋右下常設・体重グラフ点縮小 | 中 | **DONE**（2026-07-06 Keita依頼・Son直接実装＋実画面検証）。①**「PDF計画」ナビ削除**→内容をVault `30-Projects/pdf-ai/launch-plan.md`・`business-plan.md`へ格納。②**「仕事」からピボット/課題管理費/PwC転職タブ削除**→Vault `40-Resources/work-ecl/`の3ノートへ格納(概要/ナレッジ/単語帳の3タブに)。③**壁打ちチャットに名前「クレア」**付与＋タブ廃止→**画面右下の常設フローティングチャット**化(どのタブでも相談可)。④茶事・成長日記のGoogleフォトは維持。⑤**体重グラフの点 r3.5→2**に縮小。**検証**: web tsc0・dashboard build0・site再build0(99p)、apollomansion配信ハッシュ一致(再起動不要)、Vault API 5ノート200、OpenClawブラウザ実画面で /work=3タブ＋クレア右下・サイドナビ PDF計画消失・PDFサイト/PDF.ai解析/茶事維持を確認。**未コミット保留**: App.tsx等に他agent大WIP混在で分離不可→/tmp/cxo-wip-full-20260706-2231.patch＋/tmp/son-ui-20260706/にバックアップ | Son | 反映済(静的配信)・正式commitはツリー整理後にチーム調整 |
| MC-290 | MC-289/291 の web 変更（App.tsx・Work.tsx・BabyDiary.tsx）を共有ツリー整理後に正式コミット | 中 | TODO（【2026-07-18 進捗/Son】Ops出力・送信の tmux 完全一致修正 `server/src/terminalControl.ts` は単独で **commit 7d6fbfa 済**。残り（Work.tsx/App.tsx/BabyDiary/Terminal 等 web一式＋terminalUpload.ts）は **partial-commit 不可**＝(1)web の変更は未追跡の新規ファイル(PdfEditor.tsx 等)に依存し切り出すと dangling import (2)terminalUpload.ts は他agentの大容量アップロードstreaming WIP混在 (3)未追跡89件にスクショ/docx等の成果物ゴミが混在。**全体バックアップ /tmp/cxo-wip-full-20260718.patch＋status /tmp/cxo-wip-status-20260718.txt 取得済**（durability確保）。正式commitは dev系(ソラ)のWIPコミット＋未追跡のソース/ゴミ仕分けを伴うチーム整理が必要。／2026-07-06 Keita「あとでコミットして。チケットに残しておいて」）。現状: 本番反映は済だがApolloのweb作業ツリーに他agent(ソラ等)の大WIPが混在し一括commit不可→未コミット保留。対象=Work.tsx/BabyDiary.tsx（MC-289）＋App.tsx（MC-289のナビ整理）。※MC-291「Zaimu.ai」/MC-298「Governance.ai」のサイド項目は2026-07-11にプロダクト削除に伴い working tree で除去済（App.tsx/nav-order.json）→この2項目のコミットは不要化。バックアップ 最新 /tmp/cxo-wip-full-20260707-0730.patch・/tmp/son-ui-20260706/。**やること**: 共有ツリーのWIPがコミット/退避されクリーンになったら該当ファイル分を切り出して commit＋push。※`web/public/fin-mockup.html`・`web/public/fin-stmt-*.png` は独立静的ファイルのため先行コミット済。**【2026-07-11 追加】ターミナルURL化も同梱**: モバイルで表示中と別端末へ誤送信するバグの根治として、送信先の正を localStorage state から **URLの :id** に一本化（App.tsx に `/terminal-view/:id` ルート追加＋Terminal.tsx で useParams/useNavigate 化・iframe常時mountは維持）。build green・playwright モバイルUAで send-keys の terminal がURL id と一致することを実証済（3→3/4→4/5→5/1→1、素の /terminal-view は /terminal-view/1 へ正規化、/terminal-standalone は従来フォールバック）。**dist反映済＝Apolloライブ**。バックアップ /tmp/terminal-url-backup-20260711/。commitはこのMC-290の共有ツリー整理に相乗り。 **【2026-07-11 追加2】仕事ページのタブ再編も同梱**: 仕事(/work)に「動画DL」タブ(Clip広告計画要約)と「PDF」タブ(PDFサイト公開版/PDF.ai解析リンク)を新設＋サイドメニューからPDF系2項目を撤去(/analytics routeは後方互換で残置)。build green・dist反映済＝Apolloライブ。バックアップ /tmp/work-pdf-tabs-backup-20260711/。commitはMC-290相乗り。【2026-07-22 棚卸し/Son】共有ツリー実測=変更80件・未追跡51件でWIP混在継続→ブロック解消せず現状維持（バックアップpatch取得済のため実害なし）。【2026-07-23 棚卸し/Son】前進あり: 対象のうち **Work.tsx / App.tsx は commit 1e7cf71（MC-317〜321）で正式コミット済**＝MC-290残スコープは BabyDiary.tsx・Terminal.tsx・terminalUpload.ts のみに縮小。共有ツリー実測=変更29件・未追跡51件（昨日80→29に減、WIP整理が進行中）。残3ファイルは依然他WIPと混在のため引き続き整理待ち。【2026-07-25 棚卸し/Son】実測=変更29件・未追跡51件（前日から横ばい）→現状維持。【2026-07-26 棚卸し/Son】実測=変更29件・未追跡51件（横ばい）→現状維持。【2026-07-27 棚卸し/Son】実測=変更29件・未追跡51件（横ばい）→現状維持。【2026-07-28 棚卸し/Son】実測=変更31件・未追跡51件（+2、残3ファイル BabyDiary/Terminal/terminalUpload は依然WIP混在）→現状維持。【2026-07-29 棚卸し/Son】実測=変更31件・未追跡51件（横ばい）→現状維持。【2026-07-30 棚卸し/Son】実測=変更31件・未追跡51件（横ばい）→現状維持。【2026-07-31 棚卸し/Son】実測=変更31件・未追跡51件（横ばい）→現状維持。**【2026-08-01 ワークループ第1巡・Son subagent仕分け完了】**全82エントリを仕分け=docs/mc290-tree-triage-20260801.md。結果: A(単独コミット可)22/B(ペアで可)11/C(破棄候補)13/D(要作者確認)4・server typecheck/web tsc両PASS＝壊れ変更なし。**起票時の「partial-commit不可」は解消済**（当時の障害PdfEditor.tsx依存やApp.tsx WIPは消滅）→**12塊で全ソース変更を分割コミット可能**（BabyDiaryぴよログ化=本丸は単独可）。要注意発見: web/public/shibata-resume-pwc-iag*.docx×3（Keita履歴書・個人情報）が残存→コミット厳禁・**本番実測=非認証401で外部露出なし**・削除はKeita確認待ち。次=夜のまとめでコミット実行計画を提示【2026-08-01 13:25 Son/MC-354ループ】塊8/9/10(Sonレーン分)を実コミット＝BabyDiaryぴよログ化 9505f38・仕事教材残置 8f872ef・クレア線引き 6cac28d（web tsc PASS・push済）。MC-290のSon自身の残スコープはこれで解消。残りはソラ等他者レーンの塊(1〜7,11,12)＝夜まとめで実行計画を提示し担当承諾後に順次【2026-08-01 16:17 Son/MC-354ループ】他者レーン塊(1〜7,11,12)の代理コミット可否を devチャットでソラへ打診済（(a)Son代理コミット/(b)本人コミットの二択・21時未返答なら型チェック済分を(a)で実行、D判定のBottomNav/icons.tsx・artifacts 2件は除外）。返答待ち【2026-08-01 21:30 Son/MC-354ループ】21時期限までソラ返答なし→予告通り塊1〜7・11・12を代理コミット実行（fcf5bc2/7ceabd9/966568a/4f8c8a1/7341354/7d34321/4464ee8/2a109b1/d27db94・server/web両tscPASS再確認・push済・devチャットで事後報告済）。共有ツリーの未コミットは C判定(fable-progress/tsbuildinfo/tmp/スクショ/resume docx=コミット厳禁)とD判定(BottomNav/icons.tsx/artifacts 2件=作者確認待ち)のみに縮小。残処置=D判定の意図確認とC判定の.gitignore整備提案【2026-08-02 09:20 Son/MC-354ループ】残尾のC判定消化=.gitignore整備 commit 811dd50（tmp/・artifacts/・_avatar-archive-*/・web/public/*.docx を恒久除外＋*.tsbuildinfo一般化・check-ignore全パターン検証済・statusノイズ17→10行・履歴書docxのコミット事故を構造的に防止）。残尾=D判定(BottomNav/icons.tsx)のソラ意図確認・resume docx削除のKeita確認のみ | Son | 依存=共有ツリー整理（[[cxo-agent-shared-tree-concurrency]]相当）。棚卸しで挙げた117ファイル未コミット問題と同根 |
| MC-291 | 【新規プロダクト構想】企業名で検索→決算書/有価証券報告書/財務諸表を自動取得・分析するオンラインプラットフォーム | 高 | CANCELLED（2026-07-11 Keita「Zaimu.ai は削除」→プロダクト廃止。成果物は.trash/deleted-zaimu-governance-20260711へ退避・cxo-agent push 28d4ed7）。旧:TODO（2026-07-06 Keita構想）。**要件(初出)**: ①決算書の読み方の解説 ②バランスシート(BS)状態の分析 …等。企業を検索すると開示資料を自動で読みに行き分析・解説する。**Son初期スコーピング(2026-07-06)**: データ源の実現性◎— **日本企業=金融庁EDINET API v2**(2026-06仕様更新・無料APIキー・有報/四半期のXBRL取得可)／**米国企業=SEC EDGAR**(無料・XBRL companyfacts)。XBRLから財務3表を機械可読で取得→指標算出(流動比率/自己資本比率/ROE/ROA/売上・利益トレンド等)＋AI(Gemini)で「読み方解説」「BS健全性の所見」を生成、が現実的MVP。**未決定(要Keita)**: (a)対象=日本(EDINET)先行かグローバルか (b)無料/広告 or 課金 (c)独立サービスか Apollo/PDF.ai 系列に載せるか (d)対象ユーザー(投資家/就活・転職/経営学習)。方針確定後に計画分解→Son実装。**【2026-07-06 方針確定 Keita「各1番」】**: ①対象=**日本企業先行(EDINET)** ②位置づけ=**独立サービスで一般公開** ③主対象=**投資家・個人株主** ④進め方=**まず計画書**。**計画書作成済**→`docs/financial-analysis-platform-plan.md`＋Vault `30-Projects/financial-analysis/plan.md`(Apollo Vaultで閲覧可)。法務: EDINETは公共データ利用規約PDL1.0で商用二次利用可・出典記載必須／金商法=個別売買推奨せず情報提供・教育に徹し投資助言業を回避＋免責。**次アクション=Keita計画レビュー→OKでMVP(P1)着手**。**【2026-07-06 Keita追加方針】**: 助言はしないが**理論株価・健全性・バリュエーション(適正化)を客観指標として表示**(前提明示・複数モデルレンジ・売買推奨や目標株価助言にはしない=金商法両立)。AIは「気になる点/現状」の観点で分析し、**各所見を財務諸表の具体箇所(BS/PL/CFの科目)に紐づけ＋原典リンク**(グラウンディング必須)。競合他社比較で特徴を整理。UIはこだわり点→海外参考(Simply Wall St/stockanalysis/Koyfin等)。計画書に§客観指標・§UI方針を追記済。**UIモック v1 制作・公開済**→`web/public/fin-mockup.html`。**【2026-07-06 Keita追加指示】**: ①「財務諸表をどう読むか/そもそもどういう意味か」をもっと深掘り解説→**Fable5に作成させる** ②UIはもっとシンプルに・**AIっぽさを排除** ③モックをApolloサイドメニューに入れて即確認可能に。**対応済(Son)**: Fable5(claude-fable-5・CLI)に詳細spec(/tmp/fin-mockup-spec.md)を渡し**fin-mockup v2 を再制作**＝ライト基調・単色アクセント・「決算書とは」導入＋BS/PL/CF/各指標の意味を噛み砕く解説ボックス10＋用語辞書、AI演出は全廃・出典タグ(BS/PL/CF科目)は維持。**サイドメニューに「財務分析（モック）」項目を追加**(App.tsx・/fin-mockup.html外部リンク)。web再build＋site再build＋配信確認、OpenClawブラウザ実画面で 導入/BS読み方/サイドバー項目 を検証。fin-mockup.html は先行コミット、App.tsxナビ追加は MC-290 で未コミット保留。**【2026-07-07 Keita追加指示】「もっと実際の財務諸表ベースで／実際の紙ベースの決算書のキャプチャも使って説明」→対応済(Son直接)**: トヨタ2026年3月期の**実際の決算短信PDF(IFRS連結)を取得→連結BS/PL/CFの紙面をpdftoppmでPNG化**し `web/public/fin-stmt-{bs,pl,cf}.png` として配置、fin-mockup を **実キャプチャ＋読みどころ注釈版(v4)** に作り直し。実開示の実数のみ使用(現金12,659,622/棚卸5,134,996/資産合計105,522,331/営業収益50,684,952/営業利益3,766,216〈前期4,795,586から3期連続減益〉/税引前5,152,996/親会社純利益3,848,098/営業CF+5,472,920・投資-1,520,307・財務-536,659)。各行を紙面に紐づけ意味＋出典タグ(BS/PL/CF科目)で解説、自己資本比率37.8%・営業利益率7.4%は実データから算出。免責＋出典(トヨタIR/EDINET)明記。実画面検証済(実キャプチャ+ハイライト注釈)。※Fable5起動もしたが実データ精度確保のためSonが直接組成(v3途中停止)。**【2026-07-07 Keita追加「テキストはどこまで言える? 数字だけでなく文字情報も分析して」→対応済(Son直接)**: トヨタ実際の**有価証券報告書PDF(241p)を取得**し、定性開示の実テキストを抽出→モックに「④ 開示テキスト(文字情報)の分析」を追加。A:**MD&A(経営者分析)** 実引用「営業利益3兆7,662億=前期比▲21.5%減益、諸経費増(**米国関税の影響1兆3,800億円を含む**)が主因」＝PLの増収減益の"理由"を数字に結合／B:**事業等のリスク**実列挙(競争激化・需要変動・原材料価格上昇・為替金利・気候変動/電動化・供給依存・人材・情報セキュリティ等)／C:**経営方針**(モビリティカンパニーへの変革・「幸せを量産」)／D:**セグメント別**(自動車 営業益2兆7,770億=減益・金融 8,517億=増益)。各に有報出典タグ。「テキスト分析でできること」も明示。実画面検証済。fin-mockup.html＋fin-stmt-*.png コミット済(独立静的)。**【2026-07-07 Keita「検索窓／理論株価など主要情報を計算式つき／サービス名Zaimu.ai／テクニカル分析も／Fable5で実装／海外サイト参考」→対応済**: **Fable5(claude-fable-5・CLI)にspec-v5＋実データを渡し実装**。①上部に**企業検索窓**(人気企業チップ)②**サービス名Zaimu.ai化**(タイトル/ブランド/サイド項目App.tsxも改名)③**主要指標を全て計算式つき**(PER=株価÷EPS=2923÷295.25=9.9倍・PBR/EPS/BPS/ROE/ROA/時価総額等)④**理論株価**をPERベース/PBRベース/DDM/DCFの**各計算式・前提つき**参考レンジ¥2,700-3,200＋割安割高ゲージ(助言でない旨明記)⑤**テクニカル分析**(年初来高¥4,000/安¥2,686/現在¥2,923の実データチャート＋移動平均/RSI/MACD/ボリンジャー/サポレジを計算方法つき・株価時系列は参考イメージと明記)。実データ(株価¥2,923・時価総額42.66兆・EPS295.25・BPS3,063・発行済15,794,987,460株/自己株2,761,602,986株)。既存の実決算書キャプチャ＋文字情報分析は保持。web+site再build・実画面検証済(検索窓/理論株価計算式/テクニカル/サイドZaimu.ai)。**【2026-07-07 追加反復(Son/一部Fable5)】**: BS紙面が資産のみだった指摘→負債・純資産ページ(fin-stmt-bs2.png)追加／スマホ主要指標を3グリッド化／検索直後の**AIサマリー**／**現在の株価の分析**(PBR0.95=純資産割れ等)＋**最近の主要ニュースと分析**(決算/米国関税1.4兆/7月日米合意で関税27.5%→15%/来期見通し/電動車500万台)／**主要指標の計算式をデフォルト折りたたみ**＋**財務諸表の主要行ホバー/タップでハイライト解説(v6・Fable5)**。**【複数社対応 v7・Fable5】**: **トヨタ/任天堂/キーエンスを会社セレクタで切替＋冒頭に3社比較表**。任天堂(Switch2で売上1.16→2.31兆・営業益3,601億・自己資本比率77.7%・PER19.1/PBR2.74)・キーエンス(売上1.17兆・**営業利益率51%**・自己資本比率94.6%・PER43.7/PBR5.61)も**実際の決算短信PDFから実数抽出＋BS/PL紙面キャプチャ(fin-ntt-*.png/fin-key-*.png)**。実画面検証済(切替/比較表/各社実キャプチャ)。**Fable5実装は進捗ページ`apollomansion.com/fable-progress.html?token=`でライブ表示**(Keita 2026-07-07指示、[[pdf-work-via-fable5-with-progress-page]]・パーサにmeta対応)。全て独立静的ファイルとしてコミット済。【2026-07-28 Son・参考資料】構想再開時用にCowork(claude.ai)で競合10社（バフェット・コード/株探/IRBANK/moomoo/Ullet/ザイマニ＋発掘4）の実閲覧調査を実施→機能マトリクス/強み弱み/空白分析（「初心者向けAI決算翻訳の独立Webサービス」は決定版不在＝空白度中〜高）/差別化3案。レポート=Vault `30-Projects/financial-analysis/competitor-analysis-20260728.md`（ステータスはCANCELLEDのまま・再開判断はKeita） | Son | データ源: EDINET API v2 / SEC EDGAR（無料）。PDF.ai部品流用。レビュー後にMVP起票 |
| MC-292 | PDF.ai(pdfdotai.com) ダウンロード0件の分析とグロース着手 | 高 | CANCELLED（2026-07-18 Keita取り下げ・注力はClipItNow。必要時に再起票）。旧=IN_PROGRESS（2026-07-07 Keita「pdf.ai分析して。DLが0件」→Son分析）。**診断: DL機能の不具合ではなく実エンゲージメント欠如**。ファネル7/5-7/7= 訪問1,293→PDFを開いた人わずか2→編集0→DLクリック0→DL0。DL計測(open/edit/download_click/download の sendBeacon)はコード上正しく実装済＝計測バグではない。**流入の質が低い**: 参照元99.9%direct(検索はBing1件のみ)・国はFR389/US379/CA231…でJP12・PDFを開く率0.15%＝大半がボット/クローラの水増し。真因=公開2日目でGoogle未インデックス・AdSense審査待ち・被リンク無し＝オーガニック流入未発生(robots/sitemapは正常)。**着手済(Son)**: ①入口改善=モバイル/タブレットで入口画面(PDF未オープン時)のバナー広告を非表示にしCTA「Open a PDF to edit」を主役化(EditorApp.tsx・build:siteで反映済・実画面確認) ②ボット除外強化=siteAnalytics.ts のBOT_UA_REにAIクローラ/SEOツール/自動化/プレビュー系を追加(server tsc0・**反映は次のbackend再起動時**＝共有WIP巻き込み回避で深夜再起動は保留) ③DL計測はコード確認で健全。バックアップ /tmp/pdfai-fix-20260707/。**残(要Keita/準備中)**: Google Search Console/Bingへのsitemap登録・インデックス申請(手順Son用意)、初速露出(ProductHunt/HN/Reddit投稿・文面Sonドラフト)。 | Son | 未コミット(共有ツリー方針)・[[pdfai-public-launch]]。判断=viewsでなくopens/DLで評価すべき(views水増し) |
| MC-293 | Zaimu.ai モック 実害・法務・UX 即修正（fin-mockup.html） | 高 | CANCELLED（2026-07-11 Keita「Zaimu.ai は削除」→プロダクト廃止。旧成果は下記だが対象HTMLごとtrash退避）。旧:DONE（2026-07-08 実装・Playwright実機検証〈検索/会社切替/JSエラー0・数値/免責/AIっぽさ排除/法務文言 確認〉・commit 62d9221 push済・dist反映済）。2026-07-07 Keita「全部やって」→Masayoshi が4観点で改善点洗い出し→dev subagent実装。**対象=`web/public/fin-mockup.html`(独立静的・git clean)**。内容: ①検索窓を機能化(3社部分一致＋サジェスト・現状onsubmit return false) ②数値バグ修正(トヨタ「13.03億株」→130.3億株の桁誤り／キーエンスPLの為替差損・営業外費用が前期値を当期列に取違え＋営業外明細欠落／時価総額の株数がEPS/BPSと不整合=内部整合再計算) ③AIっぽさ排除(「AIサマリー」バッジ→概要・「AI分析」ラベル→解説・過剰太字削減) ④法務文言(テクニカルの買い/売りサイン→中立用語解説・「割安圏/下値目安/反発材料」→事実+相対位置・免責グローバル常設+各節注記・ダミー株価明示) ⑤a11y/モバイル(免責コントラスト・財務表overflow・h2化・ピンfocus) ⑥流動比率KPI追加・3社セクション非対称の是正。**財務検算の裏取り済**(BS貸借一致・PER/PBR/利益率等は正・誤りは②のみ)。 | Masayoshi(dev subagent) | [[zaimu-ai-review-20260707]]。MC-291派生。Son fin-mockup編集と衝突注意(着手時clean確認済) |
| MC-294 | Zaimu.ai 本番化ブロッカー: 株価・市場データ源の確定 | 高 | CANCELLED（2026-07-11 Keita「Zaimu.ai は削除」→プロダクト廃止で不要）。旧:TODO（2026-07-07 レビューで判明した"隠れ最大ブロッカー"）。EDINETは株価を持たない→PER/PBR・時価総額・理論株価・テクニカル・配当利回りが全て株価前提で全滅。第一候補=**J-Quants API(JPX公式・登録制)**。株価APIの規約(表示/再配布/帰属/遅延)とコスト確認込み。確定まで株価系は「サンプル値」表示に留める。計画書§4/§11に追記済。 | Son/Keita | MC-291/295の前提。[[zaimu-ai-review-20260707]] |
| MC-295 | Zaimu.ai MVP実装: 検索→EDINET→XBRL→指標→動的描画の縦串 | 高 | CANCELLED（2026-07-11 Keita「Zaimu.ai は削除」→プロダクト廃止で不要）。旧:TODO（2026-07-07）。現状モックは見た目9割・**本番実装は実質0割**(3社ハードコードの静的物・手書きSTMT/WHY/POINTS辞書)。MVP=1社でも「検索して動的に出る」に。キャプチャPNG方式は捨てXBRL→動的表に一本化(全社ぶんキャプチャは作れない)。会計基準差の科目マッピング＋「合計=明細和/貸借一致」検算＋取得不可は明示。React18+Viteでコンポーネント化(現1109行単一HTML＋3社複製は保守限界)。WHYはGemini自動生成(確定数値を渡し数値生成させない=グラウンディング)。 | Son | MC-294確定が前提。計画書§5完了条件。[[zaimu-ai-review-20260707]] |
| MC-296 | Zaimu.ai コンテンツ本番化: 株主還元・安全性/効率性指標・比較・時系列 | 中 | CANCELLED（2026-07-11 Keita「Zaimu.ai は削除」→プロダクト廃止で不要）。旧:TODO（2026-07-07）。投資家の必須論点の穴を埋める: **株主還元(配当利回り/配当性向/自社株買い)=現状空欄で致命的**、流動比率/当座比率/回転系、**同業比較テーブル(計画で「核」なのに現行モックから退行で消失)復活・MVP昇格**、経年時系列(現3期棒→5〜10期)、セグメント可視化。 | Son | MC-295と並走。[[zaimu-ai-review-20260707]] |
| MC-297 | Zaimu.ai 公開前 法務チェック（金商法） | 高 | CANCELLED（2026-07-11 Keita「Zaimu.ai は削除」→プロダクト廃止で不要）。旧:TODO（2026-07-07）。公開前に必ず潰す: テクニカルの売買サイン表現(個別銘柄ページ上は投資助言リスク最大)・「割安/割高」等の評価/見通し語・免責のグローバル常設と各分析節への再掲・出典の原典(EDINET docID)実リンク・ダミー株価/チャートの実データ化前公開回避。**弁護士確認を推奨**。MC-293で文言の一次対応は実施。 | Keita/Son | 計画書§8/§11。[[zaimu-ai-review-20260707]] |
| MC-298 | 【新規プロダクト】Governance.ai — 公開情報から企業ガバナンスを自動採点する独立Webアプリ（設計〜Fable5でMVP） | 高 | CANCELLED（2026-07-11 Keita「Governance.ai は削除」→プロダクト廃止。governance-ai リポ＋governance-app配信を.trash/deleted-zaimu-governance-20260711へ退避・cxo-agent push 28d4ed7）。旧:REVIEW（2026-07-08 Keita「モックは全然だめ。設計からやり直し・前提から・Fable5である程度動くものを・全部まかせる」）。前段の静的モック`web/public/governance-mockup.html`（サイド「Governance.ai（モック）」）は不採用。**Son設計(2026-07-08)**: 単体SPA(React+Vite・ハッシュルーティング)を独立ディレクトリ`/home/dev/projects/governance-ai`で実装→静的ビルドを`web/public/governance-app/`へ配置しapollomansion配信(cxo-agentバックエンド改変ゼロ=ソラWIP衝突回避)。**スコアリングエンジンをクライアント側に**(8軸=独立性/多様性/資本効率/政策保有/報酬透明性/株主権利/開示IR/リスクコンプラ・重み可変・グレード帯)=閲覧でなく操作可。**実データ**=実在TSEプライム10社の実ガバナンス指標(社外取比率/女性比率/政策保有純資産比/ROE/PBR/買収防衛策/取締役選任賛成率/機関設計/CGコードcomply率等)を出典付きでSon調査しシード化。**AI解説**=GEMINI_API_KEYで各社定性分析を生成(確定数値を渡しグラウンディング)。ピア比較/レーダー/レッドフラグ/原典リンク/methodology。**実装=Fable5(claude-fable-5・CLI)**にspec+実データを渡す。進捗は`apollomansion.com/fable-progress.html?token=`ライブ表示。**共有ブロッカー**: EDINET APIキー無し(401)→「任意企業ライブ採点」は次段(MC-294/295と同根)、経路のみ用意。**【2026-07-08 実装完了・Son独立検証済】**: Fable5(claude-fable-5)が仕様どおり実装(governance-ai リポジトリ commit 0688587)。単体SPAをビルドし cxo-agent `web/public/governance-app` へ配置、`apollomansion.com/governance-app/` でライブ(HTTP200・title正)。Playwright実機検証: 10社カード/検索/セクター絞り込み/並び替え、企業詳細の8軸スコアがルーブリック式と一致(例=三菱UFJ 総合B+71.8: 独立性100〈社外53%〉/政策保有33〈28.3%〉/株主権利40〈賛成率64.58%〉/リスク52〈貸金庫・銀証違反〉)・自前SVGレーダー・**重みスライダー8本で即時再計算**・レッドフラグ・出典13リンク・Gemini AI解説10社(grounded)・コンソールエラー0。サイドメニューを「Governance.ai（モック）」→実アプリ「Governance.ai」(/governance-app/)へ差し替え、web+site再build・実機確認。governance-app静的ファイルはcxo-agentに独立コミット、App.tsxのナビ差し替えはソラWIP混在のため未コミット(MC-290系で処理)。**次段(TODO)**: EDINET APIキー(無料登録・要Keitaメール確認)取得で任意企業ライブ採点(MC-294/295と共有ブロッカー)。 | Son | MC-291(Zaimu.ai)の姉妹。[[pdf-work-via-fable5-with-progress-page]]。EDINETキーは共有ブロッカー(MC-294) |
| MC-299 | 【新規プロダクト】動画ダウンロードサイト（yt-dlp バックエンド） | 中 | DONE（2026-07-10 Keita「動画をダウンロードできるサイト作って」→「動くサイト作って」）。**Son実装・実機検証済**。方式=独立Pythonサービス`/home/dev/projects/video-dl/server.py`(stdlib http.server・HTTP/1.1)がvenvの`yt-dlp`(curl_cffi impersonation)＋ffmpegをラップ。GET/=UI、POST/api/info(`yt-dlp -J`)、GET/api/download(プリセット固定 best/720/480/audio・シェル非経由の引数配列・URLはhttp(s)のみ許可)。**配信=専用サブドメイン** `https://videodl.apollomansion.com`（apollo名前付きトンネル90668daeにingress `videodl→:4319`を追記・DNS CNAME作成・SIGHUPリロード。既存apollomansion.com設定は保全、リロード時502は数秒で復帰=通常401に）。**永続化=systemd** `video-dl.service`(enabled/Restart=on-failure)。**重要な制約(実測)**: このサーバのデータセンターIPは**YouTube(ボット判定)・Vimeo(TLS指紋/IP)をブロック→非対応**。動くのは**直リンク動画(.mp4等)・archive.org(パブリックドメイン/CC)・IPブロックしない対応サイト**。UIに非対応明記＋権利/著作権の注意書き。クイックトンネル(trycloudflare)はこの環境で一貫404のため名前付きトンネル採用。検証: 公開URLで解析200(Big Buck Bunny)・DL(flower.mp4/archive.org 480p 61.8MB)成功・UIコンソールエラー0。**次段(任意)**: YouTube対応(Cookie投入/別ネットワーク実行)・Apolloサイドメニュー追加・アクセスゲート。 | Son | 独立サービス(cxo-agent非依存)。IPブロックが本質的制約 |
| MC-300 | PDF.ai ダウンロード前の広告インタースティシャル(黒画面+秒数カウントダウン)を撤去 | 中 | DONE（2026-07-14 Keita「タップしたら黒い画面で秒数だけ表示されるやつを削除して」）。**Son直接・実機検証済**。pdfdotai.com エディターのDL押下→AdInterstitial(全画面黒overlay+×まで秒数カウントダウン+モック動画広告)を挟む導線を撤去。変更: `web/site/pages/EditorApp.tsx` の `<PdfEditor adGate .../>` から adGate を除去／`web/src/views/PdfEditor.tsx` から adGate prop・showAdGate state・exportAllのadGate分岐・レンダリング分岐・AdInterstitial import を削除し、DL後の入口リセット(resetAfterDownload)は doExport 成功パスへ移設（挙動保持）／`web/src/views/AdInterstitial.tsx` 削除・`web/site/tailwind.config.js` の参照除去。web tsc 0・build:site green。**実機検証(Playwright/本番pdfdotai.com)**: PDFを開く→pdf-download押下で ad-interstitial/ad-countdown が出ず、直接 `*-edited.pdf` がDL・console error0＝PASS。dist/site 再build済＝ライブ反映。**git**: 共有ツリーに web/site 等が未追跡+他agent大WIP混在のためコミットは MC-290/292 と同様に保留（配信は作業ツリーdistから）。 | Son | [[pdfai-public-launch]] 関連MC-292。収益動線はレール広告/編集中バナー/AdSense審査に一本化 |
| MC-301 | ClipItNow に ExoClick 広告を追加（ExoClick API 利用） | 中 | DONE（2026-07-14 Keita「clipitnowに広告付けてね。exoclickのapi使って」）。**Son直接・実機検証済**。api.exoclick.com(v2・トークン=video-dl/.exoclick_token、login→Bearer)でサイト ClipItNow(idsite 1102818・Adult・承認済)のゾーンを取得し、実ゾーンを `/home/dev/projects/video-dl/index.html` に配線: ①**検索エリア上部バナー(#topad)= ExoClick Banner 300x250 ゾーン5974732(ClipItNow-search-banner)**。iframe隔離(sandboxed srcdoc・allow-same-origin無し=不透明オリジン)で親ページへの全画面/ポップアンダー波及を構造的に遮断。旧Adsterra上部枠は置換(Adsterraは結果直下#adbanner・ランキング上320x50に残置)。②**保存後の全画面広告= EXO_INTER_ZONE を5974730(ClipItNow-save-instream/In-Stream Video VAST)へ差し替え**(旧5973970)。黒動画/未フィル時のAdsterraフォールバック維持。配信=a.pemsrv.com/ad-provider.js。index.htmlはserver.pyがディスク直読み(mtime版管理)=再起動不要で即ライブ。**検証(clipitnow.net/Playwright mobile)**: 読込時に全画面広告なし(過去炎上の回帰なし)・ポップアップ0・遷移乗っ取りなし・ExoClick配信 ad-provider.js=200/s.pemsrv.com/v1/api.php(zone5974732広告req)=200。※既存Adsterra invoke.js(highperformanceformat.com)は403継続=本件と無関係の別課題。video-dlはローカルgit(remote無)にcommit 139b31d、配信は作業ツリー。バックアップ /tmp/clipitnow-index.bak.pre-exoclick。 | Son | 独立サービス[[MC-299]]。フィル率は新規Adultサイトで初期低め想定。ExoClick収益stats API連携(/api/adstats相当)は未実装=必要なら追加可 |
| MC-302 | ClipItNow クローズ画面を動画広告化（API新規ゾーン）＋ExoClick収益stats＋Adsterra403解消 | 中 | DONE（2026-07-15 Keita「クローズ画面のやつを動画広告に・既存でなくAPIで新規登録して」＋補足2点一任）。**Son直接・実機検証済**。①**保存後クローズ画面=動画広告**: ExoClick API(POST /v2/zones)で**新規In-Stream VASTゾーン ClipItNow-close-video(5974762)を作成**(旧outstream5974758はarchive・既存5974730/5973970不使用)。server.py `/api/closead`=5974762のVASTを**サーバ側取得→wrapper追跡→mp4(MediaFile)/Impression/ClickThrough抽出**(CORS回避・ユーザーへポップ/遷移なし)。index.html `injectInterVideo`=得たmp4を全画面`<video>`で自動再生(muted/skip後×/ミュート解除・Impression発火)、mp4無ければ白箱出さずバナーへフォールバック。実機(clipitnow.net/Playwright)=実mp4全画面再生(currentTime進行・360x480・StripChat等Adult creative)・ポップ0。②**補足(b)ExoClick収益stats**: server.py `exoclick_stats()`＋`/api/exostats`(v2 statistics/publisher/date・7日imp/clicks/video_views/revenue・5分キャッシュ)。実測263imp/3clicks/\$0.0039。③**補足(a)Adsterra403切り分け＋解消**: 原因=**旧ドメイン紐付けユニットがclipitnow.net Refererで403**(無Refererは200)。API(stats専用)では直せず、認可済みExoClickへ置換=結果直下#adbanner→**5974768(ClipItNow-result-banner・API作成)**、クローズ動画フォールバック/デスクトップ浮遊広告→exoBanner(5974768)、320x50枠は適合サイズ無く撤去。全iframe隔離。実機=読込時全画面広告なし・ポップ0・非広告JSエラー0。server.py変更につき video-dl.service 再起動済(index.htmlはディスク直読み)。commit 6a4c4ce。バックアップ /tmp/clipitnow-index.bak.pre-exoclick。**残(要Keitaダッシュボード)**: Adsterraを再併用するなら clipitnow.net をAdsterra管理画面でサイト登録し新ユニット発行が必要。 | Son | 独立サービス[[MC-301]][[MC-299]]。動画在庫はwrapper先に実在(video.saawsedge等)。フィルは新規Adultサイトで初期低め |
| MC-303 | ClipItNow アドブロック解除依頼を解除まで出し続ける（nag化） | 低 | DONE（2026-07-15 Keita「アドブロッカーを解除しないと延々に出続けるようにして」）。**Son直接・実機検証済**。index.html: ①1セッション1回の表示ガード(sessionStorage 'abmsg')撤去 ②アドブロッカー有効な間、閉じても**約2.5秒ごとに再表示するnagループ**追加(`window.__abRecheck`=bait再判定を公開し随時チェック) ③解除を検知したら自動でモーダルを閉じnag停止(clearInterval) ④計測ev('adblock_gate')は再表示スパム防止で1回だけ。ダウンロード自体は従来どおり止めない(Keita決定A維持)。実機(clipitnow.net/Playwright・アドブロッカー模擬=広告class隠すCSS注入): 表示→閉じる(消える)→約2.5秒で再表示→解除で自動停止 を確認。index.htmlはディスク直読みで即ライブ。commit 1710997。 | Son | 独立サービス[[MC-299]]。検知は bait(cosmeticフィルタ)＋netProbe の OR |
| MC-304 | ClipItNow クローズ画面動画が白い空箱に落ちる不具合を修正 | 中 | DONE（2026-07-15 Keita「動画広告がうまく出ない」＋白い空箱スクショ）。**Son直接・実機検証済**。真因=`/api/closead` は mp4 を安定返却(12/12・mp4はRange206/CORS対応で再生可)だが、client `injectInterVideo` が**5秒で未再生ならバナーへフォールバック**していたため、実モバイル回線で2.3MB動画のバッファが5秒に間に合わず**未フィルの ExoClickバナー枠(CSS白背景)=白い空箱**に落ちていた。加えてiOSミュート自動再生に muted 属性未設定。修正(index.html)=①動画URLがある限り has-video(黒背景)維持で常に動画表示・バナーへ落とさない ②5秒フォールバックのタイマー撤去(バッファ中も黒で待つ) ③iOS向けに muted/autoplay/playsinline を属性付与＋defaultMuted＋loadeddata/canplayでplay()再試行 ④読込中『広告を読み込み中…』黒背景中央表示・error時『まもなくダウンロードが始まります』で白箱化防止・Impressionは再生開始で1回。実機(Playwright)=通常は動画再生(currentTime進行・白箱なし)、mp4を7秒遅延の低速模擬でも黒背景＋読込中表示のまま(fallbackIframe無し)。ディスク直読みで即ライブ。commit 463b3de。 | Son | [[MC-302]]。フィル自体はサーバ側で担保済＝残課題は初期フィル率のみ |
| MC-305 | ClipItNow 空きスペースのExoClickバナー描画修正＋未フィル枠折り畳み＋動画15秒 | 中 | DONE（2026-07-15 Keita「空いてるスペースにexoclickの広告入れられる?／動画広告15秒にしたら収入増える?」＋空白スクショ）。**Son直接・実機検証済**。**Q1**: 空きスペースの真因=**sandbox iframe(不透明オリジン)が ad-provider の creative 描画を妨げていた**(sandbox無しなら充填を実証)。→ins をページ内に**直接 serve する方式(exoServe)**へ変更(#topad=5974732 読込時／#adbanner=5974768 結果表示時／デスクトップ浮遊枠)。ad-provider.js は読込時ロードだが interstitial ins が読込時に無く自動全画面は起きない(実機 fullscreenOnLoad=false・popup0・遷移乗っ取り無)。ただし**新規Adultサイトのバナー実フィルは現状低い**(video/VASTは100%・displayは薄い)ため、**未フィル枠は約7秒後に display:none で畳み空白を残さない**(クロスオリジンでも iframe 寸法で描画有無を判定)。**Q2**: クローズ×までの秒数=実質視聴秒を10→15秒に、動画尺が短くても静止しないよう v.loop=true。収入は主にフィル/CPM次第・15秒は完視聴/視認時間増だが離脱リスクもあり /api/exostats で計測し調整可。実機(Playwright): 未フィル枠 display:none/height0(空白なし)・countdown15・video loop再生・読込時全画面なし・popup0。ディスク直読み即ライブ。commit fa64454。 | Son | [[MC-302]][[MC-304]]。バナーフィルはサイト成熟で改善見込み・当面の主収益は動画 |
| MC-306 | ClipItNow 上の空きスペースを常時ループの動画広告に | 中 | DONE（2026-07-15 Keita「動画の方がいい／動画の最初が消えるのでずっと出たままに」）。**Son直接・実機検証済**。display バナーは新規サイトでフィル薄く空白になりがちだったため、安定充填する動画に変更。①新規 In-Stream 動画ゾーン ClipItNow-top-video(5974898)を API 作成 ②server: `closead_vast(zone)` をゾーン指定に一般化＋`/api/vast?slot=close|top`(VAST_SLOTS 白リスト・任意ゾーン照会不可) ③client: #topad を `exoVideoSlot('top')` に変更＝VAST自前`<video>`を muted/autoplay/playsinline/**loop** で常時ループ再生(最初から繰り返し・畳まず出したまま)、『広告』ラベル＋クリックでLP＋Impression発火、動画取得不可時のみ display:none。#topad は 340x300 黒box(object-fit contain)。実機(Playwright): 上枠に動画 loop 再生(360x480・ラベル有)・block維持・読込時全画面なし・popup0。commit 0550ab0。 | Son | [[MC-305]][[MC-302]]。結果枠#adbanner/浮遊枠は当面バナー(未フィルは畳む)、必要なら動画化可 |
| MC-307 | ClipItNow 自己除外(clip_optout)を funnelイベントにも適用(除外漏れ修正) | 低 | DONE（2026-07-15 Keita「俺のアクセスやダウンロードは除外して」）。**Son直接・実機検証済**。既存の `/?mute=1`(clip_optout=1 Cookie・2年)は PV(log_pageview)と DL(log_download)を除外済みだったが、`/api/event`(funnel: analyze/quality/download_*/save_download)が除外漏れで「状況」タブに自分の操作が残っていた。→ server.py の /api/event で `is_optout` なら記録せず200を返すよう修正。検証: clip_optout=1 の PV/イベントは analytics.jsonl/events.jsonl に記録されず(756/353維持)、Cookie無しは+1記録を確認。テスト行は掃除済。**Keita側の操作**: 除外は端末別Cookieなので、使う各端末/ブラウザで `https://clipitnow.net/?mute=1` を一度開く(確認HTML表示・2年有効)。video-dl.service再起動済。commit d454614。 | Son | 独立サービス[[MC-299]]。過去ログの遡及削除は未実施(vidは匿名乱数で本人特定不可・希望あればログreset可) |
| MC-308 | ClipItNow 広告を動画に統一(畳まず流し続ける)＋高収益配置を追加 | 中 | DONE（2026-07-15 Keita「左下のやつ等が数秒で消えて非表示になる／しっかり広告を流して・もっと増やして収益最大化の配置を」）。**Son直接・実機検証済**。原因=左下のデスクトップ浮遊広告(floatad)や結果枠は exoServe バナーで、新規サイトは display在庫が薄く**7秒後に未フィルで畳まれ消えていた**。→在庫が安定する動画(VAST)に統一。`exoVideoSlot` 強化=遅延ロード(画面に近づいた枠だけ /api/vast 取得)＋可視制御(画面外pause/戻れば再生)で複数配置でも同時再生/負荷を抑制、動画がある限り**畳まず loop で流し続ける**。配置(全て動画): トップ(top・既存)/結果カード下(#adbanner→inpage)/デスクトップ左下浮遊(floatad→inpage・=消えてた枠)/**変換待ち中(#runAdSlot→wait・captive新規追加)**/保存後クローズ(close・既存)。server VAST_SLOTS に inpage(5975432)/wait(5975434) 追加(API作成)。実機(Playwright): top/inpage/floatad いずれも video 再生・畳まれない・real-popup0・読込時全画面なし。※検証中の"about:blank popup"はテスト側(newPage前にpageリスナ登録)の誤検出だったと確認。commit d864b04。 | Son | [[MC-306]][[MC-302]]。動画は VAST 100%充填で信頼・displayバナーは薄い。結果/浮遊はinpageゾーン共用(VAST独立取得で各枠フィル) |
| MC-309 | 仕事「状況解析」タブ: ExoClick収益を詳しく表示・Adsterra撤去・タブ改名 | 中 | DONE（2026-07-15 Keita「状況タブ更新: ①exoClickをもっと表示 ②AdStella(Adsterra)はもう広告無いので表示消してexoClickに集約 ③『今の状況』を『状況解析』みたいな名前に」）。**Son直接・実機検証済**。web/src/views/Work.tsx: ①`ExoRevenuePanel`新設＝`/api/exostats`(Son追加済)から 収益/表示/クリック/**動画視聴**/CTR＋本日＋**日別7日テーブル**をアクセント枠で表示(旧Adsterraは4指標のみ→拡充) ②`AdRevenuePanel`(Adsterra)＋adstats fetch＋AdStats型を撤去し Exo に置換、「広告（Adsterra）」節→「広告（ExoClick）」(現配置/撤去理由を明記)、脚注・残タスクからAdsterra削除 ③タブ label '状況'→'状況解析'。tsc0・`npm run build`(dashboard+postbuild site)green・dist反映。実機(apollomansion.com/work?tab=clipitnow-status/Playwright): タブ「状況解析」表示・ExoClickパネル(収益$0.0121/CTR1.46%/日別表)描画・console error0。バックアップ /tmp/work-exostats-backup-20260715/Work.tsx。**git**: 共有ツリー(他agent WIP混在)につき配信はdist反映で完結・commitは[[MC-290]]の整理に相乗り保留。 | Son | [[MC-301]] ExoClick stats API=MC-302。静的dist配信=backend再起動不要 |
| MC-310 | ターミナル「状況」: Ops出力にSub(Son)履歴が表示される取り違えを修正 | 高 | DONE（2026-07-16 Keita「opsの出力を見るにsubのほうの履歴が表示されてる」＋スクショ）。**Son直接・実機検証済**。真因=**tmux の前方一致ターゲット**。端末4(Ops)=session `openclaw`、端末5(Sub)=`openclaw-son`。server が `capture-pane -t 'openclaw'`(bare)を使っていたため tmux3.4 が `openclaw` を `openclaw-son` に解決し、Ops の出力に Sub(Son)の履歴が出ていた（send-keys も同targetで誤送信リスク）。実測: `-t openclaw`→pane pid=614654(Son)／`-t '=openclaw:'`→977945(Ops・正)。修正(server/src/terminalControl.ts＋terminalUpload.ts): `sessTarget(t)=='=name'`(has-session)・`paneTarget(t)=='=name:'`(capture-pane/send-keys)ヘルパを追加し完全一致化。tsc0。mission-control.service(tsx src直実行)を再起動して反映。検証(live API): `/api/terminal/output?terminal=4`→agent main(Masayoshi)・`terminal=5`→agent son(Son)＝正しく分離。バックアップ /tmp/terminal-opsfix-backup-20260716/。**git**: 共有ツリーにつき配信は再起動で完結・commitは[[MC-290]]整理に相乗り保留。 | Son | [[apollo-terminal-session-mapping]]。'main'/'spare'は前方衝突無し。iframe(ttyd)は別系統で影響外 |
| MC-311 | ダッシュボードに ClipItNow PDCA 可視化タブを追加 | 中 | DONE（2026-07-19 Keita「ClipItNowのPDCA…ダッシュボードのところで可視化してほしい／実装進捗ではなく」）。**Son直接・実機検証済**。新規 `web/src/views/Pdca.tsx` を作成し、ダッシュボード(/)のタブに「PDCA」を追加（DashboardLayout.tsx の DASH_TABS＋App.tsx に /pdca ルート/lazy import）。**Check(現状の数値)** は clipitnow.net の公開API（/api/stats・/api/exostats、CORS `*`）からライブ取得＝訪問PV/実UU/DL/ExoClick収益/24hファネル。**Plan/Do/Act** は現サイクルの内容を静的に記載（回すたびに更新）。build(tsc0＋vite＋postbuildでsite再build)成功。apollomansion.com/pdca を Playwright 実描画確認＝全KPI/2×2グリッド表示・console error 0・実データ反映（PV1264/実UU3/DL15/収益$0.0164）。フロントは dist 静的配信ゆえ backend 再起動不要。**git**: 共有ツリーにつき commit は[[MC-290]]整理に相乗り保留。 | Son | [[clipitnow-exoclick-integration]]。UUが週3=大半ボット/直接流入という MC-292 の所見をそのまま可視化 |
| MC-312 | ClipItNow PDCA を回す専任エージェント「PD-CA」（自動ループ＋Apolloで楽しく可視化） | 中 | DONE 🔒[Keita] | Son |
| MC-313 | Apollo 継続改善（不要物削減/UX導線/トークン効率の網羅提案＋順次最適化） | 中 | IN_PROGRESS（2026-07-19 Keita「Apollo自体の改修/改善を提案してどんどん最適化」）。3軸を並行分析エージェントで網羅監査→**改善バックログ** `obsidian-vault/30-Projects/apollo/improvement-backlog.md` に集約(🟢即可/🟡要一声/🔴承認必須)。主要所見: ③トークン=**常駐端末がOpus既定(最大の隠れコスト・要承認でSonnet化)**・news-briefingリトライ/FC削減・feedback/night-patrolのHaiku化・moodスロットル10分。①削減=未使用アバター約14M・pdfkit依存・cron *.bak・デッドview。②UX=「仕事」の3事業寄せ集め解体/ClipItNow重複統合・実装進捗を最上段から降格・タブ/バッジの共通化。**着手済(🟢)**: 今セッション作の未使用v3/無印アバター26ファイル(5.2M)を _avatar-archive へ退避。残りはPDCA式に順次消化(🔴は決裁)。【2026-08-01 14:25 Son/MC-354ループ】🟢消化: 未使用依存pdfkit/@types/pdfkit削除（commit 7f24fd1・tsc green・バックログDONE化）。次候補=moodsスロットル(config.ts他者WIP解消待ち)/タブ共通化【2026-08-01 18:30 Son/MC-354ループ】バックログ🟢「空状態の補強」消化＝Pdca.tsx の live=false 注記を情報ボックス化＋ExoClick未取得時のサブ文言を明確化（commit e4f3314・web tsc/build green・配信バンドル一致確認＝再起動不要）。残🟢=タブ共通化(TabStrip/CountBadge)・cron-scripts退避・後方互換ルート確認等。moodsスロットルは config.ts 他者WIP解消待ち継続【2026-08-01 19:25 Son/MC-354ループ】バックログ🟢「cron-scripts退避」消化＝*.bak/*.staged/*proposed 23ファイルを /home/dev/cron-scripts/_bak-archive-20260801/ へ退避（削除なし・当日作成の session-cleanup.sh.bak.cowork-guard-20260801 は他者ロールバック点の可能性につき残置・cron参照は実体.shのみで影響なし）。残🟢=タブ共通化・後方互換ルート確認【2026-08-01 20:30 Son/MC-354ループ】バックログ🟢「後方互換ルート行き止まり確認」消化＝GlobalSearchはルートを候補に出さず問題なし。副産物で実バグ修正: agent検索結果の遷移先/agents(→/へリダイレクトで意図喪失)を/agents-liveへ（commit 438cd0d・tsc/build green・配信確認済）【2026-08-01 23:20 Son/MC-354ループ】🟢「moodsスロットル5→10分」消化＝config.ts既定値変更（commit 0cf9620・tsc green・dev一声→backend再起動→env上書き無しで10分既定が有効・service active/API 200確認）。残🟢=タブ共通化のみ | Son | [[son-owns-board-reconciliation]] | [[clipitnow-exoclick-integration]] [[son-owns-board-reconciliation]] [[cxo-agent-shared-tree-concurrency]] [[cxo-agent-prod-restart-loads-worktree]] |
| MC-314 | Apollo に claude.ai を「ブラウザ埋め込み」表示（noVNCストリーミング） | 中 | CANCELLED（機能撤去済 2026-07-19。以下は実装～撤去の経緯）DONE当時（2026-07-19 Keita「claudeのサイトをアポロに埋め込みで表示できる？」→iframe不可(claude.ai `x-frame-options: SAMEORIGIN`)を実確認→方式Bを選択「bで」）。**Son直接・実機検証済**。**方式B=ストリーミングブラウザ**: Xvfb仮想ディスプレイ上で playwright同梱Chromium(非snap)を kiosk 起動し claude.ai を表示→x11vnc(localhost束縛)→websockify/noVNC で配信→Apollo が terminalProxy と同型の reverse proxy(`/claude-browser`・WSはupgradeで認証)で中継→新ビュー `ClaudeBrowser.tsx`(React route `/claude-browser-view`)が iframe 表示。docker/podman無し・passwordless sudo有り。Keitaが埋め込み画面内で一度ログインすれば永続プロファイルでセッション維持。**実装(全て実機検証済)**: 裏方=systemd 5ユニット `apollo-claude-{xvfb,openbox,chromium,x11vnc,novnc}.service`(依存順・全て localhost 束縛・enable済で再起動後も自動復帰)。Xvfb:99(1600x900)→openbox→playwright Chromium kiosk(profile=~/.openclaw/claude-browser/profile)→x11vnc(127.0.0.1:5901・vncpass付)→websockify/noVNC(127.0.0.1:6081)。Apollo=`server/src/claudeBrowserProxy.ts`(terminalProxy 同型・HTTPは認証MW後ろに mount `/claude-browser`・WSは server.on('upgrade')→attachClaudeBrowserUpgrade で isRequestAuthorized ゲート)＋`GET /api/claude-browser/config`(VNCパスをバンドルに焼かず認証経由で配布)。フロント=`web/src/views/ClaudeBrowser.tsx`(noVNC vnc.html を同一オリジン iframe・autoconnect/resize=scale/reconnect)＋App.tsx にナビ「Claude」＋route `/claude-browser-view`。**検証**: tsc0(server/web とも)＋vite build＋site再build成功。mission-control再起動。curl: config/vnc.html/noVNCアセット=proxy越し200・未認証=401(HTTP/WS両ゲート)。**OpenClawブラウザ実描画**(apollomansion.com/claude-browser-view)=サイドバー隣に claude.ai ログイン画面が noVNC ストリーミング表示されるのを確認。**2026-07-19 追記(スマホ最適化)**: Keita「一応ログインできたがスマホだと使いづらい」。原因=デスクトップ幅(1600x900)を縮小表示で文字が小さくタップ困難。対策=1ストリーム共有ゆえ**配信元ブラウザ自体をスマホ縦(520x950)で描画**→claude.ai がモバイルレイアウト(大きな入力欄/ツールバー)になる。Xvfb screen と launch-chromium.sh の window-size を 520x950 へ変更・チェーン再起動。scrot 実測で入力欄「Type / for skills」・+/chat/model/mic/waveform ツールバーが全て収まりモバイル表示になるのを確認。デスクトップでは縦長の列表示(実用可)。**2026-07-19 追記2(サイドメニュー削除・スタック休止)**: Keita「このメニューはサイドメニューを消していい。クロードのやつは多分使わない」。App.tsx NAV から「Claude」項目を削除(route /claude-browser-view は後方互換で残置)＋未使用の SparkIcon import 除去。tsc0/build/site 再build 成功(フロント静的配信ゆえ backend 再起動不要)。実画面でサイドメニューから消えたのを確認。**裏方スタック(apollo-claude-{xvfb,openbox,chromium,x11vnc,novnc})は disable --now で停止・自動起動オフ**(ポート5901/6081解放・リソース節約)。ユニット定義/コード/route/ログイン済みプロファイルは残置＝再 enable で即復活可(再ログイン不要)。**2026-07-19 追記3(完全撤去・CANCELLED)**: Keita「それも停止していい」→機能ごと撤去。①コード全戻し(index.ts の import/`/claude-browser` mount/`/api/claude-browser/config`/upgrade フック除去、`claudeBrowserProxy.ts`・`ClaudeBrowser.tsx` 削除、App.tsx の lazy import/route 除去)。tsc0/build/site再build/mission-control再起動。curl で config/JSONは消滅(SPA フォールバックHTMLのみ)を確認。②systemd 5ユニット削除+daemon-reload。③作業ディレクトリ ~/.openclaw/claude-browser(profile/scripts/vncpass/screenshots)削除。④追加パッケージ撤去(x11vnc/openbox/x11-xserver-utils)。websockify CLI は python3-websockify 由来で不活性残置・Xvfb/scrot は既存につき保持。numpy/PIL等の共有依存を巻き込む autoremove は他ツール保護のため不実施。**残骸なし・稼働ゼロ**。 | Son | [[openclaw-browser-container-fix]]（chrome実体=ms-playwright/chromium-1223）。ttyd/terminalProxy パターン踏襲 |
| MC-315 | ターミナル分割で「2×2なのに3枚しか映らない」を修正（重複割当→空白ペイン） | 中 | DONE（2026-07-20 Keita「分割画面3つしか見えてない」＋スクショ）。**Son直接・実機検証済**。真因=ペインのセレクタで既に別ペインに映っている端末を選ぶと割当が重複し、描画（各端末は最初の一致スロットのみ mount・`Terminal.tsx` findIndex）で相手ペインが**無表示の空白**になる。修正3点: ①セレクタで既存端末を選んだら**スワップ**（相手ペインへ元の端末を渡す）②localStorage `paneAssign.v2` の重複を読み込み時に未使用idへ修復 ③保険としてどの端末も映らないスロットに端末セレクタ付きプレースホルダを表示（見えないペインを作らない）。tsc0+vite build。**実機検証(Playwright/本番apollomansion.com)**: 重複[5,5,4,1]注入→4枚表示・既定grid2x2→4枚・セレクタでOps選択→スワップ後も4枚＝全PASS（/tmp/split-case*.png）。フロント静的配信のみ＝backend再起動不要。**git**: Terminal.tsx に他WIP（出力モーダルANSIハイライト等・未検証）が混在するためコミット保留（配信は作業ツリーdistから・MC-300と同様） | Son | ユーザー側は再読み込みだけで復旧（読み込み時修復が効く） |
| MC-316 | PDF.ai(pdfdotai.com) 公開終了・この箱から完全撤去 | 中 | DONE（2026-07-20 Keita「pdf.ai.com は外して大丈夫だよ。もう使わないので」）。**Son実施**。棚卸し結果: cxo-agent 側のサイトコード（web/site・dist/site・publicSite ゲート・PdfEditor.tsx・App.tsx 参照）は既にツリーから撤去済み・cron 無し。残存は専用トンネル `cloudflared-pdfdotai.service`（既に停止・無効化済＝530 の原因）と `/etc/cloudflared/pdfdotai.env` のみ → 両ファイルを `/home/dev/.trash/pdfdotai-decommission-20260720/` へ退避し systemd から削除（daemon-reload 済・unit not found 確認）。**Cloudflare urano2 アカウント側**（トンネル定義 pdfdotai-son・DNS・ドメイン契約・AdSense）はクラウド側に残存＝解約/削除は Keita のアカウント操作が必要。記憶 pdfai-public-launch を「終了」へ更新・pdf-site-build-order-dist-wipe は退役 | Son | apollomansion 側トンネル・ダッシュボードは無関係・無傷 |
| MC-317 | Apollo画面整理: 使用量タブ削除・収益をダッシュタブ化・エージェント/実装進捗をタスクボードへ統合・かんばん可視化・RAG非表示 | 高 | DONE（2026-07-20 Keita指示）。**Son直接・実機検証済**。ダッシュタブ=Claude/ニュース/収益/PDCA（使用量・エージェント・会話ログ整理、収益をタブ化）。タスクボードに「エージェント」「実装進捗」タブを統合（TasksTabs・旧URL /agents-live /progress は互換で該当タブへ着地）。タスクボード上部に NowOverview（TODO/進行中/ブロック/レビューの件数ピル＋進行中を担当者別チップで可視化・クリックで詳細）。RAG はタブ非表示のみ（コード・データ残置・/notebooks 直URLは到達可）。**実機検証(Playwright/本番)**: dash収益タブ・使用量なし・/tasks 3タブ＋NowOverview・/agents-live→エージェントタブ・/progress→実装進捗タブ・RAGタブ非表示・/notebooks 直URL可＝全PASS | Son | 茶事は現状維持 |
| MC-318 | 育児ガイド全面更新: 手続き完了化・お世話の基本/手続きサービス非表示・1日のリズム更新・相談メモ最新化 | 高 | DONE（2026-07-20 Keita指示）。**Son直接・実機検証済**。行政手続き（出生届・児童手当・健康保険・乳幼児医療費助成・出産育児一時金・出産手当金等）全6件に done フラグ→「✓完了」バッジ表示（緊急度バッジと差し替え）。「お世話の基本」「文京区独自の手続き・サービス」はフラグで非表示（コード・データ残置・trueで復帰可）。「1日のリズム」を生後1か月〜1か月半（生後40日前後）の目安に更新（授乳3〜4h・睡眠14〜17h・起きている時間・外気浴を追加）。相談メモはサーバ自動パイプラインが最新チャット（7/14）まで処理済み＝最新state確認。育児チャットは温存。**実機検証(Playwright/本番)**: 完了バッジ・2セクション非表示・リズム更新・相談メモ表示＝全PASS | Son | 育児チャットは残す |
| MC-319 | 仕事ページ整理: 動画DL/状況解析/概要タブ削除・単語帳とナレッジは残す | 中 | DONE（2026-07-20 Keita指示）。**Son直接・実機検証済**。Work.tsx から 概要（WorkOverview）・動画DL（WorkVideoDlTab）・状況解析（WorkClipItNowStatusTab＋ExoClick統計パネル一式）を削除（計約660行）。残タブ=ナレッジ＋単語帳（既定=ナレッジ）。壁打ちは右下フローティングチャット（クレア）で継続。未使用import整理・tsc(noUnusedLocals)通過。**実機検証(Playwright/本番)**: タブ=ナレッジ/単語帳のみ・壁打ちFAB残存＝PASS | Son | ClipItNow統計はデータ・API側は無傷（UIのみ撤去） |
| MC-320 | 開発アイデア生成をワクワクする「ぶっ飛んだ」方向へプロンプト刷新 | 中 | DONE（2026-07-20 Keita指示）。**Son直接・実機検証済**。IDEA_SEEDS を実務・節約系から遊び心・意外性重視の12切り口（シミュレーター/斜め上診断/本音が聞こえる装置/架空省庁ごっこ等）へ全面刷新、buildIdeaPrompt を「実用性より意外性・触った瞬間の驚き」基準に書き換え（無害性ガード付き）。server変更のため mission-control 再起動（dev チャンネルへ事前連絡・server全体 tsc 通過確認済）。**実機検証(本番API)**: POST /api/dev/idea→job done、出力例「感情→神話ジェネレーター（3つの感情を入れると太古の神話に変換）」＝方向性◎ | Son | server変更＝backend再起動済 |
| MC-327 | ClipItNow PDCA: Doの具体化・即時実行＋レポートのPDCAタブ一覧化＋実装進捗ライブ可視化 | 高 | DONE（2026-07-20 Keita「Doを明確にして実行に移して。レポートはPDCAタブから見られるように。今何をしているかを実装進捗で確認できるように」）。**Son直接・実機検証済（Do実行は継続中）**。①Do具体化: Check(検索流入0/UU7=0)とActを踏まえ VD-09(新規ガイドLP6本)/VD-10(内部リンク強化)/VD-11(FAQ拡充) を video-dl 台帳へ起票し、subagent へ即時ディスパッチ（実行中）。②レポート一覧: GET /api/clipitnow/pdca/reports(+/:name) を backend に追加し、PDCAタブ末尾に「レポート」節（Check/Do種別バッジ・クリックでmarkdown展開）を実装。③可視化: Do実行を subagent 化＝/api/agents 経由で実装進捗タブにライブ表示（実機確認: 稼働中に「ClipItNow PDCA第3巡 Do実行」フィード表示）。夜間cron clipitnow-pdca-do.sh にも subagent委譲の指示を追記し恒常化。backend build+restart 済（共有ツリーWIPは巻き込みあり・ビルドは全体パス）。**Do完了(2026-07-20)**: VD-09=新規LP6本(douyin/weibo/kuaishou/threads/kick/iwara)全て実機200・JSON-LD確認・sitemap52URL・IndexNow52件HTTP200／VD-10=ハブ8本に関連ガイド節＋逆リンク／VD-11=主要10本にQ&A各3問(FAQPage整合)。video-dlローカルcommit e987400 [pdca-c3]。Do結果レポートは PDCAタブ「レポート」節の pdca-do-2026-07-20-son.md | Son | cxo-agent + video-dl。mission-control再起動済 2026-07-20 |
| MC-328 | Apolloターミナルの直接コピー対応＋出力モーダルのエスケープ化け修正 | 高 | DONE（2026-07-20 Keita「コピーできない。ターミナルを直でコピーできるようにしてほしい」）。**Son直接・実機検証済**。根因2つ: ①tmux mouse on のためドラッグ選択が tmux copy-mode 止まり＝ブラウザclipboardに入らない → ~/.tmux.conf に set-clipboard on＋Ms override を追加し、terminalProxy 注入スクリプト(COPY_FIX)に OSC52 ハンドラ(term.parser.registerOscHandler(52))＋copy-on-select(onSelectionChange)＋「コピーしました」トーストを実装＝**ドラッグして離すだけでコピー完了**。②出力モーダルの「]8;;URL」ゴミ → OpenClaw TUI の OSC8 ハイパーリンクを parseUserHighlight が素通し → stripNonSgrEscapes で OSC/非SGR CSI/単独ESC を除去。**実機検証(Playwright/本番)**: ドラッグ→clipboard=画面テキスト一致 PASS・トースト表示 PASS・OSC52注入テスト PASS・出力モーダル ]8;;=0件/生ESC=0件 PASS | Son | server変更＝backend再起動済。Terminal.tsx は他者WIP同居のためコミット保留（buildで配信済）・terminalProxy.ts はコミット |
| MC-329 | 収益の日本円換算表示（収益コックピット＋PDCAタブ） | 中 | DONE（2026-07-20 Keita「収益は日本円に換算して表示してほしい」）。**Son直接・実機検証済**。revenueRouter に USD/JPY レート取得を追加（open.er-api.com・キー不要・12hメモリキャッシュ・失敗時は旧レート延命→固定150フォールバック）。/api/revenue/summary に usdJpy を同梱＋単体 GET /api/revenue/usdjpy 新設。収益ページ（Revenue.tsx）は全カード・スパークラインtooltipを円主表示（USDはsub/注記に残す）・換算レート注記追加。PDCAタブの ExoClick収益カードも円表示。**実機検証(本番)**: レート $1=¥162.49 取得・収益ページ 本日¥1.80/7日¥13.7/ExoClick¥10.6・PDCAタブ ¥10.6（本日¥1.64）表示 PASS | Son | server変更＝backend再起動済 |
| MC-330 | Apolloターミナル: スマホからの履歴コピー対応（長押し選択コピー＋出力モーダル全文コピー） | 高 | DONE（2026-07-20 Keita「スマホからだと、ターミナルの履歴コピーできないんだけど」）。**Son直接・実機検証済**。①TAP_FIX(terminalProxy注入)に長押し選択を追加: 450ms長押し（バイブ30ms）→なぞりでtmux選択（SGR press/32-move/release送出）→指を離すと tmux copy→OSC52→MC-328のCOPY_FIX経由でclipboard＋「コピーしました」トースト。スワイプスクロール/タップ送出は従来通り。実装中の発見: タッチは .xterm-link-layer に命中し .xterm-viewport の子ではない＝リスナーは共通祖先 t.element に付ける必要あり（従来リスナーも同修正）。②出力モーダル(MC-123)に「全文コピー」ボタン追加（クリックで全2000行をclipboardへ・「コピーしました」フィードバック）。**実機検証(本番/Playwright)**: モバイルCDPタッチで長押し→ドラッグ→離す=clipboardに画面テキスト PASS×2・回帰=スワイプ(wheelのみ/press 0) PASS・タップ(press+release) PASS・全文コピー(31,938字) PASS・デスクトップ実速度ドラッグコピー PASS（高速同一行なぞりE2Eの失敗は空選択によるテストフレークと切り分け済・回帰なし） | Son | server変更＝backend再起動済。Terminal.tsx は他者WIP同居のためコミット保留（buildで配信済）・terminalProxy.ts はコミット。**要ページ再読込** |
| MC-326 | ClipItNow: 保存2タップ制（1回目=ExoClick Direct Link広告→2回目=DL） | 高 | DONE（2026-07-20 Keita「保存の1回目はExoClick広告を強制クリック→2回目でDL。収益性シミュレーションも」）。**Son直接・実機検証済**。ExoClick APIで Popunder ゾーン **5980966 "ClipItNow-save-directlink"** を新規作成し、Direct Link URL=`s.pemsrv.com/v1/link.php?idzone=5980966&type=8`（広告主LPへ302・実測で実広告フィル確認）。保存ボタン1回目タップ=広告を新規タブで開きラベル「もう一度タップでダウンロード」（i18n既存キー5言語）・2回目タップ=即DL開始。同日Keita「併用したほうがいい？」→**併用モードへ**: 2回目タップ=全画面VAST動画(15秒→×)→×で実DL（Direct Link訪問課金＋動画再生課金を1DLで両取り・実機検証済=1回目広告タブ→2回目全画面動画→×でDL開始）。動画はCPM型＝再生だけで課金・クリック不要（実測: 直リンクeCPV$2.1/1000、動画全枠7日$0.02）。同日Keita「プレビューからダウンロードできちゃうのは良くない・埋め込みに」→プレビュー<video>（解析時#thvideo/完了後pvPlayer）に controlsList=nodownload＋disableremoteplayback＋右クリック保存抑止＝ネイティブメニューDLでのゲート迂回を防止（実機検証済）。同日Keita「俺のテスト操作は収益からも除外して」→オーナー端末(clip_optout=1 / ?mute=1)は**広告も全停止**: 動画枠非表示・Direct Linkタブ開かず・全画面VASTスキップ即DL（実機検証: optout=広告リクエスト0/タブ0/即DL、一般=広告フル動作）。ローカル統計は元からoptoutで未記録＝削除不要。ExoClick側の既発生分(≈$0.01)は遡及除去不可。同日Keita「広告は確認用に残して」→optoutでも広告は全表示に戻し、Impression/Clickの課金ピクセルだけ抑止する方式へ（実機検証: optout=広告表示・ピクセル0件/一般=ピクセル8件発火）。Direct Linkの訪問のみ先方計測のため除外不可(1タップ¥0.3程度)。ついでにPDCA VD-04回帰(topad初期display:none→IntersectionObserver不発で上部広告が全ユーザー非表示)を修正。計測=save_gate_ad イベント追加（preview_play/scene_tap も whitelist 追補）。**実機検証(本番/headless)**: 1回目タップ→広告主タブ1枚(実広告)＋ラベル変化＋DL未開始＋全画面広告出ず→2回目タップ→mov_bbb.mp4 DL開始・広告タブ増えず・JSエラー0＝全PASS。収益試算はKeitaへ報告（DL数×直リンクeCPV$1〜4/1000が上乗せ）。同日Keita「残骸をなくして・サムネ未表示も確認」→**残骸除去＋サムネ恒久化**: Son検証DL(mov_bbb×2)を downloads/events/analytics.jsonl から除去（バックアップ cleanup-backup-20260720_1610/）・SAMPLE_URL_MARKERSにw3schools/bigbuckbunny追加でテストURLは以後ランキング/統計に常時不記録。サムネ調査: シード51件中9件がCDN署名URL失効(410/472)＝未表示の原因。全51件を thumbs/ にローカルキャッシュし /thumbs/ 配信ルート追加（失効分はyt-dlp再取得、取得不能1件は動画フレーム切出し）。同日Keita「シーン一覧の位置が狭すぎ・左の動画の下へ」→デスクトップ(≥1024px)はシーン一覧を左カラム=動画直下の広い場所へ移動（leftcolラッパー化・サムネ無し動画は枠可視化時に移設・モバイルは従来通り画質ボタン下）。実機検証: PC=動画直下536px幅gap14px同列・サムネ無し経路も同列・モバイル新規セッション=画質ボタン下・JSエラー0 | Son | video-dl。EV_TYPES変更＝video-dl.service再起動済 |
| MC-325 | ClipItNow: 解析直後（DL前）のプレビュー再生＋シーンサムネイル表示 | 高 | DONE（2026-07-20 Keita「dl-x.com のように埋め込み再生・シーンを画像で」→同日追加指示「保存前・画質選択の前に欲しい」で前倒し改修）。**Son直接・実機検証済**。最終形＝**URL解析した時点**で: サーバが裏で低画質版(≤360p)を yt-dlp 自身で取得（抽出URLの直再生はxhcdn等でセッション束縛403のため不採用）→取得完了しだいサムネイル上に「プレビュー再生」ボタンが出現しタップでその場埋め込み再生（/api/pstream・Range 206）、並行して ffmpeg でシーン8枚（短尺は自動調整）を生成し画質ボタン直下へ順次表示（/api/pscenes ポーリング・タップでシーク再生）。同一URL再解析はプレビュー再利用で二重DLなし・TTL30分でGC。完了後の旧プレビュー/シーン（commit 3c0845e）はDL前版がある場合は重複表示しない。5言語i18n。**実機検証(本番clipitnow.net/headless)**: 解析のみ→再生ボタン出現→シーン5枚表示(#dls直後)→DL前埋め込み再生paused=false→シーンタップでt=7.4へシーク→その後DLしても重複0→再解析でリセット・JSエラー0＝全PASS。xhamster実URL(552s)も低画質取得→scenes8枚→pstream206までAPI全PASS（旧方式で403だったケース解消） | Son | video-dl(:4319, clipitnow.net)。server.py変更＝video-dl.service再起動済 |
| MC-324 | サイドメニュー: 一定幅未満でアイコンだけのコンパクト表示 | 中 | DONE（2026-07-20 Keita「ある程度狭くなったら、文字はいらない。アイコンだけでいいよ」）。**Son直接・実機検証済**。幅180px未満でコンパクトモード: ナビは文字・バッジ・並べ替えハンドルを消してアイコンのみ中央寄せ（title属性でホバー時にラベル表示・未読バッジはアイコン右肩の小丸に）。ヘッダはApolloロゴのみ縦並び、フッター（検索/設定/全画面/再読み込み）もアイコンのみ。判定はResizeObserverでaside実幅を監視＝ドラッグ中もリアルタイム切替。最小幅を160→64pxへ拡大（アイコンレール成立幅）。**実機検証(Playwright/本番)**: 224(ラベルあり)→170へドラッグ=ラベル消滅・アイコン8個・title付与→最小64クランプ→リロード後64維持→260へ広げ=ラベル復活＝全PASS（/tmp/v-compact-64.png） | Son | MC-323のドラッグ幅調整の追補。フロント静的配信のみ＝backend再起動不要 |
| MC-323 | サイドメニュー幅を境目ドラッグで可変に（MC-322 の3択方式を置き換え） | 中 | DONE（2026-07-20 Keita「境目のところに行ったら自動的に幅を調整できる感じがいい」）。**Son直接・実機検証済**。サイドバー右端に幅8pxのリサイズハンドル（境目にカーソル→col-resizeカーソル・ホバーでアクセント色）。ドラッグ中はCSS変数を直接更新して即時反映、離した時点でlocalStorage保存（px単位・160〜440にクランプ・旧3択保存値はpxへ移行）。**ダブルクリックで標準幅(224px)に復帰**。設定モーダルの旧3択セクションは撤去。**実機検証(Playwright/本番)**: 初期224→ドラッグ340→180→最小クランプ160→リロード後160維持→ダブルクリックで224復帰・カーソルcol-resize・旧3択なし＝全PASS。**反応改良（同日 Keita「反応が悪すぎる」）**: setPointerCapture（高速ドラッグ・iframe上でも取りこぼし無し）＋ドラッグ中全画面オーバーレイ（選択/hover遮断・カーソル固定）＋rAF間引き＋当たり判定10px化＋ホバーでライン/グリップ表示。再検証: 1stepの高速ドラッグ420・ターミナル画面(iframe上)で260＝PASS | Son | フロント静的配信のみ＝backend再起動不要 |
| MC-322 | サイドメニュー幅を選択式で可変に（狭い/標準/広い）※MC-323の境目ドラッグ方式に置き換え済 | 中 | DONE（2026-07-20 Keita「サイドメニューの幅を選択して可変式に」）。**Son直接・実機検証済**。設定モーダルに「サイドメニューの幅」セクションを追加: 狭い176px/標準224px(従来)/広い296px の3択・選択した瞬間に反映・localStorage(`apollo_sidebar_width`)永続。実装は useFontSize と同型（lib/useSidebarWidth.ts・CSS変数 --sidebar-width を documentElement に適用・aside が参照）。モバイル表示は非対象（サイドバー自体がmd〜のみ）。**実機検証(Playwright/本番)**: 初期224→広い296→狭い176→リロード後176維持→標準224復帰＝全PASS（/tmp/v-sidebar.png） | Son | フロント静的配信のみ＝backend再起動不要 |
| MC-321 | Apollo全画面トグル（ブラウザUI非表示・Escで復帰）を確認・実装 | 中 | DONE（2026-07-20 Keita指示）。**Son直接・実機検証済**。Fullscreen API で実装: サイドバー左下（設定の下）とモバイルメニューに「全画面表示」ボタンを追加。クリックでブラウザのタブ・URLバーごと非表示、**Esc または再クリックで復帰**（Esc はブラウザ標準動作＝要望どおりのトグル）。fullscreenchange 監視でボタン表示も同期（全画面中は「全画面を解除（Esc）」）。ブラウザ仕様上、全画面への移行はクリック起点のみ許可（自動全画面は不可）。**実機検証(Playwright/本番)**: ボタン描画確認＝PASS | Son | Fullscreen API・Escは標準で復帰 |
| MC-325 | clipitnow.net 公開停止（530/error1033）復旧: 専用cloudflaredトンネル新設 | 高 | DONE（2026-07-20 インフラ緊急対応・事後起票。Keita方針=専用トンネル新設）。**林直接・実機検証済**。真因=MC-316 の pdfdotai 退役（Keita指示）で `cloudflared-pdfdotai.service` を停止した際、同トンネル(74498993…)に**相乗りしていた clipitnow.net が道連れで公開停止**（09:27〜10:41 JST 約74分）。Apollo PDCAタブの「指標を取得できませんでした」もこの巻き添え（ローカル:4319 video-dl 本体は無傷）。復旧=①CF API(urano2/.cf_clip_token)で専用トンネル `clipitnow`(1a9234c3-f691-4773-85cc-98f2fd4ec29b)を新規作成・remote config で clipitnow.net/www→http://localhost:4319 ②`/etc/systemd/system/cloudflared-clipitnow.service`＋`/etc/cloudflared/clipitnow.env`(token, 600) を新設し enable --now（再起動後も自動復帰）③DNS CNAME 2件(apex/www)を新トンネルへ PATCH。**実機検証**: トンネル接続4本(nrt)登録・https://clipitnow.net/ 302(正常リダイレクト)・/api/stats 200 実データ・www 302。pdfdotai は退役のまま（MC-316 の意図を維持・相乗り構造も解消）。備考: /api/stats の全期間PV=5 は 7/19 21:22 の意図的statsリセット（stats-reset-backup-20260719_2122 に退避済）でありデータ消失ではない | 林 | [[MC-316]]の巻き添え復旧。videodl/README.md のトンネル記述は要更新（旧「pdfdotai経由」） |
| MC-331 | ClipItNow 海外先行の流入施策（ツールディレクトリ登録＋英語Reddit/掲示板訴求） | 高 | IN_PROGRESS（2026-07-22 Keita「海外先行でやろうか。ディレクトリとか英語のRedditとか掲示板系で訴求」）。チャネル調査完了（2026-07-22・レポート=video-dl/docs/overseas-channels-plan-20260722.md）。最有力=AlternativeTo登録・r/InternetIsBeautiful・Show HN・V2EX(zh)。一般チャネルではアダルト対応は完全非言及・アダルト訴求はGFY/XBIZ系のみの二面戦略。**投稿文ドラフト完成（2026-07-22・video-dl/docs/post-drafts-20260722.md — AlternativeTo/SaaSHub/r/InternetIsBeautiful/Show HN/V2EX/r/SideProject 6本＋投稿前チェックリスト）**。次=Keitaのアカウント作成/メール認証（AlternativeTo/SaaSHub/Reddit=カルマ稼ぎ今すぐ開始推奨/HN/V2EX）＋**広告体験の事前調整=完了・本番反映済（2026-07-22 Son）**: 外部referrer/?ref=/?utm_source=の初回セッション（sessionStorage）はpopunder・Direct Linkゲート・保存後全画面をスキップし1タップ即DL。インライン広告は維持で収益影響最小。コミット=069913f。本番実測=?ref=testで__EXT_REF=1注入確認。**投稿前チェックリストの技術要件は全て充足→残るはKeitaのアカウント作成のみ**。【2026-07-28 Keita】専用メール **clipitnownet.official@gmail.com** を作成済（資格情報はSonローカル保管・台帳非記載）。【2026-07-28 Son・アカウント作成実施】**HN=作成完了**（user: clipitnow・メール設定済＝Show HN投稿可能に）。**AlternativeTo=作成完了（2026-07-28夜・MC-350埋め込みブラウザでSonがフォーム入力→KeitaがhCaptchaタップ→サインイン確認済・認証待ちバナー無し。※新規アカは7日経過後にアプリ登録可＝製品ページ登録は8/4頃）**。残=Reddit/SaaSHub: サーバのデータセンターIPがネットワーク遮断対象（Reddit=network security block・SaaSHub=Cloudflare Turnstile通過不可）→**Keitaのスマホで各2分の作成待ち（入力内容はSonが提示済）**／**V2EX=メール新規登録廃止**（Solana 10Kコイン保有 or Google+招待コードのみ）＝チャネルから除外。**【2026-07-28 深夜 Son・Cowork活用で下準備2本完了】** Apollo埋め込みCowork(MC-350)をSonが遠隔操作し並行実行→①**AlternativeToリスティング一式ドラフト**（150語/50語/15語説明文・カテゴリ/タグ・タグライン3案・alternative to関連付け8ツール・スクショ6枚プラン。アダルト非言及/YouTube非記載で整合）＝`video-dl/docs/alternativeto-listing-draft-20260728.md`（8/4のアプリ登録時に使用）②**Show HNドラフトのレビュー改稿**（過去1年のShow HN×DL系実調査ベース: 生ダウンローダー投稿はほぼ黙殺・タイトルは「iOSカメラロール難所」軸の案A推奨・本文改稿版・厳しいコメントTop5模範返答・投稿最適時刻=火水 8:30 ET≒JST火水21:30-22:30）＝`video-dl/docs/show-hn-review-20260728.md`。**次=Keitaが投稿文最終確認→Show HN投稿（HNアカウント準備済）** | Son | 外部送信はKeita確認必須（SOUL境界） |
| MC-332 | ClipItNow 英語圏向けUX/機能改善の調査 | 高 | DONE（2026-07-24 棚卸し/Son: P1・残フェーズとも実装済＋本番実測済（/en/tiktok-download 200 を本日再確認）。残余のzh版ガイド等P3は必要時に別起票→DONE化。← IN_PROGRESS（2026-07-22 Keita「英語圏向けにサイトの機能やUIの改善点がないか調査して」）。調査完了（2026-07-22・レポート=video-dl/docs/en-ux-audit-20260722.md）。訪問者の8割がenなのに (a)SSRのtitle/meta/ogが日本語固定 (b)SEOガイド59本が全部日本語 (c)hreflangゼロ＝「英語で集客して日本語を見せる」状態。P1=SSRメタのロケール差し替え・主要ガイドEN化＋hreflang・英文リライト・Pasteボタン。**P1(S)実装完了・本番反映済（2026-07-22 Son）**: SSRメタ5言語差し替え・英文リライト・ガイドリンク18本i18n・Pasteボタン・hreflang(?lang=言語別URL新設・canonical/og:url自己参照)。コミット=a94c2d2/de73870（video-dl・remote無しローカル運用）。検証=py_compile0・ローカル4399でen/esメタ・hreflang6本・canonical確認→video-dl.service再起動→https://clipitnow.net 実機でENタイトル/og:locale=en_US/hreflang確認済。**残フェーズ完了・本番反映済（2026-07-22 Son）**: ①ガイドEN化15本=/en/<slug>（twitter/tiktok/instagram/reddit/soundcloud/twitch/facebook/dailymotion/vimeo/pinterest/threads/tumblr/streamable/vk/kick・日英hreflang相互・_gen_data_en.py新設・blueskyはガイド自体無く対象外）②ENキーワードページ4本（tiktok-downloader-no-watermark/soundcloud-to-mp3/reddit-video-downloader-with-sound/how-to-save-videos-on-iphone）③FAQ JSON-LDロケール別注入④対応サイトチップ18個⑤DL後iOS/Android別保存ヒント⑥ダークモード（OS追従＋トグル）。エラーcode化はa94c2d2で対応済につきスキップ。コミット=814f0ff/6477c0e/0e83b7e。検証=py_compile0・node --check0・ローカル4402で/en/各200・sitemap en19本→service再起動→本番実測（/en/tiktok-download 200・sitemap en19・darkmode/grid確認）。**PWAのみ別タスク（SWキャッシュ事故リスク）**。残=P2のzh版ガイド(P3)等は必要になったら | Son | 調査→改善案リスト→実装は別途起票 |
| MC-333 | Apollo端末でURLリンクを開けるようにする（クリック/タップでURL検出→開くトースト） | 高 | DONE（2026-07-22 Son）。原因=ttyd iframe＋TAP_FIXがタップをtmuxへ流すためxterm link層に届かない。実装=terminalProxy注入LINK_FIX: click/touchend追加リスナー→座標からバッファ論理行(isWrapped連結・前後5行上限)→URL検出(全角/閉じ括弧トリム)→下部トースト「🔗 URL [開く][×]」(6秒・誤爆防止=ドラッグ/スワイプ/長押し/選択中は非表示)→window.open。コミット=29a3798。検証=tsc0・linkfixテスト4/4・keyfix回帰13/13→backend再起動→本番/terminal/でLINK_FIX注入確認。※tapfixテスト0/10はMC-330由来の既存故障(モックにaddEventListener無し)＝MC-334で追跡 | Son | — |
| MC-334 | terminalProxy.tapfixテストの既存故障修正（MC-330でリスナーがt.elementへ移りモック不整合・0/10） | 低 | DONE（2026-08-01 Son subagent・自動ディスパッチ第1号）。修正=terminalProxy.tapfix.test.tsのみ: term.elementをaddEventListener/style付きMockRootElement(querySelectorで子レイヤ返却)に変更し、イベント発火先とアサーション(リスナー登録先・touch-action)をt.elementへ追従。実装コードは無変更。検証=tapfix10/10・回帰keyfix13/13/linkfix4/4/inbox系4+16+3全パス・tsc --noEmitクリーン。コミット=cc2e7a0 | Son | — |
| MC-335 | ClipItNow 中国語版（zh）LP＋主要ガイド展開 | 高 | DONE（2026-07-24 Son・subagent実装）。_gen_data_zh.py新設＋_gen_guides.py拡張＋server.py /zh/ルート・sitemap。作成=ガイド15本(/zh/twitter-download等・ja/en/zh hreflang相互)＋zhキーワード3本(tiktok-no-watermark/bilibili-video-download/iphone-save-videos)。簡体のみ(繁体は個別ページ要で見送り)。検証=py_compile0・ローカル4402で200/404/hreflang4行/sitemap18件→video-dl.service再起動→本番全18ページ200・title/hreflang/sitemap確認。コミット=931bf0c(52files/+4165・他者WIP未タッチ)。LPのzhメタ/i18nはMC-332対応済で変更不要 | Son | V2EX投稿(MC-331)と連動 |
| MC-336 | ClipItNow ロングテール記事量産（サイト別×言語別生成＋PD-CA毎晩自動追加） | 高 | DONE（2026-07-24 Son・subagent実装＋Son修正）。第1弾18本公開（ja6/en6/zh6・longtail/*.json→tools/longtail_build.py品質ガード=手順4/FAQ3+/固有文最低字数/YouTube・アダルト名禁止/slug衝突拒否）。PD-CA組込=report.shから tools/longtail_autogen.sh 呼出（毎晩キュー docs/longtail-queue.md から1-2本生成→検証→commit→再起動→本番200→done化・失敗はhold退避でレポート継続・flock多重防止）。**Son修正**: JSON抽出がstrict厳格モードで文字列内生改行により必ず失敗→strict=False＋失敗時生出力ログ保存(e459ee9)。E2E実測=twitch-clip-save-android 生成→品質ガードOK→本番200(eefc01e・added=1)。sitemap119URL。コミット=a744d33/eefc01e/e459ee9 | Son | キュー残12本・毎晩自動消化 |
| MC-337 | ClipItNow アカウント不要ディレクトリへの登録実施 | 中 | DONE（2026-07-24 Son・subagent実施）。16件超調査→**登録3件完了**: Viesearch(token 276tl・メール確認クリック済 2026-07-24 Keita→審査待ち)/Launching Next(受付ID 141988)/Ctrl Alt(success・launch 7/31)。全てアダルト非言及・social media video downloaderフレーミング。Keita待ち=AlternativeTo/SaaSHub(アカ作成・最優先)＋Startup Buffer/startups.fyi/StartupStash等(CAPTCHA/手動)。見送り=一括送信系(スパムリスク)・AI特化・閉鎖サイト。記録=video-dl/docs/directory-submissions-20260724.md（2週間後の掲載確認フォローはSon） | Son | 掲載確認フォロー=2026-08-07頃 |
| MC-338 | ClipItNow リピート導線強化（PWA共有ターゲット/ブックマークレット「共有→ClipItNow」） | 中 | DONE（2026-07-24 Son・subagent実装）。①manifest.webmanifest動的生成(share_target GET action=/ params={title,text,url}・icons192/512/maskable PIL生成・standalone)②SW=fetchハンドラ無しの最小SW(skipWaiting+claimのみ・キャッシュAPI経路が構造的に存在せず更新事故ゼロ・旧Chrome installability互換用)③?url=共通受け口(入力欄自動投入→自動解析→replaceState・text/titleからURL抽出・?v=302のクエリ引継修正・ev('share_target')計測)④設置ガイドi18n ja/en/zh＋ブックマークレット(コピーボタン・ev('bm_copy'))。iOSはホーム画面追加＋ブックマークレットで代替。検証=py_compile0・node --check0・ローカル4398全200/302クエリ保持/__EXT_REF継続/広告注入継続→service再起動→本番manifest/sw/アイコン200・?url=302→200・広告16箇所・回帰全200。コミット=063d31c | Son | Android実機での共有メニュー出現確認は端末があれば推奨 |
| MC-340 | ClipItNow ランキングの英語圏対応（ja以外の訪問者に日本語タイトルを出さない） | 中 | DONE（2026-07-24 Keita「ランキングも英語圏向けにして」→Son直接・実機検証済）。/api/ranking に lang パラメータ追加・クライアントは現在言語(LANG)を送信（言語切替で再読込は既存動作）。ja以外(en/zh/es/ko)=かな入りタイトルを実績・シードとも除外（demoteだと実績1件時に1位へ残るため除外に統一。漢字のみは中華圏の可能性があるので対象外）。ja=従来どおり50件。**実機検証(本番)**: ja 3タブ=50件(日本語含む)・en/zh 3タブ=日本語0件。**追補（同日 Keita「動画も英語圏にうけるやつにして」）**: 英語圏専用シード ranking_seeds_en.json 52本を新設（xvideos/xhamster/pornhub の人気一覧から採取・日本IP由来のアジア偏りを語フィルタ(japanese/jav/asian/hijab等＋品番)で除外・ASCII率90%）＋load_seeds言語別化（ja以外=enファイル・無ければjaへフォールバック）＋サムネ52本ローカルキャッシュ(/thumbs/)・取得失敗1本は除外。tools/gen_ranking_seeds_en.py 新設・cache_seed_thumbs.py 引数対応。**本番実測**: en/zh 3タブ=50件・日本語0件・サムネ200。コミット=584ed8f/b87250d/e6ac398・service再起動済。**追補2（同日 Keita「ポップアップの文言が日本語のまま」）**: 広告ポップアップのハードコード日本語をi18n化＝「広告」ラベル/「広告を読み込み中…」/「まもなくダウンロードが始まります」/🔇音を出す・🔊ミュート を ad_* キーで5言語辞書(en/ja/ko/zh/es)へ追加・全インラインJS構文チェックOK・本番実測=?lang=enで英語辞書配信＋ハードコード0件（index.htmlはディスク直読み＝再起動不要）。コミット=8729eb4。**追補3（同日 Keitaスクショ=英語UIでアドブロック解除モーダルが日本語）**: 真因=applyStatic()がscript位置で1回実行のみ→後方描画の#abmodal/#interUnmuteに翻訳が当たらない描画順バグ。DOMContentLoadedで再適用する1行を追加。**実機検証(Playwright/本番)**: ?lang=en=Please disable your ad blocker…/Unmute/Reload・?lang=ja=従来日本語＝PASS。コミット=5370665 | Son | シード更新は gen_ranking_seeds_en.py 再実行→cache_seed_thumbs.py ranking_seeds_en.json→再起動 |
| MC-339 | ClipItNow ExoClick広告購入テスト（少額$10-20でトラフィック購入→採算検証） | 中 | IN_PROGRESS（2026-07-24 Keita「決済もしたよ」＝ExoClickへ直接入金・実質承認。22:00時点でAPI残高$0=反映待ち。**2026-07-25 10:40 Keita「動かして」→Son入稿完了**: キャンペーン **8476770 "ClipItNow-traffic-test-JP-pop"** をAPIで作成＝Popunder(ad_type7/link)・JPN・カテゴリAdult(2)・**Smart CPM $0.30**(JP×cat2の最低値・APIのprice/予算はセント単位)・日次上限$20(分散配信)・**総予算上限$20**(API最低額。$10-20枠の上限で確定)・LP=clipitnow.net/?utm_source=exoclick&utm_medium=popunder&utm_campaign=mc339test(library URL 6766416・variation 143660076)。**status=Running化済→calculated_status「No Funds」＝残高反映と同時に自動配信開始**(操作不要)。10:40時点でも残高$0続き＝入金が24h超未反映。誤作成の重複8476772はarchive済。計測=UTM(analytics.jsonl側)×/v2/statistics advertiser側で採算比較。**残ブロッカー=入金反映のみ**→Keitaに決済方法/明細の確認依頼中(カードなら即時のはずが未反映のため決済未完の可能性)。【2026-07-25 21:30 棚卸し/Son実測】API残高$0継続・8476770=calculated_status「No Funds」・variation 1件pending審査中＝決済から約24h超で未反映続き。Keitaの決済明細確認待ち。【2026-07-26 21:30 棚卸し/Son実測】API残高$0継続＝約48h超未反映。決済未完の可能性が濃厚→Keitaの決済明細確認待ち継続。【2026-07-27 21:30 棚卸し/Son実測】API残高$0継続＝約72h超未反映。決済未完がほぼ確定→Keitaの決済明細確認待ち継続。【2026-07-28 21:30 棚卸し/Son実測】API残高$0継続＝約96h超未反映→Keitaの決済明細確認待ち継続。【2026-07-29 21:30 棚卸し/Son実測】API実確認: user balance=$0・8476770 total_budget_spent=0・advertiser統計7/25-29空＝約120h(5日)未反映。決済未完がほぼ確定→Keitaの決済明細確認（or 再決済）待ち継続。【2026-07-30 21:30 棚卸し/Son実測】API実確認: balance=$0・8476770 calculated_status「No Funds」・spent=0＝約6日未反映。Keitaの決済明細確認（or 再決済）待ち継続。【2026-07-31 21:30 棚卸し/Son実測】API実確認: balance=$0継続＝約7日未反映。再決済依頼はMC-351で決裁フロー投入済（7/30）→Keita応答待ち | Son | 決裁フロー投入済→Keitaが直接入金で実質承認 |
| MC-341 | 開発アイデア生成を実用重視へ再刷新（MC-320のぶっ飛び方向を上書き） | 中 | DONE（2026-07-25 Keita「もっと実用的なアイデアにして」）。**Son直接・実機検証済**。devMockupRouter.ts の IDEA_SEEDS を実務・日常の困りごと解決系12切り口（文章代行/意思決定支援/お金実務/段取り/会議下ごしらえ/暮らし/学習補助/文章変換/健康記録/伝え方/書類手続き/定型文自動化）へ再刷新、buildIdeaPrompt を「本当に使うか・時間/手間/迷いを減らすか」基準に書き換え（AI付加価値なしの丸腰ツール禁止・医療/法律/税務断定回避ガード付き）。server全体 tsc 0→devチャンネル事前連絡→mission-control 再起動。**実機検証(本番API)**: POST /api/dev/idea→job done×3回、出力例「会議テーマと参加者を入れると時間配分付きアジェンダ＋事前確認チェックリスト」「冷蔵庫の残り食材から買い物なし献立3案」＝実用方向◎・切り口の多様性も確認 | Son | server変更＝backend再起動済 |
| MC-342 | 開発アイデア生成を市場インパクト重視へ再々刷新（MC-341の実用重視を上書き） | 中 | DONE（2026-07-25 Keita「もっと市場にインパクトのあるようなアイデアにして」）。**Son直接・実機検証済**。IDEA_SEEDS を事業性・市場機会重視の12切り口（専門サービス民主化/業界中抜き/中小DX/省人化/クリエイターエコノミー/教育個別最適化/ヘルスケア入口/越境・多言語/高齢化インフラ/BtoB定型業務/高額意思決定ナビ/公開データの民主化）へ再刷新、buildIdeaPrompt をVC視点「大きな市場×強い痛み×なぜ今AIか」基準に書き換え（既存大手の劣化コピー禁止・医療/法律/税務は入り口扱いガード維持）。tsc 0→devチャンネル事前連絡→mission-control 再起動。**実機検証(本番API)**: POST /api/dev/idea→done×2、出力例「保険証券UP→重複補償と空白を診断し最安構成と年間節約額」「内定通知→同企業経由者の5年後年収推移とキャリア資産スコア」＝市場インパクト方向◎ | Son | server変更＝backend再起動済 |
| MC-343 | 開発アイデア生成を「誰も気づいていない潜在ニーズ」方向へ刷新（MC-342の顕在市場機会を上書き） | 中 | DONE（2026-07-25 Keita「もっと世の中の人が気づいてないような潜在的なやつにして」）。Son直接。IDEA_SEEDS を潜在需給・盲点の12切り口へ刷新。実装は MC-344 と同一変更に統合（検証も MC-344 側で実施） | Son | MC-344 に統合 |
| MC-344 | 開発アイデア生成を「メルカリ級プラットフォーム＋潜在市場規模分析付き」へ再刷新 | 中 | DONE（2026-07-25 Keita「メルカリのようなサービスを作れるアイデアに。潜在市場規模も分析しつつ」）。Son直接。IDEA_SEEDS=遊休資産流動化/非公式取引の公式化/セミプロ供給解放など潜在需給接続の12切り口、buildIdeaPrompt=「潜在供給×潜在需要を初接続・なぜ今まで市場にならなかったか・AIが障壁を壊す」基準＋出力最終文に潜在市場規模のフェルミ推定（対象人数×頻度×単価）を必須化、cleanIdea=2部構成対応で400字へ拡大。server tsc 0。**【2026-07-25 棚卸し/Son】DONE確定**: コードはHEADに commit済＋backend 14:03再起動で本番反映済を確認。**実機検証(本番API)**: POST /api/dev/idea→done、出力例「成功した交渉メール本文を匿名化して200円で売買する取引市場＋潜在市場規模: 社会人5000万人×年3回×1割×200円=年300億円規模」＝メルカリ級P2P市場×フェルミ推定の両要件充足◎。※初回ジョブは claude 90sタイムアウト×2で error（プロンプト増量で生成時間の余裕が薄い。再試行で成功＝当面様子見） | Son | server変更＝backend再起動済 |
| MC-345 | 「ナオシテ」（修理のメルカリ）プロトタイプ作成・Apolloで配信 | 中 | DONE（2026-07-25 Keita「プロトタイプをアポロの開発で作って」。分析=docs/service-ideas-marketscale.md 第一候補）。**Son直接・実機検証済**。自己完結HTML（モバイルUI: ホーム→写真/サンプル選択→AI診断演出→職人入札3件→依頼確定→エスクロー進行タイムライン）を web/public/naoshite.html に作成し dist へ直接配置（共有ツリーWIP多のためビルド回避・再起動不要）。実機検証: apollomansion.com/naoshite.html を OpenClawブラウザで全4画面フロー通し確認 | Son | service-ideas-marketscale.md |
| MC-346 | ナオシテ・プロトタイプの写真診断を本物のAI（Gemini vision）に差し替え | 中 | DONE（2026-07-25 Keita「お願いします」）。**Son直接・実機検証済**。server: naoshiteRouter.ts 新規（POST /api/naoshite/diagnose、Gemini 2.5 flash vision・thinkingBudget:0（thinkingがmaxOutputTokensを食い出力が途切れる罠）・失敗時200{error}でクライアントがサンプル診断へフォールバック）、index.ts へ /api/naoshite 登録。client: naoshite.html にcanvas縮小(長辺900px/JPEG0.75=body 1MB制限対策)→実AI診断→品目に応じた職人リスト表示。devチャンネル事前連絡→mission-control再起動。実機検証: 実画像POST→スマホスクショを「損傷なし・点検推奨」と正判定、本番ページからのfetch→hasDiag:true・page errorsなし、サンプルフロー回帰OK | Son | MC-345 / server変更＝backend再起動済 |
| MC-347 | ClipItNow 競合調査→ランキング型LP展開・非アダルト方針（2026-07-25 Keita「まかせるよ。ランキングもアダルト動画ばかりじゃないほうがいい」） | 中 | IN_PROGRESS。Son直接。競合3類型分析（ソフトベンダーSEO/オンラインツール型/アフィリランキング型）→ランキング型が上位主流と判断。第1弾 /twitter-video-save-sites 公開済（自社開示・公式ページ記載ベースの正直比較・非アダルトのみ・本番200・sitemap反映・IndexNow送信済、video-dl commit 8da38de→ランキングcommit）。残: TikTok版ランキングLP、bilibiliクラスタtitle調整（GSC実クエリ反映）。被リンク外部発信は文面を用意してKeita確認後に実施。【2026-08-01 Son】第2弾 /bilibili-save-sites 作成済（GSC実データで流入ほぼ全てbilibiliクラスタ→勝ちクラスタ増強。ClipItNow/SnapAny/9xbuddy/VideoFK の公式ページ記載ベース正直比較・運営者開示明記・非アダルトのみ・既存bilibiliガイド7ページと相互リンク・sitemapはguides/自動走査で反映。PORT=4399検証200/title OK・py_compile 0・video-dl commit 115d6e2・**本番はvideo-dl.service再起動待ち**）【2026-08-01 15:30 Son/MC-354ループ】LP第3弾 /niconico-save-sites 公開（ニコニコ保存サイト正直比較・競合3件は公式ページWebFetch裏取り済/dirpy・9xbuddy等は確認不可で割愛・既存niconico3ページと相互リンク・commit ed51d84・再起動→本番200・sitemap反映・IndexNow200）。次=第4弾候補選定 or Bing反応観測【2026-08-01 17:30 Son/MC-354ループ】LP第4弾 /tiktok-save-sites 公開（TikTok保存サイト正直比較・比較軸に透かし無し可否・競合=SnapTik/SSSTik/SnapAny=全て公式ページWebFetch裏取り済、TikMateは403で裏取り不能のため不採用・非アダルトのみ・運営者開示と自社欠点も明記・既存TikTokクラスタ5ページと相互リンク・video-dl commit ebd7ad0・本番200・sitemap掲載・IndexNow140件HTTP200）。台帳記載の「残: TikTok版ランキングLP」はこれで消化。残=bilibiliクラスタtitle調整(GSC実クエリ反映)・第5弾以降はBing/GSC反応を見て判断 | Son | MC-336 / video-dl |
| MC-348 | 育児ガイドに月齢別詳細ガイド新設（0か月〜1歳: 授乳/ミルク量・注意点とリスク・親の行動/練習/準備グッズ・手続きを時系列で網羅） | 中 | DONE（2026-07-27 Keita依頼）。**Son直接・実機検証済**。childcareMonthlyGuide.ts 新規（0か月/1か月/…/9-10か月/11か月/1歳〜の13帯×8ブロック: 赤ちゃんの状態・母乳/ミルク別の目安量・注意点・リスク・親の行動・練習・グッズ・手続き）＋Childcare.tsx に MonthlyGuideSection（アコーディオン・現在月齢を自動展開「いまここ」）を成長タイムライン直後へ全幅追加。web build→dist反映（静的・backend再起動不要）。実機検証: 本番 育児ガイドタブで表示・生後1か月自動展開・母乳/ミルク2カラム描画OK。**2026-07-27 Keita「もっと詳細に」→大幅増強**: 全帯を詳細版へ書き直し（体格目安・授乳/ミルクの回数×1回量×1日合計・足りているサイン・調乳手順まで）＋新ブロック3種（😴睡眠の目安・🕐1日の生活リズム例・🏥受診/相談の目安=レッドフラグ）を追加、babyState/注意点/リスク/親の行動/練習/グッズ/手続きも各8〜10項目へ拡充（データ約1,300行）。build→本番で新ブロック描画を実機確認済 | Son | MC-226 / childcareData |
| MC-349 | ClipItNow PjD 2.0 再設計（2026-07-27 Keita「もう一回PjDを考え直して改めてやって」） | 高 | DONE（2026-07-27）。**Son直接・実機検証済**。10巡総括=LP25→72本(3倍)でも実UU3→6＝生産はボトルネックでない/Keita手動レバー(GSC申請等)は10巡未実行。新正本 obsidian-vault/30-Projects/videodl/pjd-2026-07-27.md 作成（Track A=高DA配信を主戦場・Track B=Bing先行・Track C=Google後追い・生産凍結・KPIリセット・ゲート8/24と9/21・Keita依頼は一度きりの立ち上げパック15分へ集約）。反映: clipitnow-pdca-report.sh の戦略プロンプト/フォールバック差し替え＋ロングテールautogen休止（bak-pjd2-20260727保存・bash -n OK）、Pdca.tsx の P/D/C/A カードを新サイクルへ全面更新（web build→dist反映・backend再起動不要）、pdca-latest.md 更新、旧strategy-2026-07-20.md に上書き注記。実機検証: 本番 /pdca でPjD2.0カード描画確認 | Son | MC-312 / MC-336 / MC-347 |
| MC-350 | Apollo に Cowork（claude.ai）を埋め込み表示 — 旧MC-314 noVNCストリーミング方式の復活 | 高 | DONE（2026-07-28 Keita「ここからcowork使える？アポロから。」）。**Son直接・実機検証済**。旧MC-314はコード未コミットのまま撤去済だった→台帳記録から再構築。①裏方: x11vnc/openbox/novnc を再インストール・systemd 5ユニット `apollo-claude-{xvfb,openbox,chromium,x11vnc,novnc}.service` 再作成・enable --now（Xvfb:99 520x950→openbox→playwright Chromium kiosk(https://claude.ai/ profile=~/.openclaw/claude-browser/profile)→x11vnc(127.0.0.1:5901 vncpass)→websockify/noVNC(127.0.0.1:6081)・全てlocalhost束縛）。②server: claudeBrowserProxy.ts 新規（terminalProxy同型: HTTPは認証MW後ろに `/claude-browser` mount・WSは attachClaudeBrowserUpgrade で isRequestAuthorized ゲート・`GET /api/claude-browser/config` でVNCパス認証配布）。③web: ClaudeBrowser.tsx（config取得→/claude-browser/vnc.html?autoconnect&resize=scale&path=claude-browser/websockify を同一オリジンiframe）＋ナビ「Cowork」(SparkIcon)＋route `/claude-browser-view`。検証: server/web tsc 0・build成功→devチャンネル事前連絡→mission-control再起動→未認証401/認証200/config JSON→**本番実画面(apollomansion.com/claude-browser-view)でサイドバー隣に claude.ai ログイン画面が noVNC 描画されるのを確認**。Keitaがこの画面でclaude.aiにログインすればApolloからCowork利用可（CAPTCHA/パスワード入力も画面内で直接操作可＝MC-331の3サイト登録もここで完結できる） | Son | 旧MC-314の再構築。server変更＝backend再起動済。CoworkとApollo使い分け=記憶cowork-apollo-division-of-labor |
| MC-351 | ClipItNow 訪問者増加スプリント（2026-07-30 Keita「頑張って増やしてよ」） | 高 | IN_PROGRESS（2026-07-30 Son）。現状診断: 人間UU=7日で3人・流入3チャネル全停止（Bing全クエリ圏外3週間/GSC連携切れ・ExoClick残高$0約6日・SNS/ディレクトリ投稿はKeita承認待ち）。Son即応: ①ロングテール記事の手動加速バッチ起動（longtail_autogen.sh 4本・通常1晩1-2本）②Keitaブロッカー3件（ExoClick再決済/GSC再同意/Show HN投稿承認）を決裁フローへ投入。継続: キュー補充・Bingクエリ戦略見直し（PDCA第14巡提言「bilibili 動画 保存方法」等ミッドテール化）・8/4 AlternativeToアプリ登録(MC-331)。【2026-07-30 22:15 Son実施済】加速バッチ4本公開＝/threads-save-iphone /en/facebook-reels-download /zh/instagram-reels-download /vimeo-mp3-save（本番200・commit 09db5f3）＋パイプライン2バグ根治（ja related正規化・claude stdin食い潰しで毎晩1本しか出なかった件＝6fce061）＋IndexNow全量再送信200【2026-08-01 08:17 Son/MC-354ループ】加速バッチ2本公開＝/bilibili-download-browser（GSC勝ちクラスタ深掘り・Track C）＋/en/soundcloud-playlist-download。本番200・commit 01eb1bf・IndexNow送信200。残pending=6件【2026-08-01 11:17 Son/MC-354ループ】加速バッチ第2弾2本公開＝/zh/tiktok-to-mp3 /facebook-save-iphone（本番200・commit ae4ad07・IndexNow200）。本日計4本・残pending=4件【2026-08-01 12:20 Son/MC-354ループ】第3弾2本公開＝/en/dailymotion-to-mp3 /zh/reddit-video-download（本番200・72b2b08・IndexNow200）＝本日計6本。キュー補充4件（Twitch VOD ja/en・douyin音声ja・bilibili 1080p zh・fc8ee6d）→残pending=7件 | Son | MC-331/336/339と連動 |
| MC-352 | ClipItNow Track A 配信実行（Tumblr/Blogger/note アカ開設＋投稿＋Bing WMT登録） | 高 | IN_PROGRESS（2026-07-31 23:20 Keita「いいよ」＝Cowork/埋め込みブラウザ経由の配信実行を承認）。PjD 2.0 Track A の「配る」を実行する。ベースGmail=clipitnownet.official@gmail.com（取得済・資格情報はSonローカル保管）。手順: ①Apollo埋め込みブラウザ(MC-350)でSonがTumblr/Blogger/noteアカ開設＋Bing Webmaster Tools登録をドライブ、CAPTCHA/2FAのみKeitaが画面タップ（AlternativeToと同方式）②開設後、post-drafts-20260722.md §7-9（VD-69で最新化済）の文面をSonが投稿（投稿承認は本行のKeita承認で充足）③referrers/Bing WMTで効果をPDCA Checkに組込。文面は投稿前チェックリスト充足済（広告調整069913f・MC-332デプロイ済・アダルト非言及）。ExoClick再決済(MC-339)は自然流入初速が出るまで後回しでよい旨をKeitaへ提案済。**【2026-07-31 23:49 GSC復旧・Son実測】** Keita再同意（1回目23:36=urano2→プロパティ0件で空振り、2回目23:49=keita.urano@gmail.comメインで成功）→gsc-fetch実データ取得成功: **28日でクリック11・表示72・CTR15.3%・平均掲載順位12.0。全クリックがbilibiliクラスタ**（/bilibili-download単独で10クリック/28表示）＝「Google索引ほぼ0・検索流入0」の従来認識は誤り（analytics側referrer欠落で過小計測）。PjD Track C発動条件（表示クラスタ出現）成立→勝ちクラスタ深掘りとしてlongtail-queueへ ja/bilibili-download-browser を追加（GSCクエリ実績ベース）。※プロパティは sc-domain:clipitnow.net＝**メインアカウント管理**（従来ドキュメントの「urano2」は誤記）。**【2026-08-01 06:27 Keita「CAPTCHAはやらなくていいよ」】**→アカ開設はSon単独で試行し、CAPTCHA遭遇時は突破せず保留とする方針に変更（Keitaタップ工程は廃止）。**【2026-08-01 Son実装済・ランキングbilibiliミックス】**Keita指示「ランキングの動画もアダルトだけじゃなくてbilibiliの動画も半分くらい入れて」→gen_ranking_seeds.pyにbilibili公式ランキングAPI(rid=0・buvid Cookie取得で-352回避)ソース追加。構成=アダルト3ソース各9(27)+bilibili24=計51(47%)。サムネ51件ローカルキャッシュ・video-dl.service再起動・本番/api/rankingで23/50件bilibili＋サムネ200実測。commit=06ac9dc【2026-08-01 21:45 棚卸し/Son】本日は毎時ループの外部投稿除外規定により開設未試行。8/2に埋め込みブラウザからTumblr→Blogger→noteの順で単独試行（CAPTCHA遭遇時は突破せず保留・開設/投稿自体は7/31 Keita承認済） | Son | 外部投稿はKeita承認済（2026-07-31）。MC-331/351と連動 |
| MC-353 | タスク前進システム（ブロッカーレジストリ＋Keitaアクションキュー＋自動前進エンジン） | 高 | DONE（【2026-08-01 10:35 Son/MC-354ループ】P3完了: 決裁withdraw API(POST /api/decisions/:id/withdraw・AGENT_TOKEN)＋expiresAt/fallbackOptionId受理＋期限超過時のfallback自動適用(一覧GET時lazy sweep・requesterへnotify・履歴にautoDecided明記)。実機検証済(期限5秒→B案自動適用/withdraw成功)・commit a46e976 push済。P1〜P3全フェーズ完了＝実装DONE(運用は日次board-audit/毎時MC-354ループが継続)。経緯: 2026-08-01 06:35 Keita「タスクボードのタスクを進めるため（ブロック含め）完了に向けて動けるような機能とか仕組み考えて」→Son設計）。設計書=docs/unblock-system-design-20260801.md。骨子: 層1=blockers.json（型A-E分類・停滞日数・次の一手・待ち先）を日次board-auditで生成しボードUIにバッジ表示／層2=Keita専用操作（決済/承認/スマホ）を「今日の2分」カードに集約・夜1回まとめ通知・decisions拡張（withdraw API・expiresAt/fallback）／層3=自動前進（決裁未応答3日再掲→7日縮退案自動起案・停滞TODOのsubagent自動ディスパッチ・代替ルートプレイブック・共有ツリー日次仕分け）。Phase: P1=即日(cron/スクリプトのみ・blockers生成+エスカレーション+MC-334自動消化+通知1通化)→P2=Apollo UI→P3=decisions API拡張。**【2026-08-01 07:00 P1完了・Son実測】**①HEARTBEAT.md board-auditセクションとboard-audit-daily.sh(bash -n検証済)へ層1/層3手順を組込②data/blockers.json初回生成（8件・型A2/B1/C1/E4）③自動ディスパッチ第1号=MC-334を即日消化（tapfix 0/10→全緑・Son再検証=keyfix/linkfix回帰パス・tsc0・push済cc2e7a0）④報告は夜1通＋「今日の2分」形式に変更。次=P2(Apollo UI: 今日の2分カード＋ボード停滞バッジ)【2026-08-01 09:35 Son/MC-354ループ】P2完了: /api/blockers 配信API＋タスクボードに「今日の2分」カード(Keita操作待ちのみ集約・行タップで詳細・承認決裁へ導線)＋各カードに⛔停滞バッジ実装。tsc/build green・backend再起動(dev一声済)・本番実画面で表示確認・commit 33e6cec push済。残=P3(decisions withdraw/expiresAt/fallback) | Son | 現行pending決裁の掃除含む（GSC再同意分は対応済のためP3のwithdrawで閉じる） |
| MC-354 | 常時ワークループ（タスクが無くなるまでエージェント稼働継続） | 高 | IN_PROGRESS（2026-08-01 06:43 Keita「タスクがなくなるまでエージェントが稼働し続けるようにして」→Son即実装）。実装=cron-scripts/task-work-loop.sh＋crontab毎時08-23時JST(毎時15分・bash -n検証済)。動作: Sonがblockers.json/台帳からnext_actor=sonの未完タスクを1件選び実前進（subagentディスパッチ or 直接実施）→台帳/blockers更新→通知なしで終了（報告は夜21:30まとめ1通に集約/MC-353方針）。ガード: flock二重起動防止・Keita待ち(型A)/外部待ちは選ばない・外部投稿/決済/破壊的操作は対象外・共有ツリーは自分の変更のみcommit・検証済みのみpush。進められるタスクゼロ=NO_REPLYで静音。全消化or重大障害時のみ即報告。停止=crontab該当行削除のみ | Son | MC-353層3の常時運転化。cronがセッションを跨いで駆動 |
| MC-355 | Apolloデイリーニュース改善（読みやすいデザイン＋出典リンク末尾集約＋選択テキスト深掘りポップアップ） | 高 | DONE（2026-08-01 07:41 Keita依頼）。要望: ①見た目を分かりやすく親しみやすく（不自然なハイライト回避・内容量は不変）②出典元ニュースサイトのリンクを最後にまとめて掲載③文章をタップ/ドラッグ選択→ウェブ自動検索→「これはこういうことです」解説＋関連リンクをポップアップ表示。実装方針: ②=daily-news-briefing.shのプロンプトに末尾「出典リンク一覧」セクションを追加（翌日号から反映）／①=News.tsxのコールアウト/ハイライト調整・タイポグラフィ改善／③=News.tsxに選択ハンドラ＋ポップアップ、server新設 POST /api/news/deepdive（Gemini google_search grounding・GEMINI_API_KEY・geminiText.ts方式の堅牢フォールバック）。**【2026-08-01 08:00 Son実装・本番反映済】**①news.css新設（コールアウトを淡色カード化・行間1.85・出典リンクカード化）②generatorへ「📚 出典リンク一覧」末尾セクション追加（bash -n済・翌8/2号から反映）③newsRouter.ts＋深掘りUI（選択→🔍ピル→ポップアップ・モバイル=ボトムシート・Esc/背景で閉）。検証=server typecheck 0・unit 10/10・web build成功・dev一声→backend再起動→本番 /api/news/deepdive 実測200（日銀テキストで平易解説＋grounding関連リンク）/空文字400・本番News bundleに深掘りUI確認。残=8/2号で出典セクション見た目確認【2026-08-02 08:20 Son/MC-354ループ】8/2朝号(05:15生成)で「📚 出典リンク一覧」生成確認＋実画面スクショ検証=出典カード化・カテゴリ見出し・トピック注記とも意図どおり描画→**全スコープ完了・DONE** | Son | backend再起動済（dev一声済）。共有ツリー注意 |
| MC-356 | 収益タブ: 「取得OK・実績ゼロ」と「取得失敗」を区別するソース状態バッジ追加 | 中 | DONE（2026-08-01 21:55 Keita「データ取得できてないけど」報告→調査で取得系正常・実績ゼロと判明→表示で区別できるようUI改修。①データソース状態バッジ行(ExoClick/Adsterra/ClipItNow/PDCA・summary生成時刻付き)②available時にimp/収益とも0なら「取得OK・実績ゼロ（imp 0）」表記③失敗時文言を「取得失敗」に統一＋脚注明確化。検証=web build green・localhost:4317実画面でバッジ4種描画/API available値一致/スクショ確認。commit 7707d80 push済・distライブ反映済） | Son | Revenue.tsxのみ変更・他者WIP非接触 |
| MC-357 | Keita操作キューの常設化（docs/keita-actions.md 新設＋GSC再同意/Reddit・SaaSHub登録/docx削除可否の3手順を掲載） | 高 | IN_PROGRESS（2026-08-02 09:49 Keita「手順をターミナルじゃなくてタスクボードに記載して流れていかないように」→Son即対応。**docs/keita-actions.md 新設・3手順掲載済（チェックボックス消し込み式・完了ログ付き）**。決裁決着も反映=MC-339広告中止/MC-331 Show HN承認/GSC「今やる」。残り=Apolloボードへの「Keita今日の2分」表示カード（MC-358 P4に統合）。以後Keita操作が発生したら本ファイルでなく keita-actions.md へ追記する運用 | Son | なし |
| MC-358 | タスクボード＆完了フロー全体再構築（1行インデックス＋タスク個票分離・DoD明文化・DONEアーカイブ・Keitaキュー表示） | 高 | IN_PROGRESS（2026-08-02 10:08 Keita「全体的にタスクボードとタスクを完了する仕組みを再構築してほしい」）。設計= **docs/task-system-redesign-20260802.md**（層1: 台帳=1タスク1行インデックス＋docs/tasks/MC-n.md個票へ長文分離・DONEは年次アーカイブ退避／層2: keita-actions.md常設／層3: DoD明文化=成果物・検証・反映の3点記載でDONE／層4: ApolloボードUI同期＋個票リンク）。P1完了（設計書・keita-actions.md・起票）。次=P2 個票テンプレ＋未完タスク切り出し（collector/tasks.ts のパーサ互換を先に確認）。共有ツリー注意・名指し行のみ編集・全置換禁止 | Son | MC-357 |
