...
| MC-361 | モックアップに図解を標準化 | 高 | IN_PROGRESS（8/9 待機条件解消=越境WIPはf6f7b6bでcommit済→stash apply＋衝突解消でWIP復帰・tsc両green・commit済。残=server restart反映＋MC-360 5案生成＋DoD記入。詳細→ tasks/MC-361.md） | Son(旧dev-apollo) | MC-360依存。詳細→ tasks/MC-361.md |
