...
| MC-163 | タスクボードの各タスクから、そのタスクの進捗・動き（履歴/アクティビティ）を見れるようにする | P1 | REVIEW（2026-06-07 林ティック着手→dev-apollo(ソラ)実装完了。/api/tasks/{id}/timeline API、TaskTimeline.tsx、TaskDetail 埋め込み。server tsc/web build green、systemctl restart 後実機確認済み。commit 60e298f。本番反映は Keita 承認待ち） | dev-apollo（ソラ） |
