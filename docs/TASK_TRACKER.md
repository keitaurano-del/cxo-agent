...
| MC-272 | Cookie同意(CMP)が広告/解析Cookieを実際にゲートする実装・EEA同意管理 | 中 | IN_PROGRESS（Fable） | Fable/Son | CookieConsent.tsx 既存を実効化 |
| MC-273 | 利用規約の確定（免責・アップロード物の扱い・準拠法） | 中 | IN_PROGRESS（Fable） | Fable/Son | |
| MC-274 | robots.txt / sitemap.xml / canonical 整備 | 中 | IN_PROGRESS（Fable） | Fable/Son | |
