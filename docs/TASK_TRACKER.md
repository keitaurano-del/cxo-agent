...
| MC-104 | Apollo ターミナルで claude TUI の選択肢がモバイルのタップで選べない不具合の修正 | 高 | bug / UX | IN_PROGRESS（2026-06-01 起票・dev-logic 着手中。Keita 報告「選択肢が出た時にタップが反応せずどうしようもない」＝モバイルで実質操作不能。claude TUI が選択肢メニュー〔矢印/数字選択 UI〕を出した時、PC マウスクリックは効くがモバイルのタップが反応しない。xterm.js がタッチイベントを TUI のマウスレポーティング〔SGR mouse〕に変換しきれていないのが原因と推定。MC-92/94 で terminalProxy.ts に注入している script と同じ仕組みでタッチ→マウスイベント変換を仕込む方針） | dev-logic | MC-92/93/94（ターミナル系・script 注入）、ttyd 1.7.4 |
...
| ステータス | IN_PROGRESS（2026-06-01 起票・dev-logic 着手中） |
| 担当 | dev-logic（蓮）。台帳更新は task-manager（棚町）管轄 |
