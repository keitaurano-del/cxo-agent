...
| MC-400 | 地図UIリニューアル(TripAdvisor風・ズーム段階集計・左パネル折畳) | 高 | IN_PROGRESS | Son | Keita指示2026-08-22 14:28。(1)左リストパネルはデスクトップでデフォルト折畳+開閉トグル (2)数字は表示単位で集計: 都県全体表示→都県合計、中間ズーム→区市町村合計、z13+で店舗ピン(アイコンはよりズーム後) (3)右下ヒートマップ/現在地FABが+−ズームボタンに重なる問題修正 (4)TripAdvisor参考にチップ/ピン/カード等スタイリッシュ化 |
| MC-401 | 首都圏(神奈川・埼玉・千葉)への店舗データ拡大 | 高 | TODO | Son | Keita指示2026-08-22 14:28「東京だけじゃなくて首都圏のも出せる？」。fetch-osm(JP-13固定)とfetch-gplaces(東京bbox四分木)を1都3県対応に拡張し再sync。チェーン系fetcherの都県フィルタ確認・merge/pricing整合も。Places API追加コスト見込み数百call程度 |
