...
| MC-290 | MC-289/291 の web 変更（App.tsx・Work.tsx・BabyDiary.tsx）を共有ツリー整理後に正式コミット | 中 | TODO（残尾=D判定 BottomNav/icons.tsx のソラ意図確認・resume docx削除はKeita確認待ち。詳細→ tasks/MC-290.md） | Son | 依存=共有ツリー整理（[[cxo-agent-shared-tree-concurrency]]相当）。棚卸しで挙げた117ファイル未コミット問題と同根 |
...
| MC-313 | Apollo 継続改善（不要物削減/UX導線/トークン効率の網羅提案＋順次最適化） | 中 | IN_PROGRESS（次=タブ共通化 TabStrip/CountBadge・最後の🟢。詳細→ tasks/MC-313.md） | Son | [[son-owns-board-reconciliation]] | [[clipitnow-exoclick-integration]] [[son-owns-board-reconciliation]] [[cxo-agent-shared-tree-concurrency]] [[cxo-agent-prod-restart-loads-worktree]] |
...
| MC-331 | ClipItNow 海外先行の流入施策（ツールディレクトリ登録＋英語Reddit/掲示板訴求） | 高 | IN_PROGRESS（Show HN承認済→Son火/水夜投稿。Keita残=Reddit/SaaSHub登録。詳細→ tasks/MC-331.md） | Son | 外部送信はKeita確認必須（SOUL境界） |
...
| MC-347 | ClipItNow 競合調査→ランキング型LP展開・非アダルト方針（2026-07-25 Keita「まかせるよ。ランキングもアダルト動画ばかりじゃないほうがいい」） | 中 | IN_PROGRESS（LP第1〜4弾公開済→bilibiliクラスタtitle調整・第5弾はGSC/Bing反応待ち。詳細→ tasks/MC-347.md） | Son | MC-336 / video-dl |
...
| MC-351 | ClipItNow 訪問者増加スプリント（2026-07-30 Keita「頑張って増やしてよ」） | 高 | IN_PROGRESS（夜間PDCA自動運転・Bing順位/GSC反応を観測中。詳細→ tasks/MC-351.md） | Son | MC-331/336/339と連動 |
| MC-352 | ClipItNow Track A 配信実行（Tumblr/Blogger/note アカ開設＋投稿＋Bing WMT登録） | 高 | IN_PROGRESS（Tumblr/Blogger/note開設をSon単独試行・CAPTCHA遭遇時は保留。詳細→ tasks/MC-352.md） | Son | 外部投稿はKeita承認済（2026-07-31）。MC-331/351と連動 |
...
| MC-354 | 常時ワークループ（タスクが無くなるまでエージェント稼働継続） | 高 | IN_PROGRESS（毎時ワークループ稼働中 crontab 08-23時JST 毎時15分。詳細→ tasks/MC-354.md） | Son | MC-353層3の常時運転化。cronがセッションを跨いで駆動 |
...
| MC-357 | Keita操作キューの常設化（docs/keita-actions.md 新設＋GSC再同意/Reddit・SaaSHub登録/docx削除可否の3手順を掲載） | 高 | IN_PROGRESS（keita-actions.md運用開始済。残=ApolloボードのKeitaキュー表示カード=MC-358 P4。詳細→ tasks/MC-357.md） | Son | なし |
| MC-358 | タスクボード＆完了フロー全体再構築（1行インデックス＋タスク個票分離・DoD明文化・DONEアーカイブ・Keitaキュー表示） | 高 | IN_PROGRESS（P1設計✅・P2個票切り出し✅ 2026-08-02。次=P3 DONEアーカイブ＋board-audit手順更新。詳細→ tasks/MC-358.md） | Son | MC-357 |
