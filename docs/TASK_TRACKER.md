...
| MC-381 | ラボ: 生成モックにCSSが当たらない（無スタイル表示） | 高 | DONE | Son | Keita報告2026-08-19 22:59「UIあたってる？」(マニュアル作成モック)。原因=AIがCSS途中で書き直し、未完1本目＋---HTML---＋完全2本目が丸ごと保存され、未閉じstyleが後続文書を飲み込みCSS全滅。対策=①splitPlanHtmlをやり直し耐性化(マーカー/DOCTYPE複数時は最後の1本のみ採用) ②当該モックは完全版55,961字を抽出して修復保存・実画面でスタイル適用確認済 |
