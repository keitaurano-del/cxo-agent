...
| MC-206 | 性能改善: タスクボード肥大化（281件中279件=99%がDONE/CANCELLEDで689KB）を scope＋遅延読込で軽量化 | P1 | IN_PROGRESS（★2026-06-08 08:30 Masayoshi 報告→Son 記録。現状=/api/tasks が全281件(689KB)を毎回返し肥大化の主因（279件=99%が DONE/CANCELLED）。対応=/api/tasks に **scope パラメータ追加**、ボードは**稼働中タスクのみ既定読込**・完了/キャンセルは**遅延読込（オンデマンド）**に変更。dev-apollo 実装中。完成後 Masayoshi 検証→push＋restart（検証ゲート）。MC-194 系の継続。 | dev-apollo / Masayoshi 検証 | MC-194 |
