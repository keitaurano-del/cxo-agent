...
| MC-167 | タスク詳細ビューの整理：無関係な3欄を削除し、タスク自身の履歴に集約 | P1 | TODO（2026-06-07 Keita「紐づくワークフロー/デプロイ状況/紐づくエージェントが分からない・不要。何が要るか検討して」。Masayoshi 検討結果＝以下で実装。【削除】①紐づくワークフロー（TaskDetail.tsx WorkflowSection：タスクに紐づけられず無関係 wf_xxx をトークン数つき羅列＝ノイズ）②紐づくエージェント会話（同様に特定不可で直近会話を表示）③デプロイ状況（MC-64 DeploySection：CI実行履歴でタスク単位でない）。【残す・強化】(a)概要（status/優先度/担当=擬人化/project）(b)詳細メモ・現在の状態（中身・DoD）(c)活動タイムライン（MC-163）を主役に：そのタスクのステータス遷移＋MC-IDを含む関連コミット＋本当に紐づくエージェント動作のみを時系列で。(d)ブロッカー/依存があれば上部に目立たせる。狙い＝「このタスク今どうなってる？」が一目で分かる、無関係羅列の排除。dev-apollo(ソラ)。実機検証→承認フローで反映） | dev-apollo（ソラ） |
