...
| MC-325 | ClipItNow: 完了後のプレビュー再生＋シーンサムネイル表示 | 高 | DONE（2026-07-20 Keita「dl-x.com のように埋め込み再生・サムネ/スクショでシーンを画像で見られる工夫を」）。**Son直接・実機検証済**。①埋め込みプレビュー: 準備完了ファイルを /api/preview（inline・Range対応206）で配信し、保存ボタン下の「プレビュー再生」タップで <video>/<audio> 埋め込み再生（シーク可・DL計上なし・音声はaudio要素）。②シーンサムネイル: 完了後にサーバが ffmpeg で等間隔8枚（短尺は本数自動調整）を裏生成→ /api/scenes ポーリングで生成済み分から順次グリッド表示（タイムスタンプ付き）。シーンをタップするとプレビューがその場面へシークして再生。5言語i18n（pv_play/sc_head）。**実機検証(本番clipitnow.net/headless)**: URL解析→最高画質→完了→プレビュー再生開始(paused=false)→シーン8枚表示→シーン5タップでt=337.9へシーク再生・JSエラー0＝全PASS。API検証: /api/scenes ready/8枚・/api/preview 206+Content-Range・/api/scene image/jpeg。commit 3c0845e | Son | video-dl(:4319, clipitnow.net)。server.py変更＝video-dl.service再起動済 |
