...
| MC-162 | ナビでダッシュボードと他ページ（ターミナル/フォルダ/ノートブック/チャット）が同時にアクティブ点灯する | P2 | REVIEW（2026-06-07 林再検証。web/src/lib/nav.ts の NON_DASHBOARD_PREFIXES に /terminal-view /deliverables /notebooks /chat 追加。tsc -b/npm run build green。commit 68d018c→push 済み。deploy 承認リクエスト req-372572ef-4923-400b-bff6-61b83387cf08 投げ（2026-06-07 07:17）） | dev-logic | |
...
| MC-165 | MC-164 の作り込み：エージェントをドット絵キャラ＋吹き出しで擬人化（現状チープ・誰が誰か不明） | P1 | TODO（2026-06-07 Keita「チープすぎる・誰がどのエージェントか分からない」＋デザインイメージ提示。【完成イメージ】各エージェントを **ドット絵(pixel art)のキャラ**で表示し、その横/上に **コメントの吹き出し**で「今何をしているか」をリアルタイム表示。エージェントチームが働いている様子が一目で分かる賑やかな見た目。【現状の問題】カードが subagentType の生文字列(general-purpose/unmatched:other/workflow:other/Explore)を表示＝人格名・絵文字・イラスト無し。原因：自律実行が汎用subagentで動き人格ロスター(レン🔧/ソラ🛰/ケン🧪/ユイ📊/アオイ🎨/ナオ✍️)に未紐付け。【要件】①@アオイ：6人格のドット絵アバターを作成（役割が伝わる意匠：レン=実装/工具、ソラ=衛星/宇宙、ケン=検証/ラボ、ユイ=台帳/クリップボード、アオイ=デザイン/パレット、ナオ=執筆/ペン）。稼働中/待機のバリエーションや簡単なアニメがあると尚良。②@ソラ：稼働インスタンスを人格にマッピング(cxo→ソラ/logic→レン/検証→ケン/台帳→ユイ等、non-personaバケツは責任人格に畳む・重複集約)し、ドット絵＋吹き出し(現在タスク・最新アクション)をSSEでリアルタイム表示するUIを実装。データ契約は2人で合わせる。実機検証→REVIEW→承認フローで本番反映） | designer（アオイ）＋dev-apollo（ソラ） |
