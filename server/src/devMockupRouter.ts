...
const INTERACTIVE_RULES = [
  ...
  '出力が長さの上限で途中で切れて続きを求められた場合は、必ず切れた箇所の続きだけを出力すること。',
  '絶対に最初から書き直したり、要約・簡略版に作り直したりしないこと（書き直すと既存の機能や画面が失われる）。',
].join('\n');
...
const ANTI_AI_DESIGN_RULES = [
  ...
  '  信号機3色のステータスピル（完了=緑・進行中=黄・要対応=赤 を並べる）/ 薄い色付き背景の角丸「お知らせ風」',
  '  情報ボックス。ステータスや進捗も汎用部品でなく題材の業界語彙で表現する（例: 届出なら受付印・綴り・付箋、',
  '  会計なら消し込み線。「n件中m件完了」ではなくその業界で実際に使う言い方にする）。',
...
const DESIGN_SELF_CHECK = [
  ...
  '  同じ形のカードの繰り返し / 不要なピル型ラベル / 信号機3色のステータスピル / 薄い色付き角丸の情報ボックス /',
  '  意味のないグラデーション・アイコン / 過剰な角丸・影 /',
...
/** 👍/👎 の学習内容をプロンプトへ差し込む行を組む（MC-479）。良い例に寄せ、悪い例を避ける。 */
function feedbackGuidance(): string[] {
  const fb = loadIdeaFeedback();
  const lines: string[] = [];
  if (fb.good.length) {
    lines.push(
      '',
      '【Keita が 👍 と評価したアイデアの傾向】この方向性・粒度・テイストに寄せること:',
      ...fb.good.slice(0, 5).map((s) => `・${s}`),
    );
  }
  if (fb.bad.length) {
    lines.push(
      '',
      '【Keita が 👎 と評価したアイデア】この手のもの（同種の切り口・複雑さ・偏り）は避けること:',
      ...fb.bad.slice(0, 5).map((s) => `・${s}`),
    );
  }
  return lines;
}
...
function buildIdeaPromptOffline(): string {
  ...
  const bm = pickRandom(AXIS_BM_ZUKAN); // 収益の型は 1 つだけ・ヒント扱い（MC-479: 2種強制をやめ複雑さ・偏りを解消）。
  ...
  const avoid = recentIdeas.length
    ? [
        ...
        'これらと業種・仕組みが被らないものにすること。',
      ]
    : [];
  return [
    'あなたは、動く HTML 試作品を作るためのアイデア出しを手伝う、実業に強い事業プロデューサーです。',
    'テーマは「既に現実世界で回っているオフラインの実業に、オンラインを少し足して稼ぐ小さなサービス」です。',
    '「ボタンが実際に動く小さな試作品を 1 つ作る」ための、具体的なアプリ/画面のアイデアを 1 つだけ提案してください。',
    '',
    `今回のヒント（無理なら崩してよい）: 舞台=${genba} / 効かせどころ=${lever} / 喜ぶ人=${darega} / 収益の型の一例=「${bm.title}」（${bm.gist}）`,
    ...avoid,
    ...feedbackGuidance(),
    '',
    '守ること:',
    '- 【最重要・シンプルに】仕組みは1つだけ。ビジネスモデルを複数掛け合わせない。小学生でも一言で説明できる粒度にする。凝った構造・専門用語・横文字の重ね掛けは禁止。',
    '- 【偏らない】毎回まったく違う業種・切り口にする。転売/裁定/査定SaaS/マッチング“プラットフォーム”のような同じ型に寄せない。',
    '- 儲かりやすいこと（初期投資10万円以下・粗利率50%以上・30日以内に最初の売上）。既にお金が動いている実証需要を狙い、啓蒙が要る新市場は不可。',
    '- 聞いた瞬間に利用シーンが浮かび「あったらいいね」と言いたくなること。誰かの具体的な困りごと・小さな喜びに刺さっていること。',
    '- 「何を入力して、何のボタンを押すと、何が起きるか」が一読で分かること（試作品は1画面＋数操作で中核体験を疑似体験できる粒度）。',
    '- 医療・法律・税務の断定や、規制・安全上問題のある取引は避けること。',
    '- 出力は日本語・前置き/箇条書き/見出し/引用符なし・計200文字以内。1〜2文でアイデア本体（誰が何を入力し何を押すと何が起きるか）を述べ、最後に「収益性: 」で単価×件数のざっくり月商だけ一言添える。「モデル:」などの接頭辞は付けない。',
    '',
    `（内部識別子: ${salt} — 出力には含めない）`,
  ].join('\n');
}
...
function buildIdeaPromptCrossborder(): string {
  ...
  return [
    ...
    'テーマは「国・市場・時間の“ズレ”を突いて稼ぐ、小さな越境/裁定ビジネス」です（円安・日本モノ需要・日本の中古/工芸の厚み・時間差など）。',
    '「ボタンが実際に動く小さな試作品を 1 つ作る」ための、具体的なアプリ/画面のアイデアを 1 つだけ提案してください。',
    '',
    `今回のヒント（無理なら崩してよい）: 仕入れ/舞台=${stage} / 効かせどころ=${arb} / 喜ぶ人=${who}`,
    ...avoid,
    ...feedbackGuidance(),
    '',
    '守ること:',
    '- 【最重要・シンプルに】仕組みは1つだけ。ビジネスモデルを複数掛け合わせない。小学生でも一言で説明できる粒度。凝った構造・横文字の重ね掛けは禁止。',
    '- 【偏らない】毎回まったく違う商材・切り口にする。査定SaaSやマッチング“プラットフォーム”のような同じ型に寄せない。',
    '- 儲かりやすいこと（初期投資10万円以下・在庫リスク小か無在庫・粗利率50%以上・30日以内に最初の売上）。人が現に買っている実証需要を狙う。',
    '- どこの“ズレ”（国・価格・情報・時間・供給の非対称）で利益が出るのかが明確なこと。普通の転売/代行そのままは不可、一捻り入れる。',
    '- 「何を入力して、何のボタンを押すと、何が起きるか」が一読で分かること（1画面＋数操作で中核体験を疑似体験）。',
    '- 規制・安全・知財/真贋・関税や各国法に触れる危ない取引は避けること。',
    '- 出力は日本語・前置き/箇条書き/見出し/引用符なし・計200文字以内。1〜2文でアイデア本体、最後に「収益性: 」で単価×件数のざっくり月商だけ一言。「モデル:」などの接頭辞は付けない。',
    '',
    `（内部識別子: ${salt} — 出力には含めない）`,
  ].join('\n');
}
 ...
 *  0 で全部オフライン実業。2026-08-25 Keita「偏ってる」を受け 1→0.45 に下げていたが、
 *  同日さらに「海外で流行っているスモールビジネスを教える形に」へ方針転換（MC-480）。 */
const CROSSBORDER_WEIGHT = 0.45;

// ─── 海外の流行スモールビジネスを教える（2026-08-25 Keita 指示・MC-480）───────
// 「アイデアを生成＝アメリカ等の海外でいま流行っている“個人〜数人でやれる小商い”を1つ教える」形に。
// 架空の突飛な発明ではなく、現地で実際にお金が回っている小規模ビジネスを題材にして日本へのヒントにする。
...
function buildIdeaPromptOverseas(): string {
  const region = pickRandom(AXIS_REGION);
  const cat = pickRandom(AXIS_SMALLBIZ);
  const salt = Math.random().toString(36).slice(2, 8);
  const avoid = recentIdeas.length
    ? [
        '',
        `最近教えたビジネス（要約）: ${recentIdeas.map((s) => `「${s}…」`).join(' / ')}`,
        'これらと業種・国が被らないものにすること。',
      ]
    : [];
  return [
...
function buildIdeaPrompt(): string {
  return buildIdeaPromptOverseas();
}
 ...
 * 良い例/悪い例として保存し、次回以降の buildIdeaPrompt に学習として差し込む。 */
function handleIdeaFeedback(req: Request, res: Response): void {
  const body = (req.body ?? {}) as { idea?: unknown; rating?: unknown };
  const idea = typeof body.idea === 'string' ? body.idea : '';
  const rating = body.rating === 'good' || body.rating === 'bad' ? body.rating : null;
  if (!idea.trim() || !rating) {
    res.status(400).json({ error: 'idea と rating(good|bad) は必須です' });
    return;
  }
  const fb = addIdeaFeedback(idea, rating);
  res.json({ ok: true, good: fb.good.length, bad: fb.bad.length });
}
...
export function devMockupRouter(): Router {
  ...
  router.post('/idea/feedback', handleIdeaFeedback);
