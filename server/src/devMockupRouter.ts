...
// 試作品案を狙う（2026-08-02 Keita 指示 MC-359、下記 IDEA_SEEDS コメント参照）。
// 出力はそのまま POST /mockup/generate に渡せる短い説明文（1〜2文＋市場規模のフェルミ推定）。
// 毎回違うアイデアになるよう、ランダムな切り口を種に混ぜる。
 ...
 * 切り口だったが、2026-08-02 Keita 指示（MC-359）:「実ビジネス（オフラインでの実業）と
 * オンライン（ネットでのビジネス）を掛け合わせたアイデアを。市場規模や可能性など既存の
 * ビジネスを AI を使ってどうレバレッジをかけていけるかがポイント」——により、
 * オフライン実業を主語に AI レバレッジの効きどころを示す切り口へ刷新。 */
const IDEA_SEEDS = [
  '地域の実店舗の死に時間帯・遊休設備のオンライン販売化（美容室の平日昼・飲食店のアイドルタイム・工場の夜間ライン——AI が値付けと集客まで担い、稼働率のレバレッジを生む）',
  '職人・専門家の暗黙知を AI で商品化して商圏を全国に拡張（庭師・仕立て・修理・伝統工芸——現場でしか売れなかった技能を、AI 診断・遠隔指南サービスとして販売する）',
  '紙・電話・FAX のまま回っている業務フローの AI 化で利益率にレバレッジ（受発注・見積・シフト・請求——町の会社の裏側業務を AI が肩代わりし、粗利を直接押し上げる）',
  '実業の副産物を AI でオンライン収益源に転換（施工実績・接客ノウハウ・端材・型紙・産地の信用——本業のついでに溜まる資産を AI が商品に変える）',
  'BtoB 実業の営業・見積・仕入を AI で自動化して小規模事業者の商圏を拡大（下請け町工場・卸・建材店が、営業マンなしで全国から直接受注できるようにする）',
  '一見さん商売の顧客接点を AI でデータ化して再来店レバレッジ（飲食・理美容・整体——個店に大手並みの常連化・再来店の仕組みを AI が実装する）',
  '農業・漁業の産直を AI 品質判定でオンライン直販化（規格外品・少量多品種を AI が説明・値付けして消費者へ直接届け、中間マージンを生産者の利益に変える）',
  '介護・保育など労働集約の現場の記録・報告業務を AI 化して稼働率レバレッジ（人がケアに使える時間を増やし、同じ人員で受け入れ数と質を上げる）',
  '空き店舗・倉庫の隙間・遊休スペースの時間貸しを AI マッチングで市場化（現地でしか見えなかった遊休不動産を、オンラインの全国需要につなぐ）',
  '対面教室・習い事の商圏を AI 添削つきオンライン講座で全国に拡張（地元でしか教えられなかった先生の実技指導を、動画×AI フィードバックで商品化する）',
  '建設・修繕の見積の情報非対称を AI で解消して受注コストを下げる（写真から概算見積→地元業者へ送客——施主は相場が分かり、業者は営業ゼロで受注できる）',
  '地場物流・配送の空き容量を AI が拾って埋める（帰り便・ルート便の空きスペースをオンラインで販売し、走っているだけのコストを売上に変える）',
];

/** アイデア生成プロンプトを組む（ランダムな切り口を種に、オフライン実業×オンライン×AI レバレッジの試作品案を 1 つ＋市場規模のフェルミ推定）。 */
function buildIdeaPrompt(): string {
  ...
  return [
    ...
    'テーマは「既に現実世界で回っているオフラインの実業（店舗・職人・士業・農業・建設・介護・教育・不動産・飲食・製造・物流など）に、オンライン×AI を差し込んでレバレッジをかけるサービスの試作品」です。',
    'ゼロから新市場を発明するのではなく、既に売上が立っている既存ビジネスの現場に AI を入れることで、集客・単価・回転率・利益率・商圏のどこが何倍になるかを示すアイデアを狙います。',
    'これから「ボタンが実際に動く小さな試作品を 1 つ作る」ための、具体的なアプリ/画面のアイデアを 1 つだけ提案してください。',
    '',
    `今回の切り口（ヒント。これに沿いつつ、より具体的な業種・現場に絞ってよい）: ${seed}`,
    '',
    'アイデアが満たすべき条件:',
    '- 主語がオフラインの実業であること（現実世界で既に回っているビジネスの現場が起点。純オンライン発のサービスは不可）。',
    '- 既存ビジネスの集客・単価・回転率・利益率・商圏のうち、どこに AI のレバレッジが効くかが明確なこと。',
    '- 「どの実業に、何を入力して、何のボタンを押すと、何が起きるか」が具体的に分かること。試作品は 1 画面＋数個の操作でその中核体験を疑似体験できる粒度に絞る。',
    '- 医療・法律・税務の断定や、規制・安全上問題のある取引は避けること。',
    '- 出力は日本語で 2 部構成・計 300 文字以内。前置き・箇条書き・見出し・引用符は付けないこと:',
    '  1 文目〜2 文目: アイデア本体（どの実業に、何を入力して何を押すと、何が起きるか）。',
    '  最終文: 「市場規模: 」で始め、対象事業者数（または人数）×頻度×単価のフェルミ推定で年間規模を概算し、AI レバレッジでどこが伸びるかの可能性を一言添える。',
    '- 例の形式: 「町の工務店がリフォーム現場の写真を撮って『見積を作る』を押すと、AIが部材と工数を拾って概算見積書と施主向け説明資料を数分で作り、1件数時間かかっていた見積作業をほぼゼロにする。市場規模: 工務店・リフォーム店10万社×月10件×工数削減5000円相当で年600億円、見積の速さが受注率を押し上げ単価側にも伸びしろ」',
    '',
...
function cleanIdea(text: string): string {
  ...
  // （従来は最初の段落のみ採用で市場規模行が欠落しうる）。MC-359 の刷新後も 2 部構成
  // （本文 300 字以内＋「市場規模: 」文）なので 400 字上限のままで収まる。
