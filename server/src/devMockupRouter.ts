 ...
 * 2026-07-25 Keita 指示（MC-343）:「もっと世の中の人が気づいてないような潜在的なやつにして」——
 * MC-342 の顕在的な市場機会から、皆が諦めて言語化すらしていない潜在ニーズ・盲点の切り口へ再刷新。
 * 2026-07-25 Keita 指示（MC-344）:「例えばメルカリのようなサービスを作れるようなアイデアに。
 * 潜在市場規模のようなところも分析しつつ」——盲点の発見は維持しつつ、メルカリ級に化けうる
 * 「潜在供給×潜在需要を初めて接続するプラットフォーム」の切り口へ寄せ、出力に市場規模の
 * フェルミ推定を含めるよう再々刷新。 */
const IDEA_SEEDS = [
  '家庭・個人に眠る遊休資産の流動化（モノ以外——時間・スキル・場所・権利・データ。メルカリが「押入れの不用品」でやったことの別資産版）',
  '個人間で既に非公式に行われているが市場が存在しない取引の公式化（謝礼・口利き・お下がり・順番の譲り合い——既に取引がある＝需要は実在する）',
  '「売り物だと思われていないもの」の市場化（失敗談・交渉記録・生活ログ・使いかけ——所有者自身が価値に気づいていない資産）',
  '信頼の欠如だけが理由で成立していない個人間取引に、AIが信頼レイヤーを敷く（審査・エスクロー・翻訳・仲裁が高コストで諦められていた領域）',
  'プロと素人の間に埋もれた「セミプロ供給」の解放（資格はないが上手い人・引退した専門家・育児中の元プロ——供給側が自分を供給と思っていない）',
  '地域・コミュニティ内で完結していた助け合いの全国市場化（ご近所の貸し借り・親戚の知恵・町内の口コミを距離の制約から解放）',
  '法人向けにしか存在しないサービスの個人開放（顧問・秘書・調達・与信——単価が高くて個人に降りてこなかったものをAIで1/100の価格に）',
  'ライフイベント時だけ発生する一時需要の専用市場（引越し・出産・介護開始・卒業——需要が一過性ゆえ常設市場がなかった領域）',
  '廃棄・死蔵されているものの逆流通（捨てる側がお金を払っている物に、実は欲しい人がいる——処分費用と入手困難が同時に存在する歪み）',
  '時間差の需給マッチング（今は要らないが将来要る・今持っているが将来手放す——予約・先物・世代間の受け渡しを個人間で）',
  '恥ずかしさ・気まずさが理由で頼めずにいた依頼の匿名市場（代行・付き添い・確認・練習相手——需要が感情的障壁の裏に隠れている）',
  'みんなが既にSNS・掲示板で非効率にやり取りしている取引の専用市場化（ハッシュタグ売買・DM交渉・譲渡スレ——検索とAIマッチングで専用化）',
];

/** アイデア生成プロンプトを組む（ランダムな切り口を種に、メルカリ級に化けうる潜在市場の試作品案を 1 つ＋市場規模のフェルミ推定）。 */
function buildIdeaPrompt(): string {
  ...
  return [
    'あなたは、動く HTML 試作品（モックアップ）を作るためのアイデア出しを手伝う、市場の盲点を見抜く連続起業家です。',
    'テーマは「メルカリのように、誰も市場だと気づいていなかった潜在需給を初めて接続し、巨大市場に化けうるサービスの試作品」です。',
    'メルカリは「押入れの不用品」という、誰も資産と思っていなかった眠れる供給を数千万人の売り手に変えました。同じ構造——',
    '「供給側は自分が売り手になれると気づいておらず、需要側は買える場所がないので探しもしない」市場を狙います。',
    '既に市場が可視化されている領域（皆が既に売り買いしている領域）の改良版は狙いません。',
    'これから「ボタンが実際に動く小さな試作品を 1 つ作る」ための、具体的なアプリ/画面のアイデアを 1 つだけ提案してください。',
    '',
    `今回の切り口（ヒント。これに沿いつつ、より意外な潜在市場に絞ってよい）: ${seed}`,
    '',
    'アイデアが満たすべき条件:',
    '- 「潜在供給×潜在需要」の両側が実在すること（どちらかが既に非公式・非効率な形で行動している証拠が想像できると強い）。',
    '- 「なぜ今まで市場にならなかったのか」に理由があること（信頼が築けなかった・単価が低すぎた・恥ずかしかった・分散していた・マッチングコストが高すぎた等）、かつ AI がその障壁をちょうど今壊せること。',
    '- 個人が供給側にも需要側にも回れて、取引が繰り返し起きる構造であること（ネットワーク効果で雪だるま式に育つ）。',
    '- 既存カテゴリ名で呼べないこと（「メルカリの〜版」「AI版〜」と一言で説明できてしまうものは顕在的すぎるので禁止。構造が同じでも対象資産が盲点なら可）。',
    '- 試作品は市場の核となる 1 取引に絞り、1 画面＋数個の操作で「出品→マッチ」の疑似体験ができること。',
    '- 「何を入力して、何のボタンを押すと、何が得られるか」が具体的に分かること。',
    '- 医療・法律・税務の断定や、不安・監視・詮索を煽る設計、規制・安全上問題のある取引（労働・身体・個人情報の切り売り等）は避けること。',
    '- 出力は日本語で 2 部構成・計 300 文字以内。前置き・箇条書き・見出し・引用符は付けないこと:',
    '  1 文目〜2 文目: アイデア本体（何を入力し何を押すと何が起きる市場か）。',
    '  最終文: 「潜在市場規模: 」で始め、対象人数×頻度×単価のフェルミ推定で年間の市場規模を概算する（例: 潜在市場規模: 持ち家高齢者3600万人の1割が月1回×3000円で貸せば年1.3兆円）。',
    '- 例の形式: 「引越しで手放す観葉植物や熱帯魚の写真を撮って『里親を探す』を押すと、AIが種類・状態・世話の難度を判定して近所の育てたい人と即マッチングし、譲渡の受け渡しまで段取りする——捨てるしかなかった生き物に市場を作る。潜在市場規模: 年間引越し600万世帯の2割が1件2000円で譲れば年24億円、周辺用品込みで数百億円」',
    '',
...
function cleanIdea(text: string): string {
  ...
  // MC-344: 「アイデア本体＋潜在市場規模」の 2 部構成になったため、段落を落とさず全体を
  // 1 行に結合し、上限も 200 → 400 字に拡大（従来は最初の段落のみ採用で市場規模行が欠落しうる）。
  const joined = s.split(/\n+/).map((line) => line.trim()).filter(Boolean).join(' ').trim();
  return (joined || s).slice(0, 400);
}
