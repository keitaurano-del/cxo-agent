...
const KNOWN_TOPICS: { topic: string; title: string }[] = [
  ...
  { topic: 'procedures', title: '制度・手続き' },
  { topic: 'shopping', title: '買い物・育児グッズ' },
...
function buildMergePrompt(existing: GuideNoteTopic[], pairs: QaPair[]): string {
  return [
    'あなたは育児相談の記録を整理するアシスタントです。育児チャットでの保護者の相談（Q&A）を、トピック別に要点整理した「相談メモ」にまとめます。相談には、乳幼児の発達・健康・生活に関するものだけでなく、育児にまつわる制度・手続き・お金（育児休業給付・各種手当・助成・保活・社会保険など）や、買い物・育児グッズ（出産準備・ベビー用品の選び方など）の相談も含まれます。これらも適切なトピックに振り分けて整理してください。',
    '',
    '【あなたの仕事】',
    ...
    '- 制度・手続き・お金（育休給付・各種手当・助成・保活・社会保険など）の要点は、「一般的にはこういう仕組み・流れ」という枠で書き、金額・条件・締切を個別に断定しないでください。必要に応じて「最新の内容は勤務先・ハローワーク・自治体・年金事務所・健康保険組合の窓口で確認」という一般的案内を添えます。',
    '- 買い物・育児グッズの要点は中立に書き、特定商品の推奨や購入の押し付けはしません。安全に関わる点は「公的な基準・情報を確認」という枠で書きます。',
    '',
    '【既存の相談メモ】',
