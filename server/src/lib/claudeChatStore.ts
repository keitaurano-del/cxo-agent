// claudeChatStore — 汎用 Claude チャットの会話履歴 JSONL ストア。
//
// workChatStore.ts の作法をそのまま踏襲する。送信側（ユーザー）の画像/動画添付と、返信側
// （Claude）のメディア返却（YouTube 埋め込み・生成図解・Web 画像）の両方を扱う
// （work と同じ ChatMedia 構造）。単一の会話スレッド（個人用ダッシュボードなので分岐不要）を
// サーバ側に永続化する。端末ローカルの localStorage と違い、別端末・リロードをまたいで過去の質問が残る。
//
// データストア（data/ 配下・.gitignore 済み）:
//   data/claude-chat.jsonl  : 追記専用。1 行 = 1 レコード。
...
import { CLAUDE_CHAT_FILE } from '../config.js';
...
/**
 * メッセージに添付されたメディア参照（workChatStore の ChatMedia を踏襲）。
 *   - 送信側（ユーザー）の画像/動画アップロード（kind: 'image'|'video', source: 'upload'）
 *   - 返信側（Claude）のメディア返却:
 *       - YouTube 参考動画埋め込み（kind: 'youtube', source: 'web'）— oEmbed で実在検証済み
 *       - Gemini 生成図解（kind: 'image', source: 'generated'）— data/claude-chat-media/ に保存
 *       - Web 実在画像の提示（kind: 'image', source: 'web'）— 検証後サーバへ取り込み自前配信
 * 実体は data/claude-chat-media/ 配下に保存し、ここには参照だけを持つ。YouTube は埋め込みのため
...
export interface ChatMedia {
  ...
  /**
   * 出所。
   *   - 'upload'    : ユーザーがアップロードした添付。
   *   - 'generated': Claude が Gemini で生成した図解。
...
function readAll(): ChatRecord[] {
  if (!existsSync(CLAUDE_CHAT_FILE)) return [];
  let raw: string;
  try {
    raw = readFileSync(CLAUDE_CHAT_FILE, 'utf-8');
  } catch {
    return [];
  }
...
function appendRecord(rec: ChatRecord): void {
  const dir = dirname(CLAUDE_CHAT_FILE);
  if (!existsSync(dir)) mkdirSync(dir, { recursive: true });
  appendFileSync(CLAUDE_CHAT_FILE, JSON.stringify(rec) + '\n', 'utf-8');
}
