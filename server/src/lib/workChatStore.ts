...
import { WORK_CHAT_FILE } from '../config.js';
...
function readAll(): ChatRecord[] {
  if (!existsSync(WORK_CHAT_FILE)) return [];
  let raw: string;
  try {
    raw = readFileSync(WORK_CHAT_FILE, 'utf-8');
  } catch {
    return [];
  }
...
function appendRecord(rec: ChatRecord): void {
  const dir = dirname(WORK_CHAT_FILE);
  if (!existsSync(dir)) mkdirSync(dir, { recursive: true });
  appendFileSync(WORK_CHAT_FILE, JSON.stringify(rec) + '\n', 'utf-8');
}
