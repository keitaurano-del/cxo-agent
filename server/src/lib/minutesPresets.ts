...
export const MINUTES_TYPES: MinutesTypePreset[] = [
  ...
  },
  {
    type: 'summary',
    label: '要約議事録',
    description: '要点を圧縮した議事録（標準フォーマット）',
    templates: [
      {
        id: 'summary-standard',
        label: '標準（実務）',
        body: '# 〔会議名〕\n\n開催日時： YYYY年MM月DD日（曜日）HH:mm〜HH:mm\n場所： 〇〇\n出席者： 〇〇、△△\n欠席者： 〇〇\n\n## TODO・アクション項目\n\n| No. | タスク | 内容 | 担当者 | 期限 |\n|-----|--------|------|--------|------|\n| 1 | 〔タスク名〕 | 〔具体的な内容〕 | 〇〇 | 未定 |\n| 2 | 〔タスク名〕 | 〔具体的な内容〕 | 〇〇 | 未定 |\n\n## 議題一覧\n\n議題1：〔題名〕 — 決定事項：〔決定内容〕\n議題2：〔題名〕 — 決定事項：〔決定内容〕\n\n## 各議題の要点\n\n議題1：〔題名〕\n要点： 〔議論の背景・経緯・出た意見・決まった方向性を簡潔にまとめる〕\n\n議題2：〔題名〕\n要点： 〔同上〕\n\n## 保留事項\n\n・ 〔保留・継続検討する事項〕（理由・次の対応）\n・ 〔同上〕\n\n## 次回会議予定\n\n日時： 未定\n議題（案）： 〔次回の主な議題〕\n',
      },
      {
        id: 'summary-formal',
        label: 'フォーマル',
...
export function buildMinutesPrompt(params: {
  ...
  if (templateBody) {
    lines.push('');
    lines.push('## テンプレート（この体裁を忠実に再現してください）');
    lines.push(
      '見出しの文言・順序・改行・空行、記号（箇条書きは「・」、コロンは全角「：」）、' +
        '「議題N：題名 — 決定事項：…」「要点： …」といった各行の形をそのまま守ってください。' +
        '勝手に Markdown 装飾（**強調**、箇条書きの「- 」や番号付け 等）を足したり削ったりしないこと。' +
        '「TODO・アクション項目」は表（| … |）の形のまま、行を必要数だけ増減して埋めてください。' +
        '議題や TODO の件数は会議内容に合わせて増減してよいですが、各行の体裁は崩さないでください。',
    );
    lines.push('');
