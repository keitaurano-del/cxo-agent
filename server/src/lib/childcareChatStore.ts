// childcareChatStore — 育児相談チャット「すくすく」の会話履歴 JSONL ストア。
//
// 単一の会話スレッド（個人/世帯用ダッシュボードなので分岐不要）をサーバ側に永続化する。
// 端末ローカルの localStorage と違い、別端末・リロードをまたいで過去の質問が残る。
//
// データストア（data/ 配下・.gitignore 済み）:
//   data/childcare-chat.jsonl  : 追記専用。1 行 = 1 レコード。
...
//
// babyDiaryStore.ts の作法に倣う（追記専用・全走査・壊れた行は無視）。
// 「クリア」は物理削除でなくマーカ行を追記し、読み出し時にマーカ以降のみ採用する
//   （履歴は監査上残しつつ UI からは消える＝babyDiaryStore の deleted フラグと同思想）。
//
// ─── ジョブ方式（MC: 接続から切り離した非同期生成）──────────────────────
// 育児チャットの AI 生成（Web検索/メディア生成を伴うと 1〜2 分かかる）は、クライアント接続の
// ライフサイクルから切り離す。接続が切れても回答が失われない・失敗扱いにならないようにするため、
// assistant メッセージは「pending（生成中）」として先に永続化し、バックグラウンド生成の完了時に
// 「update 行」を追記して done/error と最終本文・media に確定する（追記専用の不変条件は維持）。
//   - status: 'pending' = 生成中（フロントは「考え中…」を出し、history/job をポーリングして解決）
//   - status: 'done'    = 完了（content/media が確定）
...
import { CHILDCARE_CHAT_FILE } from '../config.js';
...
/**
 * メッセージに添付されたメディア参照。
 *   - 送信側（保護者）の画像/動画アップロード（kind: 'image'|'video', source: 'upload'）
 *   - 返信側（すくすく）のメディア返却（フェーズ2）:
 *       - YouTube 参考動画埋め込み（kind: 'youtube', source: 'web'）— oEmbed で実在検証済み
 *       - Gemini 生成図解（kind: 'image', source: 'generated'）— data/childcare-chat-media/ に保存
 *       - Web 実在画像の提示（kind: 'image', source: 'web'）— 検証後サーバへ取り込み自前配信
 * 実体（アップロード画像/動画・生成画像・取り込んだ Web 画像）は data/childcare-chat-media/ 配下に
...
export interface ChatMedia {
  ...
  /**
   * URL。
   *   - 'image'/'video': 配信 URL（GET /api/childcare/chat/media/:id）。
   *   - 'youtube': 視聴ページ URL（https://www.youtube.com/watch?v=...）。埋め込みは videoId を使う。
   */
  url: string;
  ...
  /**
   * 出所。
   *   - 'upload'    : 保護者がアップロードした添付。
   *   - 'generated': すくすくが Gemini で生成した図解。
   *   - 'web'      : Web 検索で見つけ、実在検証した YouTube 動画 / 公的機関の画像。
   */
...
function readAll(): ChatRecord[] {
  if (!existsSync(CHILDCARE_CHAT_FILE)) return [];
  let raw: string;
  try {
    raw = readFileSync(CHILDCARE_CHAT_FILE, 'utf-8');
  } catch {
    return [];
  }
...
function appendRecord(rec: ChatRecord): void {
  const dir = dirname(CHILDCARE_CHAT_FILE);
  if (!existsSync(dir)) mkdirSync(dir, { recursive: true });
  appendFileSync(CHILDCARE_CHAT_FILE, JSON.stringify(rec) + '\n', 'utf-8');
}
...
/**
 * user メッセージとアシスタント応答をまとめて 1 往復ぶん追記する（同期確定保存・done）。
 * 非ストリーム（JSON）経路など、生成が即完了するケースの確定保存に使う。
 * userMedia を渡すと user 発言に添付メディアを、assistantMedia を渡すと assistant 発言に
 * 検証済みメディアを紐づけて永続化する（再オープン・リロードで埋め込み・画像が復元される）。
 */
export function appendExchange(
  userText: string,
  assistantText: string,
  userMedia?: ChatMedia[],
  assistantMedia?: ChatMedia[],
): void {
  appendMessage('user', userText, userMedia);
  appendMessage('assistant', assistantText, assistantMedia);
}
